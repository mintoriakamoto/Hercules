"""CLI entry point for the hercules-agent ACP adapter.

Loads environment variables from ``~/.hercules/.env``, configures logging
...
try:
    ...
    # shadowing Hercules's own modules — ``hercules acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    hercules_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from HERCULES_HOME (default ``~/.hercules``)."""
    from hercules_cli.env_loader import load_hercules_dotenv

    hercules_home = get_hercules_home()
    loaded = load_hercules_dotenv(hercules_home=hercules_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", hercules_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--version", action="store_true", help="Print Hercules version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Hercules provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.hercules/node/ "
...
def _print_version() -> None:
    ...
    print(hercules_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import HerculesACPAgent  # noqa: F401

    print("Hercules ACP check OK")
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "hercules", "model"]
        hercules_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``hercules postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger = logging.getLogger(__name__)
    logger.info("Starting hercules-agent ACP adapter")
    ...
    import acp
    from .server import HerculesACPAgent
