...
from tools.approval import (
    reset_hercules_interactive_context,
    set_hercules_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from hercules_cli import __version__ as HERCULES_VERSION
except Exception:
    HERCULES_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Hercules running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Hercules cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Hercules/OpenAI-compatible user content payload."""
...
class HerculesACPAgent(acp.Agent):
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Hercules version",
        },
    )
    ...
    _MODE_TO_EDIT_APPROVAL_POLICY: ClassVar = {
        _MODE_DEFAULT: "ask",
        _MODE_ACCEPT_EDITS: "workspace_session",
        _MODE_DONT_ASK: "session",
    }
    _EDIT_APPROVAL_POLICY_TO_MODE: ClassVar = {
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Hercules maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from hercules_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from hercules_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Hercules estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_hercules_session_id: str,
        previous_hercules_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.hercules.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_hercules_session_id,
                previous_hercules_session_id=previous_hercules_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_hercules_session_id: Optional[str] = None,
        previous_hercules_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Hercules changes it.

        When the internal Hercules head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_hercules_session_id`` so the
        attached ``_meta.hercules.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # hercules_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_hercules_session_id or session_id,
            previous_hercules_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["hercules-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="hercules-agent", version=HERCULES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Hercules' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Hercules setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Hercules
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Hercules on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Hercules' local kawaii waiting/status
        ...
        # tools.approval (set_hercules_interactive_context) rather than
        # os.environ["HERCULES_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue HERCULES_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind HERCULES_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_hercules_interactive_context(True)
            ...
            previous_session_id = os.environ.get("HERCULES_SESSION_ID")
            os.environ["HERCULES_SESSION_ID"] = session_id
            try:
                ...
                if interactive_token is not None:
                    reset_hercules_interactive_context(interactive_token)
                # Restore HERCULES_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("HERCULES_SESSION_ID", None)
                else:
                    os.environ["HERCULES_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Hercules DB session id before the turn so we
            ...
            pre_turn_hercules_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (HERCULES_SESSION_KEY in
        ...
        # _meta.hercules.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_hercules_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_hercules_id
            and pre_turn_hercules_id
            and post_turn_hercules_id != pre_turn_hercules_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_hercules_session_id=post_turn_hercules_id,
                    previous_hercules_session_id=pre_turn_hercules_id,
                )
            except Exception:
                logger.debug(
        ...
        # Hercules' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Hercules Agent v{HERCULES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Hercules has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
