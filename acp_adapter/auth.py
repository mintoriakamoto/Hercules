"""ACP auth helpers — detect and advertise Hercules authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "hercules-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Hercules runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Hercules can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Hercules.
    ...
    not have Hercules provider credentials configured yet, so Hercules always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Hercules using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Hercules provider",
            description=(
                "Open Hercules' interactive model/provider setup in a terminal. "
                "Use this when Hercules has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
