...
Usage:
    ...
    python cli.py --skills hercules-agent-dev,github-auth
...
from hercules_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from hercules_cli.pt_input_extras import (
...
from hercules_cli.banner import _format_context_length, format_banner_version_label
...
load_hercules_dotenv(hercules_home=_hercules_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.hercules/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("HERCULES_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def _parse_reasoning_config(effort) -> dict | None:
    ...
    """
    from hercules_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.hercules/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERCULES_IGNORE_USER_CONFIG=1 is set (via ``hercules chat --ignore-user-config``),
    the user config at ``~/.hercules/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({HERCULES_HOME}/config.yaml)
    user_config_path = _hercules_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("HERCULES_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see hercules_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
            ...
            # hercules_cli/config.py DEFAULT_CONFIG (display.show_reasoning).
            "show_reasoning": True,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from hercules_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and HerculesCLI.__init__ checks "default" first.
    ...
    # hercules_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `hercules config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (hercules_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && hercules` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _HERCULES_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_HERCULES_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["HERCULES_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.hercules/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from hercules_logging import setup_logging
...
try:
    from hercules_cli.config import print_config_warnings
...
try:
    from hercules_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``hercules --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from hercules_cli.mcp_startup import wait_for_mcp_discovery
...
from hercules_cli.banner import build_welcome_banner
from hercules_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from hercules_cli.callbacks import prompt_for_secret
...
def _arm_exit_watchdog(timeout_s: float | None = None) -> None:
    ...
    Tune with ``HERCULES_EXIT_WATCHDOG_S`` (seconds); ``0`` disables.
    """
    if timeout_s is None:
        try:
            timeout_s = float(os.getenv("HERCULES_EXIT_WATCHDOG_S", "30"))
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.hercules/hercules-agent`` clone is updated only by ``hercules update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run hercules -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"hercules-{short_id}"
    branch_name = f"hercules/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"hercules pid={os.getpid()}", str(wt_path)],
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    ...
    ``hercules -w`` locks each worktree with reason ``hercules pid=<pid>`` so a
    concurrent hercules process' startup prune leaves an in-use worktree alone.
    ...
    - ``"dead"``  — locked but the owning pid is gone, or the reason isn't a
                    parseable hercules lock (safe to unlock + reap).
    ...
    for line in result.stdout.splitlines():
        if line.startswith("worktree "):
            ...
            m = re.search(r"hercules pid=(\d+)", reason)
            if not m:
                # Locked by something we don't recognize as a hercules session
                # (or lock reason unavailable). Treat as dead — a foreign lock
                # on a hercules -w worktree is almost certainly a leftover, and
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`hercules_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from hercules_cli.config import load_config as _load_full_config
        from hercules_constants import get_hercules_home as _get_hercules_home
        _hercules_home_maint = _get_hercules_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_hercules_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`hercules_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Lock handling (orthogonal to age): ``hercules -w`` locks each worktree with
    reason ``hercules pid=<pid>`` so a concurrent hercules process leaves an in-use
    ...
    *dead*-locked one (owning pid gone — a crashed session) is unlocked first
    ...
    Also prunes orphaned ``hercules/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("hercules-"):
            continue
        ...
        # hercules process means the worktree is actively in use — never touch
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``hercules/hercules-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``hercules -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("hercules/hercules-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. HERCULES_LIGHT / HERCULES_TUI_LIGHT env (true/false) — explicit override
#   2. HERCULES_TUI_THEME=light|dark — explicit theme
#   3. HERCULES_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Hercules assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("HERCULES_LIGHT", "HERCULES_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("HERCULES_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("HERCULES_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from hercules_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_hercules_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._hercules_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Hercules output it's common to display cron schedules verbatim.
    #
...
from hercules_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_hercules_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_hercules_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._hercules_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_hercules_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._hercules_bp_start = now
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        tiny_line = "⚕ NOUS HERCULES"
    else:
        agent_name = _skin.get_branding("agent_name", "Hercules Agent") if _skin else "Hercules Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERCULES_FAST_STARTUP_BANNER") == "1":
        from hercules_cli import __release_date__ as _release_date
        from hercules_cli import __version__ as _version

        version_line = f"Hercules Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Hercules[/]\n"
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.hercules/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.hercules/config.yaml on first use)
...
class HerculesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: HERCULES_MAX_TOKENS)
        _env_mt = os.environ.get("HERCULES_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from hercules_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("HERCULES_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("HERCULES_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("HERCULES_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `hercules chat --ignore-rules` in hercules_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("HERCULES_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("HERCULES_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `hercules update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Hercules processes for this HERCULES_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.hercules/checkpoints/.  Opt-in via
        ...
        self._history_file = _hercules_home / ".hercules_history"
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from hercules_cli.active_sessions import try_acquire_active_session
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``hercules pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from hercules_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from hercules_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from hercules_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from hercules_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from hercules_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - HerculesCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``hercules doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``hercules doctor`` so the banner stays
        small.
        """
        try:
            from hercules_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Hercules needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Hercules LLM (not agentic)
        from hercules_cli.model_switch import is_nous_hercules_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hercules_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Hercules 3 & 4 models are NOT agentic and are not "
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.hercules/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from hercules_cli.clipboard import save_clipboard_image

        img_dir = get_hercules_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'hercules setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from hercules_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from hercules_cli.skin_engine import get_active_help_header
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from hercules_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from hercules_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Hercules #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``hercules sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `hercules sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("HERCULES_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.hercules/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``hercules --resume <id>``
        ...
        saved_dir = get_hercules_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"hercules_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: hercules --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from hercules_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Hercules session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from hercules_cli.context_switch_guard import merge_preflight_compression_warning
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `hercules model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from hercules_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from hercules_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        from hercules_cli.providers import get_label
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from hercules_cli import codex_runtime_switch as crs
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_hercules_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_hercules_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in hercules_cli/commands.py instead of touching every dispatch site.
        from hercules_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from hercules_cli.main import _print_version_info
            ...
            from hercules_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `hercules plugins
                ...
                # catalog lives behind `hercules plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: hercules plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_hercules_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: hercules plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: hercules plugins list)")
                    print("  Enable/disable: hercules plugins enable/disable <name>")
            except Exception as e:
            ...
            from hercules_cli.moa_config import (
                moa_usage,
            ...
            # Check for plugin-registered slash commands
                ...
                from hercules_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from hercules_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from hercules_cli.goals import GoalManager
            from hercules_cli.config import load_config
        except Exception as exc:
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from hercules_cli.goals import gather_background_processes as _gather_bg
    ...
    def _toggle_verbose(self):
        ...
        from hercules_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``HERCULES_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``HERCULES_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from hercules_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by hercules_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from hercules_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.hercules/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_hercules_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "hercules_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "hercules_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _hercules_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard HERCULES_HOME, so no -p needed.
            try:
            ...
            print(f"  hercules --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  hercules -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from hercules_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from hercules_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from hercules_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            from hercules_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("HERCULES_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(HERCULES_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Hercules migration (especially migrations done by
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in hercules_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from hercules_cli.model_switch import resolve_persist_behavior
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_hercules_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Hercules binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        try:
            from hercules_cli.config import load_config
            from hercules_cli.voice import (
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if (lines_hit or chars_hit) and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _hercules_home / "pastes"
        ...
        # Dynamic prompt: shows Hercules symbol when agent is working,
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = HerculesCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_hercules_osd_patched", False):
                ...
                _pt_renderer._hercules_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``HERCULES_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("HERCULES_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the hercules process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: hercules setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `hercules sessions list`
...
def _run_kanban_goal_loop_q(cli: "HerculesCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``HERCULES_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("HERCULES_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from hercules_cli import kanban_db as _kb
    from hercules_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    os.environ["HERCULES_INTERACTIVE"] = "1"
    ...
    # Default to hercules-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Hercules): with no explicit --toolsets, collapse
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `hercules skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    # mode registers its own inside HerculesCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  HERCULES_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``hercules chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("HERCULES_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from hercules_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Hercules" box, no tool-gen
                        ...
                        if os.environ.get("HERCULES_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("HERCULES_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from hercules_cli.kanban_db import (
                ...
                # Single-query mode (`hercules chat -q "…"`): skip the welcome
