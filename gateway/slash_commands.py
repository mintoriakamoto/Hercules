...
Module-level run.py helpers a handler needs (``_hercules_home``,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: hercules gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Hercules commands.
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`hercules gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            atomic_json_write(
                _hercules_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _hercules_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Hercules Agent version."""
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Hercules default runtime
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _hercules_home, _platform_config_key
        import yaml
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.hercules/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``hercules skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hercules_home
        from hercules_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `hercules bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``hercules update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``hercules update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _hercules_home, _resolve_hercules_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        hercules_cmd = _resolve_hercules_bin()
        if not hercules_cmd:
            return t("gateway.update.hercules_cmd_not_found")

        pending_path = _hercules_home / ".update_pending.json"
        output_path = _hercules_home / ".update_output.txt"
        exit_code_path = _hercules_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `hercules update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `hercules update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # hercules_cmd is a list of argv parts we can pass directly
                ...
                hercules_cmd_str = " ".join(shlex.quote(part) for part in hercules_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {hercules_cmd_str} update --gateway"
