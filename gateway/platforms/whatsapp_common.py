...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    When the install tree is read-only (e.g., Docker /opt/hercules), this function
    mirrors the bridge source to a writable HERCULES_HOME location and returns that
    ...
    hercules_home_bridge = hercules_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to HERCULES_HOME if needed
    if hercules_home_bridge.exists():
        return hercules_home_bridge

    # Mirror the bridge source to HERCULES_HOME
    try:
        hercules_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            hercules_home_bridge,
            dirs_exist_ok=False,
        )
        return hercules_home_bridge
    except Exception:
