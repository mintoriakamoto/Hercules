...
# group mention gating without custom aliases, use conservative Hercules wake
...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> List[re.Pattern]:
        ...
        JSON list, or comma/newline-separated), or None (use Hercules defaults).
        """
        if raw is None:
    ...
    async def _handle_webhook(self, request):
        ...
        if self.send_read_receipts and session_chat_id:
            self._spawn_background_task(self.mark_read(session_chat_id))
