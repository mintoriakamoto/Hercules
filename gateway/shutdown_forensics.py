...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        hercules_home_str = os.environ.get("HERCULES_HOME")
        if hercules_home_str:
            takeover_path = Path(hercules_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(hercules_home_str) / ".gateway-planned-stop.json"
            if planned_stop_path.exists():
                try:
                    ...
                    pass
    except Exception:  # never raise from a signal handler
        pass
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded hercules-agent but never re-ran ``hercules setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../hercules-gateway.service"
    ...
    # that's the common case for hercules.
    ...
    # constant in hercules_cli/gateway.py.
    headroom = 30.0
