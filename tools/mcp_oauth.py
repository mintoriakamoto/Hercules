...
This module provides the glue:
    - ``HerculesTokenStorage``: persists tokens/client-info to disk so they
...
from hercules_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__hercules_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses HERCULES_HOME so each profile gets its own OAuth tokens.
    Layout: ``HERCULES_HOME/mcp-tokens/``
    """
    try:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``hercules mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `hercules mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# HerculesTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class HerculesTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        HERCULES_HOME/mcp-tokens/<server_name>.json         -- tokens
        HERCULES_HOME/mcp-tokens/<server_name>.client.json   -- client info
        HERCULES_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Hercules records an absolute wall-clock ``expires_at`` alongside the
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/guides/oauth-over-ssh.md\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `hercules mcp login <server>` later to "
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`hercules mcp login {server_name}` interactively first to complete "
