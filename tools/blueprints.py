...
`hercules skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.hercules.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.hercules.blueprint`` is a mapping containing
    ...
    hercules = meta.get("hercules") if isinstance(meta, dict) else None
    blueprint = hercules.get("blueprint") if isinstance(hercules, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.hercules.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.hercules.blueprint`` block) they can hand
    to ``hercules skills publish`` to share. ``body`` is the plain-language
