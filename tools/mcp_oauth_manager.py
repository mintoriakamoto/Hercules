...
`mcp_tool.py`, and `hercules_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# HerculesMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_hercules_provider_class() -> Optional[type]:
    ...
    class HerculesMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._hercules_server_name = server_name
            ...
            self._hercules_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`HerculesTokenStorage` persisting an absolute
            ...
            from tools.mcp_oauth import HerculesTokenStorage
            if (
                isinstance(storage, HerculesTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._hercules_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._hercules_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import HerculesTokenStorage
                        if isinstance(storage, HerculesTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._hercules_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import HerculesTokenStorage
            if not isinstance(storage, HerculesTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``hercules mcp reauth``.
            ...
            back to ``hercules mcp reauth``.
            """
            try:
                if self._hercules_preregistered:
                    return
    ...
    return HerculesMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_HERCULES_PROVIDER_CLS: Optional[type] = _make_hercules_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`HerculesMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _HERCULES_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = HerculesTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `hercules mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``hercules mcp remove <name>`` and (indirectly) by
        ``hercules mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
