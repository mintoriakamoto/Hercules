...
it is automatically downloaded from GitHub releases to $HERCULES_HOME/bin/tirith.
...
def _get_hercules_home() -> str:
    """Return the Hercules home directory, respecting HERCULES_HOME env var."""
    return str(get_hercules_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_hercules_home(), ".tirith-install-failed")
...
def _hercules_bin_dir() -> str:
    """Return $HERCULES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_hercules_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HERCULES_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_hercules_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HERCULES_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HERCULES_HOME/bin/tirith
    ...
    hercules_bin = os.path.join(_hercules_bin_dir(), "tirith")
    if os.path.isfile(hercules_bin) and os.access(hercules_bin, os.X_OK):
        _resolved_path = hercules_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return hercules_bin
