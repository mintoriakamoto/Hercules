"""Central registry for all hercules-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``hercules tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``hercules tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("hercules_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``hercules_plugins.{name}``), not the exact module string.
                # A handler defined in ``hercules_plugins.pkg.handlers`` is
                # still owned by the ``hercules_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("hercules_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``hercules tools enable foo``)
