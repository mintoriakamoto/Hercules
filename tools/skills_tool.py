...
# Cache validation (mirrors hercules_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.hercules/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set HERCULES_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped HERCULES_HOME on every call.
    """
    configured = Path(SKILLS_DIR)
    if configured != _SKILLS_DIR_AT_IMPORT:
        return configured
    return get_hercules_home() / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from HERCULES_HOME/.env."""
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set HERCULES_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (HERCULES_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("HERCULES_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("HERCULES_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_hercules_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.hercules/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``HERCULES_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: Optional[str] = None) -> bool:
    ...
    2. ``HERCULES_PLATFORM`` environment variable
    3. ``HERCULES_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from hercules_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("HERCULES_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.hercules/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``hercules skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile HERCULES_HOME; the module-level
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_hercules_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from hercules_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: hercules plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.hercules/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `hercules skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.hercules.* first (agentskills.io convention), fall back to top-level
        hercules_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hercules_meta = metadata.get("hercules", {}) or {}

        tags = _parse_tags(hercules_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            hercules_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
