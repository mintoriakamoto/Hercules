#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Hercules Skills Hub.
...
Used by hercules_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _hercules_home() -> Path:
    return get_hercules_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _hercules_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hercules_meta = metadata.get("hercules", {})
            if isinstance(hercules_meta, dict):
                tags = hercules_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hercules_meta = metadata.get("hercules", {})
            if isinstance(hercules_meta, dict):
                raw_tags = hercules_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.hercules/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hercules_meta = meta_block.get("hercules", {})
                if isinstance(hercules_meta, dict):
                    tags = hercules_meta.get("tags", [])
...
def _hercules_index_cache_file() -> Path:
    return _index_cache_dir() / "hercules-index.json"


def _load_hercules_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for HERCULES_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    hercules_index_cache_file = _hercules_index_cache_file()
    if hercules_index_cache_file.exists():
        try:
            age = time.time() - hercules_index_cache_file.stat().st_mtime
            if age < HERCULES_INDEX_TTL:
                return json.loads(hercules_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Hercules index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Hercules index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Hercules index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        hercules_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        hercules_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class HerculesIndexSource(SkillSource):
    """Skill source backed by the centralized Hercules Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_hercules_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "hercules-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "hercules-index"),
            identifier=entry.get("identifier", ""),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "hercules-index"
