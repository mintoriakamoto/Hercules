...
def _is_hercules_provider_credential(name: str) -> bool:
    """True if ``name`` is a Hercules-managed provider credential (API key,
    token, or similar) per ``_HERCULES_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Hercules API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Hercules credential into the execute_code child.
    """
    try:
        from tools.environments.local import (
            _HERCULES_PROVIDER_ENV_BLOCKLIST,
            _is_hercules_internal_secret,
        )
        ...
        return True
    # Dynamically-generated Hercules-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_hercules_internal_secret(name):
        return True
    return name in _HERCULES_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Hercules-managed provider credentials (from
    ``_HERCULES_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Hercules-managed
    ...
    Non-Hercules third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Hercules-managed provider credentials must not be passed
                ...
                if _is_hercules_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Hercules "
                        "provider credential %r from config.yaml (blocked "
                        "by _HERCULES_PROVIDER_ENV_BLOCKLIST). Operator "
