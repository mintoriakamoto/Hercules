...
# os.environ["HERCULES_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export HERCULES_INTERACTIVE.
_hercules_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "hercules_interactive",
    default=None,
)


def set_hercules_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["HERCULES_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``HERCULES_INTERACTIVE`` env var for legacy callers.
    """
    return _hercules_interactive_ctx.set("1" if interactive else "")


def reset_hercules_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_hercules_interactive_context`."""
    _hercules_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``HERCULES_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hercules_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("HERCULES_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HERCULES_SESSION_KEY", default)
...
def _is_gateway_approval_context() -> bool:
    ...
    Legacy gateway integrations set HERCULES_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind HERCULES_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds HERCULES_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("HERCULES_CRON_SESSION"):
        return False
    if env_var_enabled("HERCULES_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $HERCULES_HOME, or by the resolved absolute
# active profile home path such as /home/hercules/.hercules/config.yaml. The
# resolved-absolute form is folded into the ~/.hercules/ patterns at detection
...
# go stale when HERCULES_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HERCULES_ENV_PATH = (
    r'(?:~\/\.hercules/|'
    r'(?:\$home|\$\{home\})/\.hercules/|'
    r'(?:\$hercules_home|\$\{hercules_home\})/)'
    r'\.env\b'
)
# ~/.hercules/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HERCULES_ENV_PATH; matches the HERCULES_HOME override form as
# well as ~/.hercules/.
_HERCULES_CONFIG_PATH = (
    r'(?:~\/\.hercules/|'
    r'(?:\$home|\$\{home\})/\.hercules/|'
    r'(?:\$hercules_home|\$\{hercules_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_HERCULES_ENV_PATH}|'
    rf'{_HERCULES_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    # `hercules` and `gateway` (e.g. `hercules -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhercules\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart hercules gateway (kills running agents)"),
    (r'\bhercules\s+update\b', "hercules update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `hercules gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart hercules-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart hercules-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hercules|gateway|cli\.py)\b', "kill hercules/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill hercules` but not
    # `kill -9 $(pgrep -f hercules)` because the substitution is opaque
    ...
    # the `hercules gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.hercules.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(hercules|ai\.hercules)\b', "stop/restart hercules launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Hercules file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.hercules/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Hercules-managed security file (~/.hercules/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERCULES_CONFIG_PATH}|{_HERCULES_ENV_PATH})', "in-place edit of Hercules config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERCULES_CONFIG_PATH}|{_HERCULES_ENV_PATH})', "in-place edit of Hercules config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERCULES_CONFIG_PATH}|{_HERCULES_ENV_PATH})', "in-place edit of Hercules config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.hercules/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / HERCULES_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Hercules home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hercules), so folding the user home
    # first would eat the prefix the Hercules-home fold needs.
    command = _rewrite_resolved_hercules_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / HERCULES_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.hercules``); the matched
...
def _rewrite_resolved_hercules_home(command: str) -> str:
    """Rewrite the resolved absolute Hercules home prefix to ``~/.hercules/``.

    Resolves the active ``HERCULES_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.hercules/`` so the static ``_HERCULES_CONFIG_PATH`` / ``_HERCULES_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/hercules/.hercules/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$HERCULES_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from hercules_constants import get_hercules_home
        home = get_hercules_home().expanduser()
        candidates = [
            str(home),
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Hercules ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"hercules-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["HERCULES_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "HERCULES_SPINNER_PAUSE" in os.environ:
            del os.environ["HERCULES_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Hercules approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``HERCULES_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            HERCULES_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("HERCULES_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set HERCULES_INTERACTIVE or "
                "HERCULES_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set HERCULES_INTERACTIVE or "
            "HERCULES_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("HERCULES_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``HERCULES_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("HERCULES_EXEC_ASK")
