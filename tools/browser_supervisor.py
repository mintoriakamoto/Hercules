...
One ``CDPSupervisor`` runs per Hercules ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Hercules logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "hercules-dialog-bridge.invalid"
...
(() => {
  if (window.__herculesDialogBridgeInstalled) return;
  window.__herculesDialogBridgeInstalled = true;
  const ENDPOINT = "http://hercules-dialog-bridge.invalid/";
