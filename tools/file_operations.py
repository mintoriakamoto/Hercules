...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__HERCULES_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__HERCULES_FENCE_" in line or "\x1b]" in line
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        for i, line in enumerate(lines, start=start_line):
            ...
            numbered.append(f"{i}|{display_line}")
        return '\n'.join(numbered)
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.hercules-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".hercules-tmp.XXXXXX")
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.hercules-tmp``
