...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``hercules-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. hercules-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.hercules/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_hercules_dir("cache/delegation", "delegation_cache")
...
# Marker separating a task's real outcome from the appended adversarial
# verdict in ``entry['summary']``. Shared by the verify wave and the repair
# loop so they stay in lock-step.
_VERIFY_TRAILER = "[adversarial verification]"

# Repair loop bounds. ``verify_repair_rounds`` defaults to 0 (disabled — the
# verify wave stays purely advisory, exactly as before), and is capped so a
# misconfiguration can't spin an unbounded, token-burning correction tree.
_REPAIR_MAX_ITERATIONS = 30
_VERIFY_REPAIR_MAX_ROUNDS = 3

# Total-task ceiling for a dependency-DAG batch. Concurrency is still bounded
# by max_concurrent_children; this only stops a single pipeline from spawning
# an unreasonable number of sequential stages.
_DAG_MAX_TASKS = 24


def _verdict_is_refuted(verdict_line: str) -> bool:
    """True when an adversarial verdict's first line is a refutation."""
    return (verdict_line or "").strip().lower().startswith("verdict: refuted")


def _get_verify_repair_rounds() -> int:
    """Read delegation.verify_repair_rounds (default 0 = disabled).

    When > 0, a task the verifier refuted is handed to a repair child seeded
    with the refutation evidence, then re-verified — looping up to this many
    rounds or until the verdict flips to verified. Clamped to
    ``_VERIFY_REPAIR_MAX_ROUNDS`` so it can never run away.
    """
    cfg = _load_config()
    val = cfg.get("verify_repair_rounds")
    if val is None:
        return 0
    try:
        ival = int(val)
        ...
        logger.warning(
            "delegation.verify_repair_rounds=%r is not a valid integer; "
            "disabling repair",
            val,
        )
        return 0
    return max(0, min(ival, _VERIFY_REPAIR_MAX_ROUNDS))


def _build_repair_goal(
    original_goal: str, claimed_summary: str, verifier_detail: str
) -> str:
    """Prompt a repair child to fix exactly what the verifier refuted."""
    return (
        "A previous agent attempted this task but an adversarial verifier "
        "REFUTED its claimed outcome. Your job is to make the task actually "
        "correct.\n\n"
        f"--- ORIGINAL TASK ---\n{original_goal}\n--- END TASK ---\n\n"
        f"--- WHAT THE PREVIOUS AGENT CLAIMED ---\n{claimed_summary}\n"
        "--- END CLAIM ---\n\n"
        f"--- WHY THE VERIFIER REFUTED IT ---\n{verifier_detail}\n"
        "--- END REFUTATION ---\n\n"
        "Fix the specific problem the verifier found. Do the real work with "
        "your tools — create/repair the files, run the commands, make the "
        "assertions true. Do NOT merely re-describe the plan. When done, your "
        "summary must state concretely what you changed and how you confirmed "
        "it now holds (the exact command output or file contents you checked)."
    )
    ...
    with DaemonThreadPoolExecutor(max_workers=max_children) as executor:
        ...
        while pending:
            ...
            for future in done:
                ...
                # Stash the pre-verdict outcome so the repair loop can reseed a
...
def _run_children_pairs(
    pairs: List[tuple], max_children: int, parent_agent, goal_label: str
) -> Dict[int, Dict[str, Any]]:
    """Run (entry, child) pairs concurrently; return {id(entry): result}.

    Shared by the repair wave and its re-verification wave. A crashed child
...
def _run_repair_rounds(
    ...
    Runs AFTER ``_run_verification_wave`` and only touches entries whose
    verdict is ``refuted``. Each round, for every still-refuted entry, a repair
    child (with the worker toolset) is seeded with the original goal, the
    claimed outcome, and the verifier's refutation, then a fresh verifier
    re-checks the repaired outcome. Entry summary/verification are rewritten to
    the corrected state; a ``repairs`` trail records each round. All helper
    spend is folded into the entry's cost rollup so the parent's total stays
    ...
    for round_num in range(1, max_rounds + 1):
        refuted = [
            ...
            and _verdict_is_refuted(e["verification"].get("verdict", ""))
        ]
        if not refuted:
            return
        if getattr(parent_agent, "_interrupt_requested", False) is True:
        ...
        # --- Repair wave: one fixer child per refuted entry ---
        _saved = list(_model_tools._last_resolved_tool_names)
        repair_pairs = []
        try:
            for e in refuted:
                ...
                base_summary = e.get("_verify_base_summary", e.get("summary", "")) or ""
                detail = (e.get("verification") or {}).get("detail", "") or ""
                child = _build_child_agent(
                    task_index=idx,
                    goal=_build_repair_goal(original_goal, base_summary, detail),
                    context=None,
                    toolsets=None,
                    model=creds["model"],
                    max_iterations=_REPAIR_MAX_ITERATIONS,
                    task_count=len(refuted),
                    parent_agent=parent_agent,
                    override_provider=creds["provider"],
                    override_base_url=creds["base_url"],
                    override_api_key=creds["api_key"],
                ...
                child._delegate_saved_tool_names = parent_tool_names
                repair_pairs.append((e, child))
        finally:
            _model_tools._last_resolved_tool_names = _saved

        repair_out = _run_children_pairs(
            repair_pairs, max_children, parent_agent, "repair"
        )

        # Keep only entries that produced a real repaired outcome to re-verify.
        made_progress = False
        reverify_targets = []
        for e in refuted:
            r = repair_out.get(id(e))
            if r is None:
                continue
            _fold_child_cost(e, r)
            repaired_summary = (r.get("summary") or "").strip()
            if r.get("status") == "completed" and repaired_summary:
                e["_repair_candidate"] = repaired_summary
        ...
        # --- Re-verification wave over the repaired outcomes ---
        _saved = list(_model_tools._last_resolved_tool_names)
        verify_pairs = []
        try:
            for e in reverify_targets:
                idx = e["task_index"]
                original_goal = (
                    task_list[idx]["goal"] if idx < len(task_list) else "(unknown)"
                )
                ...
                child._delegate_saved_tool_names = parent_tool_names
                verify_pairs.append((e, child))
        finally:
            _model_tools._last_resolved_tool_names = _saved

        verify_out = _run_children_pairs(
            verify_pairs, max_children, parent_agent, "verify"
        )

        for e in reverify_targets:
            v = verify_out.get(id(e))
            repaired_summary = e.pop("_repair_candidate", "")
            if v is None:
                continue
            _fold_child_cost(e, v)
            v_summary = (v.get("summary") or "").strip()
            if v.get("status") == "completed" and v_summary:
                new_verdict = v_summary
            else:
                new_verdict = (
                    "VERDICT: unverifiable — re-verifier did not complete "
                    f"({v.get('error') or v.get('status')})"
                )
            # Adopt the repaired outcome as the entry's real result.
            e["_verify_base_summary"] = repaired_summary
            e["summary"] = f"{repaired_summary}\n\n{_VERIFY_TRAILER}\n{new_verdict}"
            e["verification"] = {
...
def _parse_dependencies(
    task_list: List[Dict[str, Any]],
) -> tuple[Optional[Dict[int, set]], Optional[str]]:
    """Parse per-task ``depends_on`` (0-based task indices) into ``{i: {j,...}}``.

    Returns ``(deps, None)`` on success or ``(None, error)`` on a bad spec.
    Validates that every index is in range, no task depends on itself, and the
    dependency graph is acyclic (Kahn's algorithm) — so the scheduler can never
    deadlock. Tasks with no ``depends_on`` map to an empty set (the fast path).
    """
    n = len(task_list)
    ...
    for i, t in enumerate(task_list):
        ...
        if not isinstance(raw, (list, tuple)):
            return None, f"Task {i} 'depends_on' must be a list of task indices."
        for d in raw:
            try:
                di = int(d)
            except (TypeError, ValueError):
                return None, f"Task {i} 'depends_on' has a non-integer entry: {d!r}."
            if di < 0 or di >= n:
                return None, (
                    f"Task {i} depends on out-of-range task {di} "
                    f"(there are {n} task(s), valid indices 0..{n - 1})."
                )
            if di == i:
                return None, f"Task {i} cannot depend on itself."
            deps[i].add(di)
    # Acyclicity check (Kahn): peel off tasks with no remaining prerequisites.
    indeg = {i: len(deps[i]) for i in range(n)}
    dependents: Dict[int, list] = {i: [] for i in range(n)}
    for i in range(n):
        for j in deps[i]:
    ...
    while q:
        ...
        for i in dependents[j]:
            ...
            if indeg[i] == 0:
                q.append(i)
    if seen != n:
        return None, "Task 'depends_on' forms a cycle; dependencies must be acyclic."
    return deps, None
...
def _run_dag_batch(
    ...
    Runs each task only once all of its prerequisites have completed, and
    injects each prerequisite's summary into the dependent's goal so the
    downstream agent sees upstream output. Concurrency is bounded to
    ``max_children``; a task is submitted to the pool only when it is runnable,
    so a worker is never parked waiting on an unscheduled producer — meaning
    the scheduler is deadlock-free given the acyclicity guaranteed by
    ``_parse_dependencies``. A failed/interrupted prerequisite still counts as
    ...
    def _compose_goal(i: int) -> str:
        t, _ = child_by_index[i]
        base = t["goal"]
        d = deps.get(i) or set()
        if not d:
            return base
        ...
        return (
            f"{base}\n\n[Inputs from completed prerequisite tasks — build on these]\n"
            + "\n\n".join(blocks)
        )

    def _emit_line(entry: Dict[str, Any]) -> None:
        completed_count[0] += 1
        idx = entry["task_index"]
        label = task_labels[idx] if idx < len(task_labels) else f"Task {idx}"
        dur = entry.get("duration_seconds", 0)
        icon = "✓" if entry.get("status") == "completed" else "✗"
        line = f"{icon} [{idx + 1}/{n_tasks}] {label}  ({dur}s)"
        if spinner_ref:
            try:
    ...
    with DaemonThreadPoolExecutor(max_workers=max(1, max_children)) as executor:
        while len(done) < n_tasks:
            ...
            if not in_flight:
                break
            finished, _ = _cf_wait(
                set(in_flight.keys()), timeout=0.5, return_when=FIRST_COMPLETED
            )
            for fut in finished:
                i = in_flight.pop(fut)
                try:
...
def delegate_task(
    ...
    if tasks and isinstance(tasks, list):
        # Size guard is applied AFTER dependency parsing below: a flat batch is
        # still capped at max_concurrent_children (all run at once), but a
        # dependency DAG sequences tasks, so its concurrency is bounded by the
    ...
    # Optional dependency DAG: a task may declare depends_on=[indices] to run
    # only after those tasks complete, receiving their output as input. Absent
    # any depends_on this is a no-op and the flat parallel path runs unchanged.
    deps, _dep_err = _parse_dependencies(task_list)
    if _dep_err:
        return tool_error(_dep_err)
    has_deps = any(deps.values())

    # Size guard. Flat batches run fully in parallel, so they stay capped at
    # max_concurrent_children (unchanged). A dependency DAG runs at most
    # max_concurrent_children at once regardless of total count, so only its
    # total is bounded — by a looser ceiling — to keep a pipeline from
    # ballooning cost unnoticed.
    if len(task_list) > max_children:
        if has_deps:
            if len(task_list) > _DAG_MAX_TASKS:
                return tool_error(
                    f"Too many tasks: {len(task_list)} provided, but the "
                    f"dependency-DAG ceiling is {_DAG_MAX_TASKS}. Split the "
                    f"pipeline into multiple delegate_task calls."
                )
        else:
            return tool_error(
    ...
    def _execute_and_aggregate() -> dict:
        ...
        if n_tasks == 1:
            ...
            # Dependency-DAG batch: run tasks in prerequisite order, feeding
            # each producer's output into its dependents. Deadlock-free given
            # the acyclicity check in _parse_dependencies.
            results.extend(
                _run_dag_batch(
                    children,
                    deps,
                    ...
                    max_children,
                    parent_agent,
                    getattr(parent_agent, "_delegate_spinner", None),
                )
            )
        ...
        if verify:
            ...
            # Self-correcting loop (opt-in via delegation.verify_repair_rounds):
            # hand every refuted task to a repair child, then re-verify, until
            # the verdict flips to verified or the round budget is spent. A
            # no-op at the default (0 rounds), so the plain verify wave is
            # unchanged unless an operator turns repair on.
            try:
                _repair_rounds = _get_verify_repair_rounds()
                if _repair_rounds > 0:
                    _run_repair_rounds(
                        results,
                        task_list,
        ...
        for entry in results:
            ...
            # Internal repair-loop scratch — never surface to the parent.
            entry.pop("_verify_base_summary", None)
            entry.pop("_repair_candidate", None)
            try:
                if child_cost:
    ...
    if background:
        ...
        try:
            ...
            _source = get_session_env("HERCULES_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("HERCULES_UI_SESSION_ID", "")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'hercules auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Hercules config.

    Prefer the shared persistent loader because it follows the active
    HERCULES_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``HERCULES_IGNORE_USER_CONFIG=1`` (``hercules chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("HERCULES_IGNORE_USER_CONFIG") == "1"
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "depends_on": {
                            ...
                            "items": {"type": "integer"},
                            "description": (
                                "0-based indices of tasks in THIS array that must "
                                "finish before this task runs. This task starts "
                                "only once all of them complete, and receives their "
                                "summaries as input — use it to chain producers to "
                                "consumers (e.g. task 2 analyzes what tasks 0 and 1 "
                                "produced). Tasks with no depends_on run in parallel "
                                "as before. Must be acyclic."
                            ),
                        },
                    },
                    "required": ["goal"],
