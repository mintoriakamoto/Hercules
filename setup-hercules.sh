...
# 5. Symlinks the 'hercules' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/hercules-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink hercules into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up hercules command..."

HERCULES_BIN="$SCRIPT_DIR/venv/bin/hercules"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERCULES_BIN" "$COMMAND_LINK_DIR/hercules"
echo -e "${GREEN}✓${NC} Symlinked hercules → $COMMAND_LINK_DISPLAY_DIR/hercules"
...
# ============================================================================
# Seed bundled skills into ~/.hercules/skills/
# ============================================================================

HERCULES_SKILLS_DIR="${HERCULES_HOME:-$HOME/.hercules}/skills"
mkdir -p "$HERCULES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.hercules/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$HERCULES_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "⚕ Hercules is ready. All tools available, no web/CLI interface included."
echo ""

if is_termux; then
    echo "Installation Location:"
    echo "  Virtual environment: $SCRIPT_DIR/venv/"
    echo "  Persistent memory:   ~/.hercules/memory/"
    echo "  Skills:              ~/.hercules/skills/"
    echo ""
else
    echo "Installation Location:"
    echo "  Virtual environment: $SCRIPT_DIR/venv/"
...
echo "  ✓ Code execution (Python/Node/Shell)"
echo "  ✓ Web search (Exa, Parallel, Firecrawl)"
echo "  ✓ Image generation (FAL.ai)"
echo "  ✓ Browser automation (Browserbase)"
echo "  ✓ All LLM providers"
echo "  ✓ Local voice/vision (optional)"
echo "  ✓ Persistent memory (local)"
echo ""

echo "What's NOT Included (By Design):"
echo "  ✗ No local web interface or CLI"
echo "  ✗ No messaging integrations (Slack, Telegram, etc.)"
echo "  ✗ No gateway/web endpoints"
echo "  ✗ No telemetry or analytics"
echo ""
echo "Use as library or build custom interfaces:"
echo "  - Import in Python: from hercules import Agent"
echo "  - Build custom CLI with Click/Typer"
echo "  - Wrap with REST API if needed"
echo "  - Create skills for specific workflows"
echo ""

echo "For API configuration:"
echo "  - Edit .env with your API keys (optional for offline mode)"
echo "  - Or use: hercules config set <key> <value>"
echo ""
