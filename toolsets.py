...
_HERCULES_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane, and close an agent's
    # read-only terminal tab (both gated on HERCULES_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (HERCULES_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `hercules tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``hercules tools`` → Video Generation."
        ),
    ...
    "blackboard": {
        "description": "Shared blackboard for agent-to-agent communication (parent + subagents post/read a merged board)",
        "tools": ["blackboard"],
        "includes": []
    },
    ...
    # Coding posture (base Hercules — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in hercules_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Hercules toolsets (CLI + messaging platforms)
    #
    ...
    # the `hercules send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "hercules-cli": {
        ...
        "tools": _HERCULES_CORE_TOOLS,
        "includes": []
    },

    "hercules-cron": {
        # Mirrors hercules-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `hercules tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as hercules-cli; gated by `hercules tools`",
        "tools": _HERCULES_CORE_TOOLS,
        "includes": []
    },
    ...
    "hercules-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _HERCULES_CORE_TOOLS,
        "includes": []
    },

    "hercules-homeassistant": {
    ...
    "hercules-email": {
        "description": "Email bot toolset - interact with Hercules via email (IMAP/SMTP)",
        "tools": _HERCULES_CORE_TOOLS,
        "includes": []
    },
    ...
    "hercules-sms": {
        "description": "SMS bot toolset - interact with Hercules via SMS (Twilio)",
        "tools": _HERCULES_CORE_TOOLS,
        "includes": []
    },
    ...
    "hercules-webhook": {
        ...
        "tools": _HERCULES_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "hercules-gateway": {
        ...
        "tools": [],
        "includes": ["hercules-telegram", "hercules-discord", "hercules-whatsapp", "hercules-slack", "hercules-signal", "hercules-bluebubbles", "hercules-homeassistant", "hercules-email", "hercules-sms", "hercules-mattermost", "hercules-matrix", "hercules-dingtalk", "hercules-feishu", "hercules-wecom", "hercules-wecom-callback", "hercules-weixin", "hercules-qqbot", "hercules-webhook", "hercules-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``hercules-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_HERCULES_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``hercules-gateway``
...
def resolve_toolset(name: str, visited: Optional[Set[str]] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (hercules-<name>).
        # Gives them _HERCULES_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("hercules-"):
            platform_name = name[len("hercules-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_HERCULES_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
