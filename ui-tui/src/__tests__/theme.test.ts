...
// HERCULES_TUI_THEME=light (or HERCULES_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#F7B23B')
...
describe('LIGHT_THEME', () => {
  it('avoids bright-yellow accents unreadable on white backgrounds (#11300)', async () => {
    ...
    expect(LIGHT_THEME.color.primary).not.toBe('#F7B23B')
    expect(LIGHT_THEME.color.accent).not.toBe('#E8712E')
    expect(LIGHT_THEME.color.muted).not.toBe('#8E2B3F')
    expect(LIGHT_THEME.color.statusWarn).not.toBe('#F7B23B')
  })
...
describe('detectLightMode', () => {
  ...
  it('honors HERCULES_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ HERCULES_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ HERCULES_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ HERCULES_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ HERCULES_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ HERCULES_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets HERCULES_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', HERCULES_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors HERCULES_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ HERCULES_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ HERCULES_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', HERCULES_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', HERCULES_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses HERCULES_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ HERCULES_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // HERCULES_TUI_THEME=light would flip the base palette and make these
  ...
  it('normalizes non-banner foregrounds on light Apple Terminal', async () => {
    ...
    expect(theme.color.primary).toBe('#F7B23B')
    expect(theme.color.accent).toBe('#E8712E')
    expect(theme.color.border).toBe('#C73E3A')
