"""Tests for ``hercules update`` / ``--check`` inside the Docker container.
...
fix, ``hercules update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Hercules, not an update to the running container, so the message
...
from hercules_cli.main import _cmd_update_check, cmd_update


# ---------- cmd_update (apply path) ----------


@patch("hercules_cli.config.is_managed", return_value=False)
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``hercules update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``hercules update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "HERCULES_HOME" in msg or "/opt/data" in msg, (
