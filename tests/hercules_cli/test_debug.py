...
def hercules_home(tmp_path, monkeypatch):
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO hercules_cli.web_server: dashboard request\n"
    )
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, hercules_home):
    ...
    def test_empty_primary_reports_file_empty(self, hercules_home):
    ...
    def test_truncates_large_file(self, hercules_home):
        """Files larger than max_bytes get tail-truncated."""
        from hercules_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (hercules_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, hercules_home):
        ...
        (hercules_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, hercules_home):
    ...
    def test_falls_back_to_rotated_file(self, hercules_home):
    ...
    def test_prefers_primary_over_rotated(self, hercules_home):
    ...
    def test_falls_back_when_primary_empty(self, hercules_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of HERCULES_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def hercules_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated HERCULES_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".hercules"
        home.mkdir()
        monkeypatch.setenv("HERCULES_HOME", str(home))
        ...
        monkeypatch.delenv("HERCULES_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, hercules_home_with_secret):
        from hercules_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, hercules_home_with_secret):
        from hercules_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, hercules_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when HERCULES_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("HERCULES_REDACT_SECRETS", "false")

        from hercules_cli.debug import _capture_log_snapshot

        assert os.environ.get("HERCULES_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, hercules_home_with_secret
    ):
        from hercules_cli.debug import _capture_log_snapshot

        log_path = hercules_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, hercules_home_with_secret):
        from hercules_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from hercules_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hercules_home):
        from hercules_cli.debug import collect_debug_report
        ...
        assert "--- hercules dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, hercules_home):
        from hercules_cli.debug import collect_debug_report
    ...
    def test_report_includes_errors_log(self, hercules_home):
        from hercules_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, hercules_home):
        from hercules_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, hercules_home):
        from hercules_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, hercules_home):
        from hercules_cli.debug import collect_debug_report
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "hercules debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, hercules_home):
        from hercules_cli.debug import run_debug
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from hercules_cli.debug import delete_paste
...
class TestScheduleAutoDelete:
    ...
    ``~/.hercules/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``hercules debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, hercules_home):
        ...
        import ast
        import inspect
        from hercules_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, hercules_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from hercules_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, hercules_home):
    ...
    def test_merges_with_existing_pending(self, hercules_home):
        """Subsequent calls merge into existing pending.json."""
        from hercules_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, hercules_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, hercules_home):
        from hercules_cli.debug import _sweep_expired_pastes

        deleted, remaining = _sweep_expired_pastes()
        assert deleted == 0
        assert remaining == 0

    def test_sweep_deletes_expired_entries(self, hercules_home):
        ...
        with patch("hercules_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, hercules_home):
        from hercules_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("hercules_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, hercules_home):
        ...
        with patch(
            "hercules_cli.debug.delete_paste",
    ...
    def test_sweep_drops_entries_past_grace_window(self, hercules_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, hercules_home):
        from hercules_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("hercules_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, hercules_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: hercules debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from hercules_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("hercules_cli.debug.delete_paste", return_value=True):
...
class TestBuildDebugShare:
    ...
    Backs both ``hercules debug share`` (CLI) and ``POST /api/ops/debug-share``
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, hercules_home):
        from hercules_cli.debug import collect_share_bundle
    ...
    def test_no_redact_omits_banner(self, hercules_home):
        from hercules_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, hercules_home):
        from hercules_cli.debug import collect_share_bundle
...
class TestDebugSlashCommand:
    ...
    ``HerculesCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("hercules_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`hercules debug share` requires explicit consent before uploading.
