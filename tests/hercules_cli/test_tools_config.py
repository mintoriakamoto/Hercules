"""Tests for hercules_cli.tools_config platform tool persistence."""
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="hercules_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "hercules" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["hercules-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["hercules-cli", "bogus"]}}
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `hercules tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "hercules_cli.tools_config._xai_credentials_present", lambda: True
    )
...
# via the platform's native composite. The composite ``hercules-discord``
...
def test_discord_composite_only_enables_discord_toolsets():
    """Layer 1: ``platform_toolsets.discord: [hercules-discord]`` is an explicit
...
def test_discord_composite_plus_configurable_enables_discord_toolsets():
    ...
    config = {"platform_toolsets": {"discord": ["hercules-discord", "spotify"]}}
    enabled = _get_platform_tools(config, "discord")
...
def test_discord_composite_plus_partial_explicit_enables_sibling():
    """Layer 3: ``[hercules-discord, discord]`` lists discord explicitly but
    discord_admin arrives only via the composite. Both must survive."""
    config = {"platform_toolsets": {"discord": ["hercules-discord", "discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["hercules-telegram", "discord"]}}
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "hercules_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `hercules tools`,
    ...
    # User explicitly opted into spotify but not x_search via `hercules tools`.
    config = {"platform_toolsets": {"cli": ["hercules-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[hercules-cli, spotify]`` (composite + configurable) must keep the full
    ``hercules-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``hercules-cli`` on the floor,
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["hercules-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``hercules-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["hercules-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _HERCULES_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `hercules tools`
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (hercules-cli, hercules-telegram, etc.) must NOT
    ...
    ``hercules tools``, but hercules-cli stays in the config and re-enables
    ...
    # hercules-cli must NOT survive — it's a platform default, not an MCP server
    assert "hercules-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_hercules_telegram():
    """Same bug for Telegram — hercules-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
    ...
    assert "hercules-telegram" not in saved
    assert "web" in saved
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("hercules_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from hercules_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "hercules_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("hercules_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "hercules_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("hercules_cli.tools_config.save_config", lambda config: None)
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from hercules_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """hercules-gateway includes list should cover all messaging platforms."""
        from hercules_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["hercules-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from hercules_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from hercules_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/hercules-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `hercules tools` must reach the
...
class TestImagegenModelPicker:
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("hercules_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`hercules tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'hercules-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `hercules tools`,
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `hercules tools`, the toolset appears."""
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `hercules tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from hercules_cli.tools_config import _get_effective_configurable_toolsets
...
# ---------------------------------------------------------------------------
# Non-interactive provider selection
# ---------------------------------------------------------------------------
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from hercules_cli.tools_config import apply_provider_selection
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    as ``hercules model`` and writes the selection to the auxiliary config keys
    the resolver reads.
    """
    monkeypatch.setenv("HERCULES_HOME", str(tmp_path))
    import hercules_cli.tools_config as tc
