"""Tests for hercules_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_hercules_home):
    """Drop the example-dashboard fixture into the per-test HERCULES_HOME
    ...
    isolated ``HERCULES_HOME``.

    The user-plugin source is preferred over a transient
    ``HERCULES_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, hercules-achievements, model providers
    ...
    #      and start serving requests against a torn-down HERCULES_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Hercules vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Hercules env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints HERCULES_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("HERCULES_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("HERCULES_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hercules_home):
        """Create a TestClient and isolate the state DB under the test HERCULES_HOME."""
        try:
    ...
    def test_get_status(self):
        ...
        assert "hercules_home" in data
        assert "active_sessions" in data
        assert data["can_update_hercules"] is True
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(hercules_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("hercules_state.SessionDB", _FakeDB)
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_hercules"] is False
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in hercules-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "hercules"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        ...
        config_path = get_hercules_home() / "hindsight" / "config.json"
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_hercules_home() / "images" / "nope.png"
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_hercules_home() / "images"
    ...
    def test_update_hercules_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        assert "docker pull nousresearch/hercules-agent:latest" in data["message"]
    ...
    def test_update_hercules_spawns_on_non_docker_install(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_hercules_action", fake_spawn)
        web_server._ACTION_PROCS.pop("hercules-update", None)
        web_server._ACTION_RESULTS.pop("hercules-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "hercules-update"}
        assert calls == [(["update"], "hercules-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("hercules-update", None)
        web_server._ACTION_RESULTS.pop("hercules-update", None)
        web_server._ACTION_PROCS["hercules-update"] = proc

        resp = self.client.get("/api/actions/hercules-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "hercules-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["hercules-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["hercules-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/hercules-update/status?lines=3")
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from hercules_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `hercules model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from hercules_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Hercules provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `hercules import` doesn't auto-abort at the overwrite prompt."""
        import hercules_cli.web_server as ws
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_hercules_home() / "backups"
        assert archive.name.startswith("hercules-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_hercules_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import hercules_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("HERCULES_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "hercules-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_hercules_action", fail_spawn)
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from hercules_cli.config import save_env_value
        from hercules_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/messaging/weixin.md"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from hercules_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/messaging/teams.md"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/messaging/google_chat.md"
        )
    ...
    def test_update_messaging_platform_saves_slack_allowed_users(self):
        from hercules_cli.config import load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Hercules Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("HerculesDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "hercules_pair123_bot",
                "deep_link": "https://t.me/newbot/HerculesSetupBot/hercules_pair123_bot",
                "qr_payload": "https://t.me/newbot/HerculesSetupBot/hercules_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "hercules_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/HerculesSetupBot/hercules_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/HerculesSetupBot/hercules_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "hercules_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_hercules_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "hercules_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/HerculesSetupBot/hercules_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/HerculesSetupBot/hercules_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "hercules_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_hercules_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``hercules gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "hercules_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/HerculesSetupBot/hercules_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/HerculesSetupBot/hercules_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "hercules_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "hercules_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/HerculesSetupBot/hercules_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/HerculesSetupBot/hercules_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "hercules_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/HerculesSetupBot/hercules_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/HerculesSetupBot/hercules_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/hercules_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`hercules serve` (HERCULES_SERVE_HEADLESS) must NOT serve the SPA even
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from hercules_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from hercules_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``hercules model`` custom flow. Regression for the desktop loop where a
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from hercules_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_hercules_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("hercules://")
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        named = hercules_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_hercules_for_default_profile(self):
        from hercules_constants import get_hercules_home
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_hercules_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_hercules_home() / "profiles" / "builder"
        token = set_hercules_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import hercules_cli.skills_config as skills_config
        import hercules_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import hercules_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from hercules_cli.web_server import _normalize_config_for_web
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.hercules/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``hercules_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in hercules_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hercules_home):
        ...
        # Pin the SessionDB to the isolated HERCULES_HOME so each test
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hercules_home, _install_example_plugin):
        ...
        installs it into the per-test ``HERCULES_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into HERCULES_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, hercules-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (hercules-achievements is loaded by default).
        resp = self.client.get("/api/plugins/hercules-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``hercules --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["HERCULES_TUI_DASHBOARD"] == "1"
        assert env["HERCULES_TUI_INLINE"] == "1"
        assert env["HERCULES_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_hercules_home
    ):
        import hercules_cli.main as main_mod
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/hercules-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        monkeypatch.setattr(
            ...
            lambda resume=None, sidecar_url=None, profile=None: (
                ["/bin/sh", "-c", "printf hercules-ws-ok"],
                None,
                None,
            ),
        )
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"hercules-ws-ok" in buf:
                    break
            assert b"hercules-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from hercules_cli.pty_bridge import PtyUnavailableError
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        HERCULES_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from hercules_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import hercules_cli.main as cli_main
    import hercules_cli.web_server as ws
    ...
    gateway_url = env.get("HERCULES_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hercules_home, _install_example_plugin):
        ...
        the per-test ``HERCULES_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_hercules_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under HERCULES_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_hercules_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("HERCULES_DESKTOP", raising=False)
