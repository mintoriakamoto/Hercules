"""Tests for the update check mechanism in hercules_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "hercules-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    with patch("hercules_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when HERCULES_HOME has no git repo."""
    import hercules_cli.banner as banner
    ...
    # Point HERCULES_HOME at a temp dir with no hercules-agent/.git
    monkeypatch.setenv("HERCULES_HOME", str(tmp_path))
    with patch("hercules_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    HERCULES_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `hercules update` correctly refusing to run there. The guard
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from hercules_cli.main import _invalidate_update_cache

    # Build a fake ~/.hercules with default + two named profiles
    default_home = tmp_path / ".hercules"
    default_home.mkdir()
