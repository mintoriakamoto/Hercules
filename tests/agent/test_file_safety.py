...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hercules_env(self):
        """Hercules' own .env inside HERCULES_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: hercules internal .env is in ~/.hercules/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.hercules/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.hercules/.env"))
        assert error is not None
