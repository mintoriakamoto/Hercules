...
from hercules_cli.memory_setup import _CANCELLED
...
class TestConfig:
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "hercules"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["hercules"]["bankId"] == "env-bank"
        assert cfg["banks"]["hercules"]["budget"] == "high"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.post_setup(str(hercules_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (hercules_home / ".env").exists()
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        monkeypatch.setattr("hercules_cli.config.save_config", lambda cfg: None)

        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(hercules_home))
        provider.post_setup(str(hercules_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        hercules_env = user_home / ".hindsight" / "profiles" / "hercules.env"
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(hercules_home))
        ...
        saved = json.loads((hercules_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["metadata"]["source"] == "hercules"
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("hercules") == "hercules"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "hercules-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="hercules",
        )
        assert p._bank_id == "hercules-coder"
        assert p._bank_id_template == "hercules-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p.initialize(session_id="s1", hercules_home=str(tmp_path), platform="cli")
        assert p._bank_id == "hercules"
