...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_hercules_home_override(self, tmp_path):
        ...
        (real_home / ".hercules").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "HERCULES_HOME": str(hercules_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``hercules tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("hercules_cli.config.load_config", return_value={"web": None}):
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from hercules_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.hercules/.env were invisible to the provider."""
    ...
    def test_direct_config_reads_via_get_env_value(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch(
            "hercules_cli.config.get_env_value",
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.hercules/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_get_provider_env_falls_back_to_os_environ(self, monkeypatch):
        ...
        with patch("hercules_cli.config.get_env_value", return_value=None):
