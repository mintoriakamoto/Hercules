"""Shared fixtures for the hercules-agent test suite.
...
2. **Isolated HERCULES_HOME.** HERCULES_HOME points to a per-test tempdir so
   code reading ``~/.hercules/*`` via ``get_hercules_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hercules"`` instead of the canonical
   ``get_hercules_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No HERCULES_SESSION_* inheritance** — the agent's current gateway
...
# HERCULES_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_HERCULES_BEHAVIORAL_VARS = frozenset({
    "HERCULES_YOLO_MODE",
    ...
    "HERCULES_SESSION_CHAT_ID",
    "HERCULES_SESSION_CHAT_NAME",
    "HERCULES_SESSION_THREAD_ID",
    "HERCULES_SESSION_SOURCE",
    "HERCULES_SESSION_KEY",
    "HERCULES_GATEWAY_SESSION",
    "HERCULES_CRON_SESSION",
    "_HERCULES_GATEWAY",
    "HERCULES_PLATFORM",
    "HERCULES_MODEL",
    "HERCULES_INFERENCE_MODEL",
    "HERCULES_INFERENCE_PROVIDER",
    "HERCULES_TUI_PROVIDER",
    "HERCULES_MANAGED",
    "HERCULES_MANAGED_DIR",
    "HERCULES_DEV",
    "HERCULES_CONTAINER",
    "HERCULES_EPHEMERAL_SYSTEM_PROMPT",
    "HERCULES_TIMEZONE",
    "HERCULES_REDACT_SECRETS",
    "HERCULES_BACKGROUND_NOTIFICATIONS",
    "HERCULES_EXEC_ASK",
    "HERCULES_HOME_MODE",
    "HERCULES_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.hercules/kanban.db instead of the per-test HERCULES_HOME.
    "HERCULES_KANBAN_DB",
    "HERCULES_KANBAN_BOARD",
    "HERCULES_KANBAN_HOME",
    "HERCULES_KANBAN_WORKSPACES_ROOT",
    "HERCULES_KANBAN_LOGS_ROOT",
    "HERCULES_KANBAN_TASK",
    "HERCULES_KANBAN_WORKSPACE",
    "HERCULES_KANBAN_RUN_ID",
    "HERCULES_KANBAN_CLAIM_LOCK",
    "HERCULES_KANBAN_DISPATCH_IN_GATEWAY",
    "HERCULES_TENANT",
    ...
    "HERCULES_DASHBOARD_OAUTH_CLIENT_ID",
    "HERCULES_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and HERCULES_HOME to per-test tempdirs so code that
    reads ``~/.hercules/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral HERCULES_* vars that could change test semantics.
    for name in _HERCULES_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect HERCULES_HOME to a per-test tempdir. Code that reads
    #    ``~/.hercules/*`` via ``get_hercules_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.hercules/*`` via
    ...
    #    ~/.hercules/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import hercules_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal hercules config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``hercules-gateway`` process
...
# correlated with a ``tests/hercules_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> hercules-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart hercules-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting hercules/python patterns are also blocked.
    """
    ...
    def _matches_hercules_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _HERCULES_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_hercules_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch hercules-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m hercules_cli.main".
                if (
                    "hercules" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live hercules-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting hercules/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `hercules update` (or the
        # equivalent `python -m hercules_cli.main update`).  These commands
        ...
        if "update" in low and (
            # hercules update / hercules update --gateway / setsid bash -c ... hercules update
            ("hercules" in low and "update" in low.split())
            or
            # python -m hercules_cli.main update --gateway
            ("hercules_cli" in low and "update" in low.split())
            or
            # venv/bin/hercules update  (absolute path variant used in tests)
            (".venv/bin/hercules" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`hercules update` against the real checkout, fetching "
