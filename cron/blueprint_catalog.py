...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``hercules://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``hercules://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"hercules://blueprint/{quote(blueprint.key)}{qs}"
