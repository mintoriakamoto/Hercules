...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `$HERCULES_HOME/.env`** so keys
  Hercules already loads for its own tools are picked up without re-exporting - each one converts a
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `browser_*` tools. Nothing is stored. This requires Hercules to be pointed at the operator's own
      ...
      `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.hercules/chrome-debug"` (a dedicated
...
The engine stores data under `$PDD_DATA_DIR` (default `$HERCULES_HOME/unbroker`), written
...
- **Dossiers are plaintext by default** (JSON, `0600` under `HERCULES_HOME`). For at-rest encryption run
  ...
  a full-`HERCULES_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
