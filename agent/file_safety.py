...
def is_write_denied(path: str) -> bool:
    ...
    hercules_dirs = []
    for base in (_hercules_home_path(), _hercules_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in hercules_dirs:
                hercules_dirs.append(real)
        except Exception:
            continue

    for base_real in hercules_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Hercules path.

    Three categories are blocked:

      * Internal Hercules cache files under ``HERCULES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERCULES_HOME and the global Hercules
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.hercules/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active HERCULES_HOME (profile-aware) AND the global
    # Hercules root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (HERCULES_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    hercules_dirs: list[Path] = []
    for base in (_hercules_home_path(), _hercules_root_path()):
        try:
            real = base.resolve()
            if real not in hercules_dirs:
    ...
    for hd in hercules_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Hercules cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # HERCULES_HOME or <root>.
    ...
    for hd in hercules_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Hercules credential store "
    ...
    for hd in hercules_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Hercules MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Hercules MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
