...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Hercules-Session-Id header), and an
        ...
        if env_var_enabled("HERCULES_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from hercules_cli.config import load_config as _load_moa_cfg
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if agent.provider == "gemini-oauth":
        # Gemini "Login with Google" reaches the model only through Google's
        # Code Assist API (cloudcode-pa.googleapis.com/v1internal:generateContent),
        # which is neither OpenAI- nor Anthropic-shaped. A plain OpenAI client
        # pointed at that base URL POSTs to `/v1internal/chat/completions`, a
        # route Google's frontend answers with a generic HTML `Error 404`.
        # Building the Code Assist client HERE — the single chokepoint every
        # primary/recreated/per-request client passes through — guarantees the
        # translating httpx transport is present on every code path, not just
        # the one-off swap in agent_init. The token provider re-resolves the
        # bearer per request so refreshed OAuth creds are always picked up.
        from agent.gemini_code_assist import build_gemini_code_assist_client

        def _gemini_oauth_token_provider() -> str:
            from hercules_cli.auth import resolve_gemini_oauth_runtime_credentials

            return resolve_gemini_oauth_runtime_credentials().get("api_key", "")

        client = build_gemini_code_assist_client(token_provider=_gemini_oauth_token_provider)
        _ra().logger.info(
    ...
    # Delegate all rate-limit / 5xx retry to hercules's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from hercules_cli.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from hercules_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from hercules_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from hercules_cli.plugins import resolve_pre_tool_block
    ...
    from hercules_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: hercules' own dispatch loop intentionally keeps an
