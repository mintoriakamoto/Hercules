...
from hercules_constants import get_hercules_home, get_skills_dir, is_wsl
...
_HERCULES_MD_NAMES = (".hercules.md", "HERCULES.md")


def _find_hercules_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.hercules.md`` or ``HERCULES.md``.
    ...
    # could pick up a .hercules.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _HERCULES_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Hercules Agent, an intelligent AI assistant created by the Hercules project. "
...
HERCULES_AGENT_HELP_GUIDANCE = (
    "You run on Hercules Agent (by the Hercules project). When the user needs help with "
    "Hercules itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://github.com/mintoriakamoto/Hercules/tree/main/website/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `hercules-agent` skill with skill_view(name='hercules-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.hercules/kanban.db`. Your task id is in "
    "`$HERCULES_KANBAN_TASK`; your workspace is `$HERCULES_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $HERCULES_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $HERCULES_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${HERCULES_KANBAN_BRANCH:-wt/$HERCULES_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$HERCULES_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`hercules profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `hercules kanban <verb>` for board operations. Use "
...
#
# The hercules-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to hercules-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `hercules computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Hercules "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Hercules terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Hercules desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Hercules WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Hercules itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Hercules runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Hercules itself is running. The host OS, home, and cwd "
                f"of the Hercules process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Hercules "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Hercules
    ...
    extra = (os.getenv("HERCULES_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_hercules_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("HERCULES_PLATFORM") or os.environ.get("HERCULES_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.hercules/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "skills, voice, gateway, plugins, or any feature — load the `hercules-agent` skill "
            "first. It has the actual commands (e.g. `hercules config set …`, `hercules tools`, "
            "`hercules setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from HERCULES_HOME and return its content, or None.
    ...
    """
    try:
        from hercules_cli.config import ensure_hercules_home
        ensure_hercules_home()
    except Exception as e:
        logger.debug("Could not ensure HERCULES_HOME before loading SOUL.md: %s", e)

    soul_path = get_hercules_home() / "SOUL.md"
...
def _load_hercules_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.hercules.md / HERCULES.md — walk to git root."""
    hercules_md_path = _find_hercules_md(cwd_path)
    if not hercules_md_path:
        return ""
    try:
        content = hercules_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = hercules_md_path.name
        try:
            rel = str(hercules_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".hercules.md", context_length=context_length,
            read_path=str(hercules_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", hercules_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    # SOUL.md from HERCULES_HOME only — skip when already loaded as identity
    if not skip_soul:
