...
from hercules_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: hercules config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$HERCULES_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_hercules_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$HERCULES_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hercules.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / HERCULES_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.hercules/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from hercules_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.hercules/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from hercules_logging import setup_logging, setup_verbose_logging
    setup_logging(hercules_home=_ra()._hercules_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that hercules_logging.setup_logging() does not
        ...
        # handler level inside hercules_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from hercules_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `hercules model` to "
                        "select a provider, or run `hercules setup` for first-time "
                        "configuration."
                    )
        ...
        try:
            ...
            # Gemini "Login with Google" (provider ``gemini-oauth``) reaches the
            # model only via Google's Code Assist API. The Code Assist client
            # (whose httpx transport translates OpenAI<->Code Assist) is built
            # inside ``_create_openai_client`` so EVERY client-construction path
            # — this init, primary recreation, and per-request rebuilds — gets
            # the transport, not just this one-off swap. See
    ...
    # worker (kanban_show tool is present iff HERCULES_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["HERCULES_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.hercules/sessions/ alongside gateway sessions
    hercules_home = get_hercules_home()
    agent.logs_dir = hercules_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.hercules/sessions/session_{sid}.json)
    ...
    try:
        from hercules_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from hercules_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "hercules"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Hercules' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "hercules", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, hercules, off.",
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from hercules_cli.plugins import get_plugin_context_engine
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from hercules_cli.model_switch import _check_hercules_model_warning
