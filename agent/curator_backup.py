...
A pre-run snapshot of ``~/.hercules/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.hercules/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.hercules/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_hercules_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.hercules/cron/jobs.json``)."""
    return get_hercules_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    ...
    if not skills.exists():
        logger.debug("No ~/.hercules/skills/ directory — nothing to back up")
        return None
...
def _restore_cron_skill_links(snapshot_dir: Path) -> Dict[str, Any]:
    ...
    try:
        with _jobs_lock():
            ...
            if changed:
                save_jobs(live_jobs)
    except Exception as e:  # — rollback must not die mid-restore
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.hercules/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `hercules curator rollback --list` to see available snapshots)",
            None,
        )
