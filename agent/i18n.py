"""Lightweight internationalization (i18n) for Hercules static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Hercules itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``HERCULES_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``HERCULES_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("HERCULES_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "HERCULES_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def reset_language_cache() -> None:
    ...
    Call after :func:`hercules_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("HERCULES_LANGUAGE")
    if env_lang:
