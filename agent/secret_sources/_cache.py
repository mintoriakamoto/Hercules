...
cache problem must never block Hercules startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Hercules home used for cache paths.

    ``home_path`` is whatever ``load_hercules_dotenv()`` already resolved;
    falling back to ``$HERCULES_HOME`` / ``~/.hercules`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("HERCULES_HOME", Path.home() / ".hercules"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<hercules_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if HERCULES_HOME is ever made traversable.
            try:
