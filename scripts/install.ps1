# ============================================================================
# Hercules Agent Installer for Windows · mintoriakamoto
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HerculesHome = $(if ($env:HERCULES_HOME) { $env:HERCULES_HOME } else { "$env:LOCALAPPDATA\hercules" }),
    [string]$InstallDir = $(if ($env:HERCULES_HOME) { "$env:HERCULES_HOME\hercules-agent" } else { "$env:LOCALAPPDATA\hercules\hercules-agent" }),
    ...
    # builds apps/desktop into a launchable Hercules.exe.
    #
    # Why opt-in:
    #   * Hercules-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Hercules.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Hercules.exe
    ...
    #     `hercules desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:mintoriakamoto/Hercules.git"
$RepoUrlHttps = "https://github.com/mintoriakamoto/Hercules.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  The self-improving open source AI agent.              |" -ForegroundColor Magenta
    Write-Host "|  by mintoriakamoto                                           |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $HerculesHome)) {
        New-Item -ItemType Directory -Force -Path $HerculesHome | Out-Null
    }
    $envFile = Join-Path $HerculesHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $HerculesHome "node"
...
function Install-Uv {
    # Hercules owns its own uv at $HerculesHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hercules_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `hercules update` stay in sync.
    $managedUv = Join-Path $HerculesHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $HerculesHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $HerculesHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $HerculesHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $HerculesHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and HERCULES_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Hercules-Setup.exe
...
function Install-Git {
    ...
    which Hercules uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\hercules\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Hercules needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Hercules copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\hercules\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERCULES_GIT_BASH_PATH`` (User scope) so Hercules can find it in a fresh
    ...
    # Download PortableGit into $HerculesHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $HerculesHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Hercules features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Hercules needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERCULES_GIT_BASH_PATH`` (User env scope) so Hercules can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $HerculesHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $HerculesHome\git\usr\bin\bash.exe
    $candidates += "$HerculesHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$HerculesHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("HERCULES_GIT_BASH_PATH", $candidate, "User")
            $env:HERCULES_GIT_BASH_PATH = $candidate
            Write-Info "Set HERCULES_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Hercules may not find Git Bash."
    Write-Info "If needed, set HERCULES_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    # Prefer a Hercules-managed Node from a previous run over a too-old system one.
    $managedNode = "$HerculesHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$HerculesHome\node;$env:Path"
        Write-Success "Node.js $version found (Hercules-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Hercules-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $HerculesHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $HerculesHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\hercules-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$HerculesHome\node") { Remove-Item -Recurse -Force "$HerculesHome\node" }
                Move-Item $extractedDir.FullName "$HerculesHome\node"
                ...
                $version = & "$HerculesHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $HerculesHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\hercules-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `hercules
                    # update` path (#4735).
                    ...
                    $stashName = "hercules-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``hercules update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Hercules behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running hercules processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/mintoriakamoto/Hercules/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/mintoriakamoto/Hercules/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/mintoriakamoto/Hercules/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\hercules-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\hercules-agent-extract"
    ...
    # next `hercules update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Hercules-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running hercules process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running hercules processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Hercules_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (hercules.exe) plus its child tree.
            & taskkill /F /T /IM hercules.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM hercules.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m hercules_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not hercules.exe.
            ...
            # user (see hercules_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `hercules update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `hercules.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'hercules-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install hercules-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and hercules.exe broken with
    ...
    # here, not 30 seconds later when the user runs `hercules`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register hercules.exe in dist-info/RECORD but fail to
        ...
        # `hercules` command while hercules-agent.exe / hercules-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m hercules_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `hercules dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\hercules_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `hercules dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `hercules dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: hercules_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up hercules command..."
    
    if ($NoVenv) {
        $herculesBin = "$InstallDir"
    } else {
        $herculesBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so hercules is globally available
    # On Windows, the hercules.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$herculesBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$herculesBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $herculesBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HERCULES_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hercules instead
    # of the Unix default ~/.hercules
    $currentHerculesHome = [Environment]::GetEnvironmentVariable("HERCULES_HOME", "User")
    if (-not $currentHerculesHome -or $currentHerculesHome -ne $HerculesHome) {
        [Environment]::SetEnvironmentVariable("HERCULES_HOME", $HerculesHome, "User")
        Write-Success "Set HERCULES_HOME=$HerculesHome"
    }
    $env:HERCULES_HOME = $HerculesHome
    
    # Update current session
    $env:Path = "$herculesBin;$env:Path"
    
    Write-Success "hercules command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.hercules-bootstrap-complete which tells the Hercules
    ...
    # Hercules-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `hercules update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".hercules-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the HERCULES_HOME directory structure ($HerculesHome, default %LOCALAPPDATA%\hercules)
    New-Item -ItemType Directory -Force -Path "$HerculesHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HerculesHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$HerculesHome\.env"
    ...
    # Create config.yaml
    $configPath = "$HerculesHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Hercules's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$HerculesHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in hercules_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $HerculesHome"
    
    # Seed bundled skills into $HerculesHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $HerculesHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $HerculesHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$HerculesHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $HerculesHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Hercules-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'hercules setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\hercules-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\hercules-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\hercules-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Hercules` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Hercules.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_hercules_desktop command
    # resolves apps/desktop/release/win-unpacked/Hercules.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Hercules-managed
    ...
    # The --dir mode produces an unpacked Hercules.exe in
    ...
    # are NOT trying to work around). The Hercules icon + product name are
    # stamped onto Hercules.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\hercules-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Hercules.exe",
        "$desktopDir\release\win-arm64-unpacked\Hercules.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Hercules.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Hercules icon + identity are stamped onto Hercules.exe by the
    ...
    #    Hercules.exe. We deliberately do NOT point them at `hercules desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Hercules.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Hercules.lnk')
        )
        ...
        # with the Hercules icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.hercules/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'hercules setup'."
        return
    }
    ...
    # Run hercules setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m hercules_cli.main setup
    } else {
        python -m hercules_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $herculesCmd = "$InstallDir\venv\Scripts\hercules.exe"
    if (-not (Test-Path $herculesCmd)) {
        $herculesCmd = "hercules"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$HerculesHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'hercules whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: hercules gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$HerculesHome\logs\gateway.log"
            Start-Process -FilePath $herculesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$HerculesHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: hercules gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: hercules gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HerculesHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$HerculesHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HerculesHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HerculesHome\hercules-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   hercules              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   hercules setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   hercules config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   hercules config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   hercules gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   hercules update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Hercules repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Hercules-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Hercules to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/mintoriakamoto/Hercules/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
