#!/bin/bash
# ============================================================================
# Hercules Agent Installer · mintoriakamoto
# ============================================================================
...
# Python-driven tool session (e.g. Hercules terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:mintoriakamoto/Hercules.git"
REPO_URL_HTTPS="https://github.com/mintoriakamoto/Hercules.git"
HERCULES_HOME="${HERCULES_HOME:-$HOME/.hercules}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${HERCULES_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$HERCULES_INSTALL_DIR"
...
#   code at /usr/local/lib/hercules-agent, command at /usr/local/bin/hercules,
#   data still at /root/.hercules (HERCULES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --postinstall)
            ...
            ;;
        -h|--help)
            echo "Hercules Agent Installer · mintoriakamoto"
            echo ""
            ...
            echo "                   write \$HERCULES_HOME/.no-bundled-skills so future"
            echo "                   'hercules update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Hercules.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.hercules/hercules-agent"
            echo "                   default (root, Linux): /usr/local/lib/hercules-agent"
            echo "  --hercules-home PATH  Data directory (default: ~/.hercules, or \$HERCULES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hercules installs the code under"
            echo "  /usr/local/lib/hercules-agent and links the command into"
            echo "  /usr/local/bin/hercules (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERCULES_HOME"
            echo "  (default /root/.hercules).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERCULES_HOME/hercules-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs hercules setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  The self-improving open source AI agent.              │"
    echo "│  by mintoriakamoto                                           │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `hercules update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Hercules-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Hercules Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hercules command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `hercules` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERCULES_HOME/hercules-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERCULES_HOME/hercules-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/hercules-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HERCULES_HOME/hercules-agent — then preserve it)
#
# Always no-op when the user set --dir or $HERCULES_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in HERCULES_HOME.
    if is_termux; then
        INSTALL_DIR="$HERCULES_HOME/hercules-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERCULES_HOME/hercules-agent/.git" ]; then
            INSTALL_DIR="$HERCULES_HOME/hercules-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/hercules-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/hercules-agent"
        ...
        # /usr/local/bin/hercules wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/hercules"
        log_info "  Data:    $HERCULES_HOME (unchanged)"
...
# Point a Hercules-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $HERCULES_HOME/node/bin — which is NOT on
...
# Hercules's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Hercules-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$HERCULES_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$HERCULES_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$HERCULES_HOME/node/etc/npmrc"
}

get_hercules_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/hercules" ]; then
        echo "$link_dir/hercules"
    else
        echo "hercules"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/mintoriakamoto/Hercules/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hercules owns its own uv at $HERCULES_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hercules_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hercules update` stay in sync.
    local _managed_uv="$HERCULES_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $HERCULES_HOME/bin ..."
    mkdir -p "$HERCULES_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/hercules-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/hercules-uv-installer.$$.sh")"
    ...
    # directly into $HERCULES_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$HERCULES_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Hercules-
...
check_node() {
    ...
    # Repair pre-existing Hercules-managed installs where `npm install -g` lands
    ...
    if [ -x "$HERCULES_HOME/node/bin/node" ] && node_satisfies_build "$("$HERCULES_HOME/node/bin/node" --version)"; then
        export PATH="$HERCULES_HOME/node/bin:$PATH"
        log_success "Node.js $("$HERCULES_HOME/node/bin/node" --version) found (Hercules-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Hercules-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.hercules/node/..."
    ...
    # Place into ~/.hercules/node/ and symlink binaries into the same bin dir
    # the hercules command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$HERCULES_HOME/node"
    mkdir -p "$HERCULES_HOME"
    mv "$extracted_dir" "$HERCULES_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$HERCULES_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$HERCULES_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$HERCULES_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$HERCULES_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.hercules/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Hercules install may complete, but web search and dependency downloads can fail."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `hercules update` path
                # (#4735).
                ...
                local stash_name
                stash_name="hercules-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``hercules update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Hercules behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up hercules command..."

    if [ "$USE_VENV" = true ]; then
        HERCULES_BIN="$INSTALL_DIR/venv/bin/hercules"
    else
        HERCULES_BIN="$(which hercules 2>/dev/null || echo "")"
        if [ -z "$HERCULES_BIN" ]; then
            log_warn "hercules not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERCULES_BIN" ]; then
        log_warn "hercules entry point not found at $HERCULES_BIN"
    ...
    # Create a user-facing shim for the hercules command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $HERCULES_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$HERCULES_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/hercules"
    cat > "$command_link_dir/hercules" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HERCULES_BIN" "\$@"
EOF
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_success "hercules command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v hercules' \
                >/dev/null 2>&1; then
        ...
        log_info "hercules not on PATH in non-login shells (common on RHEL-family)"
    ...
    # Export for current session so hercules works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.hercules directory structure (config at top level, code in subdir)
    mkdir -p "$HERCULES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.hercules/.env (top level, easy to find)
    if [ ! -f "$HERCULES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HERCULES_HOME/.env"
            log_success "Created ~/.hercules/.env from template"
        else
            touch "$HERCULES_HOME/.env"
            log_success "Created ~/.hercules/.env"
        fi
    else
        log_info "~/.hercules/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.hercules/config.yaml (top level, easy to find)
    if [ ! -f "$HERCULES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HERCULES_HOME/config.yaml"
            log_success "Created ~/.hercules/config.yaml from template"
        fi
    else
        log_info "~/.hercules/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in hercules_cli/default_soul.py — the
    ...
    if [ ! -f "$HERCULES_HOME/SOUL.md" ]; then
        cat > "$HERCULES_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.hercules/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.hercules/"

    # Seed bundled skills into ~/.hercules/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `hercules update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'hercules update'." \
            > "$HERCULES_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $HERCULES_HOME/.no-bundled-skills"
        log_info "  Future 'hercules update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.hercules/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.hercules/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HERCULES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$HERCULES_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.hercules/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$HERCULES_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Hercules Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Hercules will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Hercules Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (hercules --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `hercules update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'hercules setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m hercules_cli.main setup < /dev/tty
    else
        python -m hercules_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$HERCULES_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Hercules to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$HERCULES_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'hercules whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HERCULES_CMD="$(get_hercules_command_path)"
                $HERCULES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'hercules whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'hercules gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERCULES_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $HERCULES_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: hercules gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: hercules gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HERCULES_CMD gateway > "$HERCULES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.hercules/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: hercules gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: hercules gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $HERCULES_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $HERCULES_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $HERCULES_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}hercules${NC}              Start chatting"
    echo -e "   ${GREEN}hercules setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}hercules config${NC}       View/edit configuration"
    echo -e "   ${GREEN}hercules config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}hercules gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}hercules update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'hercules' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'hercules' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'hercules' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$HERCULES_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$HERCULES_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$HERCULES_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$HERCULES_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$HERCULES_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Hercules for pip install"
    ...
    HERCULES_CMD="$(command -v hercules 2>/dev/null || echo "")"
    if [ -n "$HERCULES_CMD" ]; then
        log_info "Running hercules setup..."
        "$HERCULES_CMD" setup
    else
        log_warn "hercules command not found on PATH"
        log_info "Try: python -m hercules_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Hercules` rename
...
# _purge_electron_build_cache() used by `hercules desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Hercules-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `hercules desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Hercules" ]; then
            app="$desktop_dir/release/linux-unpacked/Hercules"
        elif [ -x "$desktop_dir/release/linux-unpacked/hercules" ]; then
            app="$desktop_dir/release/linux-unpacked/hercules"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Hercules.app" \
            "$desktop_dir/release/mac/Hercules.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Hercules is damaged and can't be opened").
    ...
    # checkout stays clean for the next `hercules update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Hercules-managed Node
            # provisioned during prerequisites/node-deps (at $HERCULES_HOME/node/bin)
        ...
        complete)
            ...
            # $HERCULES_HOME. $HERCULES_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'hercules update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $HERCULES_HOME.
    # $HERCULES_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'hercules update' on this host install.
