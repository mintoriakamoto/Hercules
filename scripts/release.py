...
VERSION_FILE = REPO_ROOT / "hercules_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level HERCULES_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (hercules journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "hercules.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.hercules/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows hercules-shim resolution + prefer --update on recovery; #52378)
    ...
    "hercules@marian.local": "Schrotti77",
    ...
    "274096618+hercules-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "hercules@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on hercules mcp add)
    ...
    "hercules@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@hercules-agent": "teknium1",
    ...
    "github@nadyahercules.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (HERCULES_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (HERCULES_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: hercules-eval community link)
    ...
    "herculesagent26@gmail.com": "herculesagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+herculesagent26@users.noreply.github.com": "herculesagent26",
    ...
    "vigo@hercules": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.hercules@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Hercules runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level HERCULES_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Hercules
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"hercules-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "hercules@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hercules-agent",
...
def main():
    ...
    print(f"{'='*60}")
    print("  Hercules Agent Release Preview")
    print(f"{'='*60}")
