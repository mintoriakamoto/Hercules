...
Environment:
    HERCULES_TEST_WORKERS  Override worker count (default: os.cpu_count())
    HERCULES_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        nousresearch/hercules-agent:test image, via
#                        ``HERCULES_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or HERCULES_TEST_FILE_TIMEOUT.
#
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/hercules-agent/hercules-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        "--paths",
        default=os.environ.get("HERCULES_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("HERCULES_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Env: HERCULES_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "--generate-slices",
    ...
    # Parse --slice (or HERCULES_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("HERCULES_TEST_SLICE")
