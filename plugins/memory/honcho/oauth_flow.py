...
``hercules://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Hercules OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "hercules-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``hercules-desktop`` / ``hercules-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Hercules.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local HERCULES_HOME override can't reach.
