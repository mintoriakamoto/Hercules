...
from hercules_constants import get_hercules_home  # noqa: F401 — patch target for mem0 setup tests
...
def _save_mem0_json(hercules_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(hercules_home) / "mem0.json"
    existing = {}
...
def _setup_platform(hercules_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "hercules-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "hercules"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.hercules/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.hercules/.env (or unset it) or Hercules will keep "
    ...
    provider.save_config(provider_config, hercules_home)

    if env_writes:
        _write_env(Path(hercules_home) / ".env", env_writes)
...
def _setup_selfhosted(hercules_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "hercules-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "hercules")
...
def _setup_oss(hercules_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(hercules_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "hercules-user")
    ...
    if env_writes:
        _write_env(Path(hercules_home) / ".env", env_writes)
    _save_mem0_json(hercules_home, {"mode": "oss", "user_id": user_id, "agent_id": "hercules", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, hercules_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    if not existing:
        env_path = Path(hercules_home) / ".env"
        if env_path.exists():
...
_PGVECTOR_CONTAINER = "hercules-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "hercules"
...
def _setup_oss_interactive(hercules_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], hercules_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], hercules_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'hercules-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "hercules-user")

    agent_id = input("  Agent ID [hercules]: ").strip()
    agent_id = agent_id or "hercules"
    ...
    if env_writes:
        _write_env(Path(hercules_home) / ".env", env_writes)
    _save_mem0_json(hercules_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(hercules_home: str, config: dict) -> None:
    """Entry point called by hercules memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(hercules_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(hercules_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(hercules_home, config, flags)
        return
