"""agentmemory memory provider for Hercules.

Connects to agentmemory server for persistent, distributed memory with
semantic search, cross-session recall, and automatic memory consolidation.

Configuration (set via hercules memory setup or env vars):
  AGENTMEMORY_URL    — agentmemory server URL (default: http://localhost:3111)
  AGENTMEMORY_SECRET — optional Bearer auth token
  AGENTMEMORY_PROJECT_NAME — project scope override (auto-detected from git/cwd)
...
logger = None  # Defer import to avoid circular deps
...
def _resolve_project(cwd: str) -> str:
    """Canonical project scope: env override, git toplevel, cwd basename."""
...
def _preload_agentmemory_dotenv() -> None:
    """Preload agentmemory .env file to support systemd service deployments."""
    candidates = []
...
def _api(
    ...
    """Make HTTP request to agentmemory server."""
...
class AgentMemoryProvider(MemoryProvider):
    ...
    def is_available(self) -> bool:
        """Check if agentmemory server URL is valid (no network call)."""
        base = os.environ.get("AGENTMEMORY_URL", DEFAULT_BASE_URL)
        return _validate_url(base)

    def initialize(self, session_id: str, **kwargs: Any) -> None:
        """Initialize provider for this session."""
    ...
    def get_config_schema(self) -> list[dict]:
        """Return configuration schema for setup wizard."""
        return [
            {
                "key": "url",
    ...
    def save_config(self, values: dict, hercules_home: str) -> None:
        """Save config to agentmemory.json in hercules home."""
        config_path = Path(hercules_home) / "agentmemory.json"
        config_path.write_text(json.dumps(values, indent=2))

    def system_prompt_block(self) -> str:
        """Fetch memory context to inject into system prompt."""
        result = _api(
            self._base,
            "context",
            {
    ...
    def queue_prefetch(self, query: str, **kwargs: Any) -> None:
        """Prefetch memory in background."""
        _api_bg(self._base, "smart-search", {"query": query, "limit": 3})

    def get_tool_schemas(self) -> list[dict]:
        """Return tool schemas for model."""
        return [
            {
                "name": "memory_recall",
    ...
    def sync_turn(self, user: str, assistant: str, **kwargs: Any) -> None:
        """Record turn in memory after completion."""
        _api_bg(
            self._base,
            "observe",
            {
    ...
    def on_session_end(self, messages: list, **kwargs: Any) -> None:
        """Clean up session in agentmemory on exit."""
        _api(
            self._base,
            "session/end",
            {
    ...
    def on_pre_compress(self, messages: list, **kwargs: Any) -> None:
        """Inject memory summary before context compression."""
        result = _api(
            self._base,
            "context",
            {
    ...
    def on_memory_write(self, action: str, target: str, content: str, **kwargs: Any) -> None:
        """Save important content to agentmemory."""
