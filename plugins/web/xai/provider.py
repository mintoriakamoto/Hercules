...
``{title, url, description, position}`` rows every other Hercules web
provider produces.
...
prefers Hercules-managed xAI Grok OAuth (via ``hercules auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.hercules/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``hercules tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `hercules auth` to sign in with "
