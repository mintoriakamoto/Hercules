...
Wires ``hercules meet <subcommand>``:
...
def _auth_state_path() -> Path:
    return Path(get_hercules_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``hercules meet`` argparse tree.
    ...
    try:
        ...
        # ``e`` is unbound once the except block exits, so capture it.
        import_error = e

        def _node_unavailable(args):
            print(f"hercules meet node: module unavailable ({import_error})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: hercules meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: hercules meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: hercules meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "hercules meet join https://meet.google.com/abc-defg-hij"
        )
    else:
    ...
    if realtime:
        ...
        if system == "Linux":
            ...
            print(
                ...
                "  hercules will not switch your default input for you."
            )
    else:
        print("\n[3/3] skipped (pass --realtime to install audio tooling too)")

    print("\ndone. verify with: hercules meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: hercules meet join <url>")
    return 0
...
def _cmd_stop() -> int:
    res = pm.stop(reason="hercules meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1


if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="hercules meet")
    register_cli(parser)
