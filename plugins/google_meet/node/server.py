...
Launched by ``hercules meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$HERCULES_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``hercules meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_hercules_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "hercules-meet-node",
    ) -> None:
        self.host = host
        self.port = port
