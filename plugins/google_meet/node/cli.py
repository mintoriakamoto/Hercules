"""`hercules meet node ...` subcommand tree.

Wired into the existing ``hercules meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``hercules meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="hercules-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``hercules meet node ...``.
