...
``$HERCULES_HOME/workspace/meetings/<meeting-id>/``. The main hercules process
...
Run standalone for debugging::
    ...
    HERCULES_MEET_URL=https://meet.google.com/abc-defg-hij \\
    HERCULES_MEET_OUT_DIR=/tmp/meet-debug \\
    HERCULES_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``HERCULES_MEET_OUT_DIR``.
...
# and exposes ``window.__herculesMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__herculesMeetInstalled) return;
  window.__herculesMeetInstalled = true;
  window.__herculesMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__herculesMeetQueue.push({
      ts: Date.now(),
  ...
  window.__herculesMeetDrain = () => {
    const out = window.__herculesMeetQueue.slice();
    window.__herculesMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "hercules_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:
    ...
    auth_state = os.environ.get("HERCULES_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("HERCULES_MEET_GUEST_NAME", "Hercules Agent")
    duration_s = _parse_duration(os.environ.get("HERCULES_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when HERCULES_MEET_MODE=realtime.
    mode = os.environ.get("HERCULES_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("HERCULES_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("HERCULES_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("HERCULES_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("HERCULES_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — HERCULES_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: HERCULES_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in HERCULES_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("HERCULES_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__herculesMeetDrain && window.__herculesMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__herculesMeetInstalled) {
