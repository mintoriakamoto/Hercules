"""ntfy platform adapter (Hercules plugin).
...
a Hercules dependency.

This adapter ships as a Hercules platform plugin under
``plugins/platforms/ntfy/``. The Hercules plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "hercules-in"              # subscribe topic (incoming)
          publish_topic: "hercules-out"     # optional — defaults to topic
...
_ECHO_TAG = "hercules-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``hercules cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Hercules dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `hercules gateway status` without
