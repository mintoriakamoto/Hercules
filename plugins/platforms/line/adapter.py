...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``HERCULES_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hercules_home = Path(get_hercules_home()).resolve()
        except Exception:
            hercules_home = Path.home().joinpath(".hercules").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``hercules status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``hercules status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hercules setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.hercules/.env`` via ``hercules_cli.config``.
    """
    print()
    ...
    try:
        from hercules_cli.config import get_env_var, set_env_var
    except ImportError:
        print("hercules_cli.config not available; set LINE_* vars manually in ~/.hercules/.env")
        return
