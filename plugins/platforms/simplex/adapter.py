"""SimpleX Chat platform adapter (Hercules plugin).
...
This adapter ships as a Hercules platform plugin under
``plugins/platforms/simplex/``. The Hercules plugin loader scans the
...
Optional environment variables:
    ...
    HERCULES_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``hercules setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Hercules — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("HERCULES_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hercules setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.hercules/.env`` via
    ``hercules_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "hercules_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.hercules/.env"
        )
        return
