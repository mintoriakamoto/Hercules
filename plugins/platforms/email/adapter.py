"""
Email platform adapter for the Hercules gateway.

Allows users to interact with Hercules by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "hercules-agent" "version" "{_hercules_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `hercules config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `hercules gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        msg_id = f"<hercules-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# hercules_cli/gateway.py, and the _send_email dispatch in
