"""
Photon Spectrum (iMessage) platform adapter for Hercules Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Hercules dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `hercules photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `hercules setup` / status.
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `hercules update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``hercules photon install-sidecar``: ``npm ci`` for an exact,
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Hercules defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Hercules-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
                ...
                async with client.stream(
                    ...
                    async for line in resp.aiter_lines():
                        ...
                        if not stripped_line:
                            continue  # heartbeat
                        await self._on_inbound_line(stripped_line)
            except asyncio.CancelledError:
                raise
            except Exception as e:
                if not self._inbound_running:
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Hercules checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `hercules photon setup`)"
            )
        # A `hercules update` that bumps the spectrum-ts pin rewrites
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Hercules-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Hercules plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `hercules photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: hercules photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `hercules gateway setup` alongside every other
    ...
    # Register CLI subcommands — `hercules photon ...`
