...
messages to/from the Hercules agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "hercules-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: hercules_, hercules_1, hercules_2...
...
def check_requirements() -> bool:
    ...
    # hercules setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `hercules gateway setup` flow for the IRC platform.

    Lazy-imports ``hercules_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Hercules to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #hercules — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.hercules/.env")
    print_info("Restart the gateway for changes to take effect: hercules gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "hercules-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "hercules-bot"
