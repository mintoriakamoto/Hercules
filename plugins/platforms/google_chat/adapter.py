...
# ``model_tools`` import time, so ``hercules status``, ``hercules chat``, etc.
...
# ``hercules_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Hercules is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a hercules MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${HERCULES_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _hercules_home = _get_hercules_home()
        except (ModuleNotFoundError, ImportError):
            _hercules_home = _Path.home() / ".hercules"
        self._thread_count_store = _ThreadCountStore(
            _hercules_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Hercules is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HERCULES_HOME", str(_Path.home() / ".hercules"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${HERCULES_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a hercules MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Hercules is thinking…' marker message.
        ...
        "Hercules is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Hercules is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Hercules is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Hercules is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Hercules is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``hercules setup``.

    The setup wizard at ``hercules_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.hercules/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. hercules-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.hercules/.env")
    print_info("Restart the gateway: hercules gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``hercules cron`` running as a
    separate process from ``hercules gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Hercules plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'hercules-agent[google_chat]'",
        ...
        # the "Hercules is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Hercules is thinking…' marker message "
