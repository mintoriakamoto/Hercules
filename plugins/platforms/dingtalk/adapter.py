...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "Hercules", "text": normalized},
        }
    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            out_track_id = f"hercules_{uuid.uuid4().hex[:12]}"
...
class _IncomingHandler(
    ...
    async def process(self, message: "CallbackMessage"):
        ...
        try:
            ...
            self._adapter._spawn_background_task(self._safe_on_message(chatbot_msg))
        except Exception:
            logger.exception(
...
# dict in hercules_cli/gateway.py, and the _send_dingtalk dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces hercules_cli/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in hercules_cli/gateway.py. CLI helpers are
    ...
    if method == 0:
        try:
            from hercules_cli.dingtalk_auth import dingtalk_qr_auth
