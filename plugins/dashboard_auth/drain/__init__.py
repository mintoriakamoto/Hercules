...
What it is
----------
A service-to-service auth provider. The hosting account service (NAS) provisions
a **per-agent unique** shared secret into each deployed agent's environment; this
...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    HERCULES_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
When ``HERCULES_DASHBOARD_DRAIN_SECRET`` is unset, the plugin is a no-op (records
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``HERCULES_DASHBOARD_DRAIN_SECRET`` is
    ...
    secret = os.environ.get("HERCULES_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        LAST_SKIP_REASON = (
            "HERCULES_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent "
    ...
    if reason is not None:
        LAST_SKIP_REASON = (
            f"HERCULES_DASHBOARD_DRAIN_SECRET rejected — {reason}. "
