...
``~/.hercules/logs/`` (profile-aware via ``get_hercules_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``hercules_cli.web_server``,
    ``hercules_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Hercules (TUI, gateway, ``hy_memory`` server, MCP
...
from hercules_constants import get_config_path, get_hercules_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_hercules_session_injector", False):
    ...
    _session_record_factory._hercules_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``hercules logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``hercules logs --component gateway``.
    "gateway": ("gateway", "hercules_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("hercules_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "hercules_cli.web_server",
        "hercules_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    hercules_home: Optional[Path] = None,
    ...
    """Configure the Hercules logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hercules_home
        Override for the Hercules home directory.  Falls back to
        ``get_hercules_home()`` (profile-aware).
    log_level
    ...
    home = hercules_home or get_hercules_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_hercules_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._hercules_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the hercules group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from hercules_cli.config import is_managed
...
# the module header): when several Hercules processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._hercules_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="hercules-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_hercules_queue", False):
