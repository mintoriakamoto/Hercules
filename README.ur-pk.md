...
> **توجہ فرمائیں:** مقامی ونڈوز (Native Windows) پر ہرکیولیس بغیر WSL کے چلتا ہے — CLI، گیٹ وے، TUI، اور ٹولز سب مقامی طور پر کام کرتے ہیں۔ اگر آپ WSL2 استعمال کرنا پسند کرتے ہیں، تو اوپر دی گئی لینکس/میک او ایس کی کمانڈ وہاں بھی کام کرے گی۔ کوئی مسئلہ نظر آیا؟ براہ کرم [مسائل (issues) درج کریں](https://github.com/mintoriakamoto/Hercules/issues)۔
...
انسٹالر سب کچھ خود سنبھالتا ہے: uv، Python 3.11، Node.js، ripgrep، ffmpeg، **اور ایک پورٹ ایبل (portable) گٹ بیش (Git Bash)** (یعنی MinGit، جو `%LOCALAPPDATA%\hercules\git` میں ان پیک ہوتا ہے — اس کے لیے ایڈمن کی اجازت درکار نہیں، اور یہ سسٹم کے کسی بھی گٹ انسٹال سے بالکل الگ ہے)۔ ہرکیولیس اس بنڈل شدہ گٹ بیش کو شیل کمانڈز چلانے کے لیے استعمال کرتا ہے۔
...
> **اینڈرائیڈ (Android) / ٹرمکس (Termux):** ٹیسٹ کیا گیا مینوئل طریقہ [Termux گائیڈ](website/docs/getting-started/termux.md) میں موجود ہے۔ ٹرمکس پر ہرکیولیس ایک مخصوص `.[termux]` ایکسٹرا انسٹال کرتا ہے کیونکہ مکمل `.[all]` ایکسٹرا میں ایسی وائس ڈیپینڈینسیز شامل ہیں جو اینڈرائیڈ کے ساتھ مطابقت نہیں رکھتیں۔
>
> **ونڈوز (Windows):** مقامی ونڈوز کی مکمل سپورٹ موجود ہے — اوپر دی گئی پاور شیل کی کمانڈ سب کچھ انسٹال کر دیتی ہے۔ اگر آپ WSL2 استعمال کرنا چاہتے ہیں، تو لینکس کی کمانڈ وہاں کام کرتی ہے۔ مقامی ونڈوز میں انسٹالیشن `%LOCALAPPDATA%\hercules` میں ہوتی ہے؛ جبکہ WSL2 میں لینکس کی طرح `~/.hercules` میں ہوتی ہے۔ ہرکیولیس کا وہ واحد فیچر جسے فی الحال خاص طور پر WSL2 کی ضرورت ہے وہ براؤزر پر مبنی ڈیش بورڈ چیٹ پین ہے (یہ POSIX PTY استعمال کرتا ہے — کلاسک CLI اور گیٹ وے دونوں مقامی طور پر چلتے ہیں)۔
...
```bash
hercules              # انٹرایکٹو CLI — بات چیت شروع کریں
hercules model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
hercules tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
hercules config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
hercules gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
hercules setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
hercules claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
hercules update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
hercules doctor       # کسی بھی مسئلے کی تشخیص کریں
```
...
ہرکیولیس کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `hercules` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `hercules`                                      | `hercules gateway setup` اور `hercules gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
تمام دستاویزات **[website/docs](website/docs/)** پر موجود ہیں:
...
اگر آپ OpenClaw سے منتقل ہو رہے ہیں، تو ہرکیولیس آپ کی سیٹنگز، یادیں (memories)، مہارتیں (skills)، اور API کیز کو خود بخود امپورٹ کر سکتا ہے۔

**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`hercules setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
hercules claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
hercules claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
hercules claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
hercules claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.hercules/skills/openclaw-imports/`
...
تمام آپشنز دیکھنے کے لیے `hercules claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔
...
معاونین (contributors) کے لیے فوری آغاز — کلون (clone) کریں اور `setup-hercules.sh` چلائیں:
...
- 🐛 [مسائل (Issues)](https://github.com/mintoriakamoto/Hercules/issues)
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — ہرکیولیس اور دیگر MCP ہوسٹس کے لیے لینکس (Linux) ڈیسک ٹاپ کنٹرول MCP سرور، جس میں AT-SPI ایکسیسیبلٹی ٹریز، Wayland/X11 ان پٹ، سکرین شاٹس، اور کمپوزیٹر ونڈو ٹارگیٹنگ شامل ہے۔
- 🔌 [HerculesClaw](https://github.com/AaronWong1999/herculesclaw) — کمیونٹی وی چیٹ (WeChat) برج: ہرکیولیس ایجنٹ اور OpenClaw کو ایک ہی وی چیٹ اکاؤنٹ پر چلائیں۔
