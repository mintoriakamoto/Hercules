...
# Kimi Code OAuth: OpenAI-compatible inference endpoint the OAuth Bearer hits
# (see the Kimi Code OAuth section below for the full credential wiring).
DEFAULT_KIMI_OAUTH_BASE_URL = "https://api.kimi.com/coding"
...
SPOTIFY_DOCS_URL = "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/features/spotify.md"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/guides/oauth-over-ssh.md"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "kimi-oauth": ProviderConfig(
        id="kimi-oauth",
        name="Kimi Code (OAuth · subscription login)",
        auth_type="oauth_external",
        # OpenAI-compatible; the OAuth access token is used as a plain Bearer.
        # Credentials come from the Kimi Code CLI's device-code login, read by
        # resolve_kimi_oauth_runtime_credentials().
        inference_base_url=DEFAULT_KIMI_OAUTH_BASE_URL,
        base_url_env_var="KIMI_OAUTH_BASE_URL",
    ),
    "gemini-oauth": ProviderConfig(
        id="gemini-oauth",
        name="Gemini (OAuth · Login with Google)",
        auth_type="oauth_external",
        # NOT OpenAI-compatible directly: the Gemini "Login with Google" token
        # reaches the model only via the Code Assist API. agent_init builds a
        # translating client (agent/gemini_code_assist.py) so the runtime still
        # sees api_mode == chat_completions. Creds come from the Gemini CLI,
        # read by resolve_gemini_oauth_runtime_credentials().
...
def resolve_provider(
    ...
    *,
    ...
    _PROVIDER_ALIASES = {
        ...
        "kimi-oauth": "kimi-oauth", "kimi-code": "kimi-oauth", "kimi-code-oauth": "kimi-oauth",
        "gemini-oauth": "gemini-oauth", "gemini-cli": "gemini-oauth", "gemini-code-assist": "gemini-oauth",
...
# =============================================================================
# Kimi Code OAuth — reuse the Kimi Code CLI's device-code login
# ("login to the app, not an API key"). The Kimi Code CLI runs the device-code
# flow and writes creds to ~/.kimi/credentials/kimi-code.json, refreshing the
# access token itself (~10 min cadence). Hercules reads that token and uses it
# as a plain Bearer against the OpenAI-compatible https://api.kimi.com/coding
# endpoint — the same shape as the existing kimi-coding provider. Mirrors the
# qwen-oauth "reuse the CLI's creds" pattern.
#
# The read path depends only on confirmed facts (cred file location +
# OpenAI-compatible base URL). Refresh is best-effort, in-memory only (never
# rewrites the CLI's file), and every endpoint/id is env-overridable; any
# mismatch degrades to a clear "re-login via the Kimi Code CLI" message.
# =============================================================================

# DEFAULT_KIMI_OAUTH_BASE_URL is defined above (near the other base-URL
# constants) so the PROVIDER_REGISTRY entry can reference it at import time.
KIMI_OAUTH_DEFAULT_HOST = "https://auth.kimi.com"
KIMI_OAUTH_DEFAULT_CLIENT_ID = "17e5f671-d194-4dfb-9706-5516cb48c098"
...
def _kimi_oauth_host() -> str:
    return (
        ...
        or KIMI_OAUTH_DEFAULT_HOST
    ).strip().rstrip("/")


def _kimi_oauth_client_id() -> str:
    return (os.getenv("KIMI_OAUTH_CLIENT_ID") or KIMI_OAUTH_DEFAULT_CLIENT_ID).strip()


def _kimi_oauth_token_url() -> str:
    return f"{_kimi_oauth_host()}/api/oauth/token"


def _kimi_oauth_base_url() -> str:
    return (
        os.getenv("KIMI_OAUTH_BASE_URL")
        or os.getenv("KIMI_BASE_URL")
        or DEFAULT_KIMI_OAUTH_BASE_URL
    ).strip().rstrip("/")


def _kimi_oauth_cred_candidates() -> List[Path]:
    """Credential files the Kimi Code CLI may have written, priority order.

    An explicit HERCULES_KIMI_OAUTH_CREDS_FILE always wins so a nonstandard
    layout can be pointed at without a code change.
    """
    override = os.getenv("HERCULES_KIMI_OAUTH_CREDS_FILE", "").strip()
    paths: List[Path] = []
    if override:
...
def _kimi_oauth_cred_path() -> Optional[Path]:
    for p in _kimi_oauth_cred_candidates():
        try:
            if p.exists():
                return p
        except OSError:
...
def _kimi_pick(data: Dict[str, Any], *keys: str) -> str:
    """First non-empty string among candidate key names (defensive: the Kimi
    CLI credential schema is not a stable public contract)."""
    for k in keys:
        v = data.get(k)
        if isinstance(v, dict):
            for kk in ("access_token", "accessToken", "token", "value"):
                vv = v.get(kk)
...
def _kimi_expiry_ms(data: Dict[str, Any]) -> Optional[int]:
    """Best-effort expiry in epoch-ms from whatever the CLI stored."""
    for k in ("expiry_date", "expires_at", "expiresAt", "expiry", "expires"):
        v = data.get(k)
        if v is None:
            continue
        try:
            ...
            continue
        return n if n > 10_000_000_000 else n * 1000  # ms vs seconds heuristic
    for k in ("expires_in", "expiresIn"):
        v = data.get(k)
        try:
            return int(time.time() * 1000) + max(1, int(float(v))) * 1000
        except (TypeError, ValueError):
            continue
    return None


def _read_kimi_oauth_authstore_tokens() -> Optional[Dict[str, Any]]:
    """Tokens from Hercules' own device-code login, if present in the auth store.

    Returns a copy tagged with ``_source="hercules-login"`` so callers can tell
    a Hercules-owned grant (which we may refresh and persist) apart from a
    read-only Kimi Code CLI session.
    """
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "kimi-oauth") or {}
    tokens = state.get("tokens")
    if isinstance(tokens, dict) and _kimi_pick(tokens, "access_token", "accessToken", "token"):
        out = dict(tokens)
        out["_source"] = "hercules-login"
        return out
    return None


def _read_kimi_oauth_tokens() -> Dict[str, Any]:
    # Prefer Hercules' own device-code login over a Kimi Code CLI session.
    store_tokens = _read_kimi_oauth_authstore_tokens()
    if store_tokens is not None:
        return store_tokens
    path = _kimi_oauth_cred_path()
    if path is None:
        raise AuthError(
            "Kimi Code OAuth credentials not found. Run `hercules model` (or "
            "`hercules setup`) and choose Kimi to log in (device-code — opens "
            "kimi.com), or use the Kimi Code CLI (`kimi` → /login).",
            provider="kimi-oauth",
    ...
    try:
        ...
        raise AuthError(
            f"Failed to read Kimi Code credentials from {path}: {exc}",
            provider="kimi-oauth",
            code="kimi_auth_read_failed",
        ) from exc
    if not isinstance(data, dict):
        raise AuthError(
            f"Invalid Kimi Code credentials in {path}.",
            provider="kimi-oauth",
...
def _kimi_access_token_is_expiring(
    expiry_ms: Any, skew_seconds: int = KIMI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
) -> bool:
    try:
        expiry = int(expiry_ms)
    except (TypeError, ValueError):
        return False  # unknown expiry → trust the CLI to keep it fresh
    return (time.time() + max(0, int(skew_seconds))) * 1000 >= expiry


def _refresh_kimi_oauth_tokens(
    tokens: Dict[str, Any], timeout_seconds: float = 20.0
) -> Dict[str, Any]:
    """Best-effort, in-memory refresh (never rewrites the CLI's file). The Kimi
    Code CLI normally refreshes its own token; this only fires if Hercules sees
    an expired token with a usable refresh_token. Any failure raises an
    actionable re-login AuthError."""
    refresh_token = _kimi_pick(tokens, "refresh_token", "refreshToken")
    if not refresh_token:
        raise AuthError(
            "Kimi Code OAuth token expired and no refresh token is available. "
            "Re-login with the Kimi Code CLI (`kimi` → /login).",
            provider="kimi-oauth",
    ...
    try:
        ...
        raise AuthError(
            f"Kimi Code OAuth refresh failed: {exc}. Re-login with the Kimi Code CLI.",
            provider="kimi-oauth",
            code="kimi_refresh_failed",
        ) from exc
    if response.status_code >= 400:
        raise AuthError(
            "Kimi Code OAuth refresh failed. Re-login with the Kimi Code CLI (`kimi` → /login).",
            provider="kimi-oauth",
            code="kimi_refresh_failed",
        )
    try:
        ...
        raise AuthError(
            f"Kimi Code OAuth refresh returned invalid JSON: {exc}",
            provider="kimi-oauth",
            code="kimi_refresh_invalid_json",
        ) from exc
    access = (
        _kimi_pick(payload, "access_token", "accessToken", "token")
        if isinstance(payload, dict)
        else ""
    )
    if not access:
        raise AuthError(
            "Kimi Code OAuth refresh response missing access token.",
            provider="kimi-oauth",
            code="kimi_refresh_invalid_response",
        )
    refreshed = dict(tokens)
    refreshed["access_token"] = access
    new_refresh = _kimi_pick(payload, "refresh_token", "refreshToken")
    if new_refresh:
        refreshed["refresh_token"] = new_refresh
    exp = _kimi_expiry_ms(payload)
    if exp:
        refreshed["expiry_date"] = exp
    return refreshed
...
def resolve_kimi_oauth_runtime_credentials(
    *,
    ...
    tokens = _read_kimi_oauth_tokens()
    owns_login = tokens.pop("_source", "") == "hercules-login"
    access_token = _kimi_pick(tokens, "access_token", "accessToken", "token")
    should_refresh = bool(force_refresh)
    if not should_refresh and refresh_if_expiring and access_token:
        should_refresh = _kimi_access_token_is_expiring(
            _kimi_expiry_ms(tokens), refresh_skew_seconds
        )
    if should_refresh:
        tokens = _refresh_kimi_oauth_tokens(tokens)
        access_token = _kimi_pick(tokens, "access_token", "accessToken", "token")
        # Persist the rotated grant back to the auth store — but only for a
        # Hercules-owned login. A Kimi Code CLI session stays read-only.
        if owns_login:
            _save_kimi_oauth_tokens(tokens, base_url=_kimi_oauth_base_url())
    if not access_token:
        raise AuthError(
            "Kimi Code OAuth access token missing. Re-login via `hercules model` "
            "(choose Kimi) or the Kimi Code CLI (`kimi` → /login).",
            provider="kimi-oauth",
            code="kimi_access_token_missing",
        )
    return {
        "provider": "kimi-oauth",
        "base_url": _kimi_oauth_base_url(),
        "api_key": access_token,
        "source": "device-code" if owns_login else "kimi-cli",
        "expires_at_ms": _kimi_expiry_ms(tokens),
        "auth_file": "" if owns_login else str(_kimi_oauth_cred_path() or ""),
    }
...
def get_kimi_oauth_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "logged_in": False,
            "auth_file": str(path or _kimi_oauth_cred_candidates()[0]),
            "error": str(exc),
        }


# -----------------------------------------------------------------------------
# Kimi Code OAuth — real in-app login via the Device Authorization Grant
# (RFC 8628). Unlike the read-only "reuse the Kimi Code CLI session" path above,
# this performs the login itself: request a device code, show the user a URL +
# code, poll the token endpoint, and persist the grant to Hercules' auth store.
#
# The token endpoint + client_id are the same ones the refresh path already
# uses successfully. The device-authorization endpoint path is not officially
# documented, so it is env-overridable and we try known candidates in order.
# -----------------------------------------------------------------------------

# Kimi's inference scope. Unknown/optional — override via KIMI_OAUTH_SCOPE.
KIMI_OAUTH_DEFAULT_SCOPE = ""


def _kimi_oauth_scope() -> str:
    return os.getenv("KIMI_OAUTH_SCOPE", KIMI_OAUTH_DEFAULT_SCOPE).strip()


def _kimi_oauth_device_code_urls() -> List[str]:
    """Candidate device-authorization endpoints, priority order.

    An explicit KIMI_OAUTH_DEVICE_CODE_URL always wins. The RFC-style
    ``/api/oauth/device_authorization`` is tried first (surfaced from a live
    Kimi error message), then the GitHub-style ``/api/oauth/device/code`` used
    by Hercules' other device-code providers.
    """
    override = os.getenv("KIMI_OAUTH_DEVICE_CODE_URL", "").strip()
    if override:
        return [override]
    host = _kimi_oauth_host()
    return [
        f"{host}/api/oauth/device_authorization",
        f"{host}/api/oauth/device/code",
    ]


def _save_kimi_oauth_tokens(tokens: Dict[str, Any], *, base_url: str = "") -> None:
    """Persist a Hercules-owned Kimi OAuth grant to the auth store."""
    clean = {k: v for k, v in tokens.items() if k != "_source"}
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "kimi-oauth") or {}
        state["tokens"] = clean
        state["source"] = "device-code"
        state["last_refresh"] = (
...
def _kimi_oauth_request_device_code(client: httpx.Client) -> Dict[str, Any]:
    """POST the device-authorization request, trying endpoint candidates."""
    scope = _kimi_oauth_scope()
    data = {"client_id": _kimi_oauth_client_id(), **({"scope": scope} if scope else {})}
    urls = _kimi_oauth_device_code_urls()
    last_status = None
    last_text = ""
    for url in urls:
        try:
            ...
            last_status, last_text = None, str(exc)
            continue
        if response.status_code == 404:
            last_status, last_text = 404, response.text
            continue  # wrong path — try the next candidate
        if response.status_code != 200:
            raise AuthError(
                f"Kimi device-code request failed (HTTP {response.status_code}) "
                f"at {url}." + (f" Response: {response.text.strip()}" if response.text else ""),
                provider="kimi-oauth",
                code="kimi_device_code_request_failed",
            )
        payload = response.json()
        if "device_code" not in payload or "user_code" not in payload:
            raise AuthError(
                "Kimi device-code response missing device_code/user_code.",
                provider="kimi-oauth",
                code="kimi_device_code_invalid",
            )
        return payload
    raise AuthError(
...
def _kimi_oauth_poll_device_token(
    client: httpx.Client,
    *,
    device_code: str,
    expires_in: int,
    ...
    while time.monotonic() < deadline:
        ...
        if response.status_code == 200:
            payload = response.json()
            if not _kimi_pick(payload, "access_token", "accessToken", "token"):
                raise AuthError(
                    "Kimi device-code token response did not include an access_token.",
                    provider="kimi-oauth",
                    code="kimi_device_token_invalid",
                )
            return payload
        try:
        ...
        if error_code == "slow_down":
            ...
            continue
        description = (
            error_payload.get("error_description") or error_code or response.text
        )
        raise AuthError(
            f"Kimi device-code authorization failed: {description}",
            provider="kimi-oauth",
            code="kimi_device_token_failed",
        )
    raise AuthError(
        "Timed out waiting for Kimi device authorization.",
        provider="kimi-oauth",
        code="kimi_device_code_timeout",
    )


def kimi_oauth_device_code_login(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 20.0,
) -> Dict[str, Any]:
    """Run the Kimi device-code login end to end and persist the grant.

    Prints the verification URL + user code, optionally opens the browser,
    polls until the user approves, then writes the tokens to the auth store so
    the runtime resolver (and every future session) can use them. Returns the
    post-login auth status dict.
    """
    timeout = httpx.Timeout(max(20.0, timeout_seconds))
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}) as client:
        device = _kimi_oauth_request_device_code(client)
        verification_url = str(
            device.get("verification_uri_complete")
            or device.get("verification_uri")
            or device.get("verificationUrl")
            or ""
        )
        user_code = str(device.get("user_code") or device.get("userCode") or "")
        expires_in = int(device.get("expires_in") or device.get("expiresIn") or 900)
        interval = int(device.get("interval") or 5)

        print()
        print("To log in to Kimi:")
        if verification_url:
        ...
        if (
            ...
            print(
                ...
                if opened
                else "  Could not open browser automatically -- use the URL above."
            )
        print(f"Waiting for approval (polling every {max(1, interval)}s)...")

        payload = _kimi_oauth_poll_device_token(
            client,
            device_code=str(device.get("device_code") or device.get("deviceCode") or ""),
            expires_in=expires_in,
            poll_interval=interval,
        )

    tokens: Dict[str, Any] = {
        "access_token": _kimi_pick(payload, "access_token", "accessToken", "token"),
        "token_type": str(payload.get("token_type") or "Bearer").strip() or "Bearer",
    }
    refresh_token = _kimi_pick(payload, "refresh_token", "refreshToken")
    if refresh_token:
        tokens["refresh_token"] = refresh_token
    expiry = _kimi_expiry_ms(payload)
    if expiry:
...
# =============================================================================
# Gemini OAuth — reuse the Gemini CLI's "Login with Google" (subscription
# login, not an API key). The Gemini CLI performs the browser OAuth and writes
# creds to ~/.gemini/oauth_creds.json. Hercules reads that access token; a
# separate Code Assist transport adapter (agent/gemini_code_assist.py) uses it
# to call cloudcode-pa.googleapis.com. This module only owns the credential
# read/refresh, mirroring the qwen/kimi "reuse the CLI's creds" pattern.
#
# The OAuth client id/secret are the PUBLIC values embedded in the open-source
# gemini-cli (code_assist/oauth2.ts) — they are not secrets. Everything is
# env-overridable; refresh degrades to a clear "re-login via the Gemini CLI"
# message on failure.
# =============================================================================

# The gemini-cli OAuth client id is public (a ...apps.googleusercontent.com id,
# not a secret). The client SECRET is deliberately NOT hardcoded — it is read
# from the credential file the Gemini CLI wrote (~/.gemini/oauth_creds.json
# contains ``client_id`` and ``client_secret``) or from an env override — so no
# credential-looking literal lives in the repo.
GEMINI_OAUTH_DEFAULT_CLIENT_ID = (
...
# gemini-cli's no-browser redirect: Google shows the authorization code on a
# codeassist.google.com page for the user to copy/paste back into the terminal.
GEMINI_OAUTH_NO_BROWSER_REDIRECT_URI = "https://codeassist.google.com/authcode"
GEMINI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

# Where Google publishes the gemini-cli installed-app client secret. Per
# Google's OAuth documentation installed-app secrets are not confidential
# (every gemini-cli install ships the value), but this repo deliberately does
# NOT embed it: secret-scanning tools cannot distinguish a published
# installed-app constant from a leaked credential. The built-in Login with
# Google therefore reads it from GEMINI_OAUTH_CLIENT_SECRET (or a gemini-cli
# creds file), and points the user at the public source when unset.
GEMINI_OAUTH_CLIENT_SECRET_SOURCE_URL = (
    "https://github.com/google-gemini/gemini-cli/blob/main/"
    "packages/core/src/code_assist/oauth2.ts"
)
# Raw view of the same public source file, used by the login-time auto-fetch.
_GEMINI_CLIENT_SECRET_RAW_URL = (
    "https://raw.githubusercontent.com/google-gemini/gemini-cli/main/"
    "packages/core/src/code_assist/oauth2.ts"
)


def _fetch_published_gemini_client_secret(timeout_seconds: float = 15.0) -> str:
    """Fetch gemini-cli's published installed-app client secret from Google's
    open-source repo at login time.

    This keeps the value out of this repository (secret scanners cannot tell a
    published installed-app constant from a leak) while sparing the user a
    manual copy-paste. The result is validated against the GOCSPX installed-app
    shape and cached with the stored grant. Returns "" on any failure — callers
    fall back to an actionable error naming GEMINI_OAUTH_CLIENT_SECRET.
    """
    import re as _re
    ...
    try:
        ...
        if response.status_code != 200:
            return ""
        match = _re.search(
            r"OAUTH_CLIENT_SECRET\s*=\s*['\"]([A-Za-z0-9_-]+)['\"]",
            response.text,
        )
        if not match:
            return ""
        secret = match.group(1).strip()
        # Refuse anything not shaped like a Google installed-app secret rather
        # than forward it to the token endpoint.
        if not _re.fullmatch(r"GOCSPX-[A-Za-z0-9_-]{10,64}", secret):
            return ""
        return secret
    except Exception:
        return ""
...
def _gemini_oauth_client_id(tokens: Optional[Dict[str, Any]] = None) -> str:
    return (
        os.getenv("GEMINI_OAUTH_CLIENT_ID")
        or (str((tokens or {}).get("client_id") or "").strip())
        or GEMINI_OAUTH_DEFAULT_CLIENT_ID
    ).strip()


def _gemini_oauth_client_secret(tokens: Optional[Dict[str, Any]] = None) -> str:
    """Resolve the client secret from env or a creds file (never hardcoded)."""
    return (
        os.getenv("GEMINI_OAUTH_CLIENT_SECRET")
        or str((tokens or {}).get("client_secret") or "").strip()
    ).strip()


def _gemini_oauth_cred_candidates() -> List[Path]:
    override = os.getenv("HERCULES_GEMINI_OAUTH_CREDS_FILE", "").strip()
    paths: List[Path] = []
    if override:
        paths.append(Path(override).expanduser())
    paths.append(Path.home() / ".gemini" / "oauth_creds.json")
    return paths


def _gemini_oauth_cred_path() -> Optional[Path]:
    for p in _gemini_oauth_cred_candidates():
        try:
            if p.exists():
                return p
        except OSError:
...
def _gemini_expiry_ms(data: Dict[str, Any]) -> Optional[int]:
    """Best-effort expiry (epoch ms). gemini-cli stores google-auth-library's
    ``expiry_date`` in epoch ms; OpenCode-style stores ``expires`` in ms; a raw
    ``expires_in`` is seconds-relative."""
    for k in ("expiry_date", "expires", "expires_at", "expiresAt"):
        v = data.get(k)
        if v is None:
            continue
        try:
...
def _read_gemini_oauth_authstore_tokens() -> Optional[Dict[str, Any]]:
    """Tokens from Hercules' own 'Login with Google', if present in the store.

    Tagged ``_source="hercules-login"`` so callers can tell a Hercules-owned
    grant (refreshable + persistable) from a read-only gemini-cli session.
    """
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _save_gemini_oauth_tokens(tokens: Dict[str, Any]) -> None:
    """Persist a Hercules-owned Google login to the auth store.

    Read-modify-write: preserves unrelated state keys (e.g. the onboarded
    Code Assist ``project``)."""
    clean = {k: v for k, v in tokens.items() if k != "_source"}
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _read_gemini_oauth_tokens() -> Dict[str, Any]:
    # Prefer Hercules' own Login-with-Google over a gemini-cli session.
    store_tokens = _read_gemini_oauth_authstore_tokens()
    if store_tokens is not None:
        return store_tokens
    path = _gemini_oauth_cred_path()
    if path is None:
        raise AuthError(
            "Gemini OAuth credentials not found. Run `hercules model` and "
            "choose Gemini to log in with Google, or log in with the Gemini "
            "CLI (`gemini`, 'Login with Google'), then re-run.",
    ...
    try:
        ...
        raise AuthError(
            f"Failed to read Gemini credentials from {path}: {exc}",
            provider="gemini-oauth",
            code="gemini_auth_read_failed",
        ) from exc
    if not isinstance(data, dict):
        raise AuthError(
            f"Invalid Gemini credentials in {path}.",
            provider="gemini-oauth",
...
def _gemini_access_token_is_expiring(
    expiry_ms: Any, skew_seconds: int = GEMINI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
) -> bool:
    try:
        expiry = int(expiry_ms)
    except (TypeError, ValueError):
...
def _refresh_gemini_oauth_tokens(
    ...
    """In-memory refresh via Google's token endpoint (never rewrites the CLI's
    file). Params per gemini-cli / OpenCode gemini-auth: grant_type,
    refresh_token, client_id, client_secret."""
    refresh_token = str(tokens.get("refresh_token", "") or "").strip()
    if not refresh_token:
        raise AuthError(
            "Gemini OAuth token expired and no refresh token is available. "
            "Re-login with the Gemini CLI ('Login with Google').",
            provider="gemini-oauth",
            code="gemini_refresh_token_missing",
        )
    client_secret = _gemini_oauth_client_secret(tokens)
    if not client_secret:
        raise AuthError(
            "Gemini OAuth client secret not found in the Gemini CLI credentials "
            "(~/.gemini/oauth_creds.json). Re-login with the Gemini CLI or set "
            "GEMINI_OAUTH_CLIENT_SECRET.",
            provider="gemini-oauth",
            code="gemini_client_secret_missing",
        )
    try:
        response = httpx.post(
            ...
            data={
                "grant_type": "refresh_token",
                "refresh_token": refresh_token,
                "client_id": _gemini_oauth_client_id(tokens),
        ...
        raise AuthError(
            f"Gemini OAuth refresh failed: {exc}. Re-login with the Gemini CLI.",
            provider="gemini-oauth",
            code="gemini_refresh_failed",
        ) from exc
    if response.status_code >= 400:
        raise AuthError(
            "Gemini OAuth refresh failed. Re-login with the Gemini CLI ('Login with Google').",
            provider="gemini-oauth",
            code="gemini_refresh_failed",
        )
    try:
        ...
        raise AuthError(
            f"Gemini OAuth refresh returned invalid JSON: {exc}",
            provider="gemini-oauth",
    ...
    if not access:
        raise AuthError(
            "Gemini OAuth refresh response missing access_token.",
            provider="gemini-oauth",
    ...
    if payload.get("refresh_token"):
        refreshed["refresh_token"] = str(payload["refresh_token"]).strip()
    expires_in = payload.get("expires_in")
    try:
        refreshed["expiry_date"] = int(time.time() * 1000) + max(1, int(float(expires_in))) * 1000
    except (TypeError, ValueError):
        pass
    return refreshed


def _mark_gemini_oauth_active(creds: Dict[str, Any]) -> None:
    with _auth_store_lock():
        auth_store = _load_auth_store()
        # Read-modify-write: replacing the state outright would wipe a stored
        # Hercules-owned login (state["tokens"]) just for marking a project.
        state = _load_provider_state(auth_store, "gemini-oauth") or {}
        if creds.get("project"):
            state["project"] = str(creds["project"])
        _save_provider_state(auth_store, "gemini-oauth", state)
        _save_auth_store(auth_store)
...
def resolve_gemini_oauth_runtime_credentials(
    *,
    force_refresh: bool = False,
    refresh_if_expiring: bool = True,
    refresh_skew_seconds: int = GEMINI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
) -> Dict[str, Any]:
    tokens = _read_gemini_oauth_tokens()
    owns_login = tokens.pop("_source", "") == "hercules-login"
    access_token = str(tokens.get("access_token", "") or "").strip()
    should_refresh = bool(force_refresh)
    if not should_refresh and refresh_if_expiring and access_token:
        should_refresh = _gemini_access_token_is_expiring(
            _gemini_expiry_ms(tokens), refresh_skew_seconds
        )
    if should_refresh:
        tokens = _refresh_gemini_oauth_tokens(tokens)
        access_token = str(tokens.get("access_token", "") or "").strip()
        # Persist the rotated grant for a Hercules-owned login; a gemini-cli
        # session stays read-only (the CLI refreshes its own file).
        if owns_login:
            _save_gemini_oauth_tokens(tokens)
    if not access_token:
        raise AuthError(
            "Gemini OAuth access token missing. Re-login via `hercules model` "
            "(choose Gemini) or the Gemini CLI ('Login with Google').",
            provider="gemini-oauth",
            code="gemini_access_token_missing",
        )
    return {
        "provider": "gemini-oauth",
        # The token is consumed by the Code Assist transport (cloudcode-pa),
        # not a base_url the OpenAI SDK hits directly.
        "base_url": "https://cloudcode-pa.googleapis.com/v1internal",
        "api_key": access_token,
        "source": "google-login" if owns_login else "gemini-cli",
        "expires_at_ms": _gemini_expiry_ms(tokens),
        "auth_file": "" if owns_login else str(_gemini_oauth_cred_path() or ""),
    }
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    path = _gemini_oauth_cred_path()
    try:
        creds = resolve_gemini_oauth_runtime_credentials(refresh_if_expiring=True)
        return {
            "logged_in": True,
            "auth_file": str(path or ""),
            "source": creds.get("source"),
        ...
        return {
            "logged_in": False,
            "auth_file": str(path or _gemini_oauth_cred_candidates()[0]),
            "error": str(exc),
        }


# -----------------------------------------------------------------------------
# Gemini — built-in "Login with Google" (no gemini-cli install required)
#
# Authorization-code flow with PKCE against Google's OAuth endpoints, using the
# gemini-cli installed-app client (published constants). Two variants,
# mirroring gemini-cli:
#   * loopback: local http://127.0.0.1:<port>/oauth2callback redirect, browser
#     opens, code is captured automatically;
#   * paste: redirect to codeassist.google.com/authcode — Google displays the
#     code and the user pastes it into the terminal (works over SSH).
# Tokens persist to Hercules' auth store; the reader above prefers them over a
# gemini-cli session, and refresh writes rotated grants back.
# -----------------------------------------------------------------------------


def _gemini_google_authorize_url(
    *, redirect_uri: str, code_challenge: str, state: str
    ...
    params = {
        ...
        "state": state,
        # Required for a refresh token on every login.
        "access_type": "offline",
        "prompt": "consent",
    }
...
def _gemini_google_exchange_code(
    *, code: str, redirect_uri: str, code_verifier: str,
    client_secret: str = "", timeout_seconds: float = 30.0
) -> Dict[str, Any]:
    try:
        response = httpx.post(
            GEMINI_OAUTH_TOKEN_URL,
            ...
            data={
                ...
                "code": code,
                "client_id": _gemini_oauth_client_id(),
                "client_secret": client_secret or _gemini_oauth_client_secret(),
                "redirect_uri": redirect_uri,
        ...
        raise AuthError(
            ...
            provider="gemini-oauth",
            code="gemini_token_exchange_failed",
        ) from exc
    if response.status_code >= 400:
        raise AuthError(
    ...
    if not isinstance(payload, dict) or not payload.get("access_token"):
        raise AuthError(
            ...
            provider="gemini-oauth",
            code="gemini_token_exchange_invalid",
        )
    return payload


def _gemini_google_loopback_login(authorize_url_for: Callable[[str], str],
                                  timeout_seconds: float) -> Dict[str, str]:
    """Run the loopback leg: local server, browser, wait for the callback.
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 (BaseHTTPRequestHandler API)
            ...
            self.wfile.write(
                ...
                b"<p>You can close this tab and return to Hercules.</p>"
                b"</body></html>"
            )
            done.set()
    ...
    thread.start()
    try:
        url = authorize_url_for(redirect_uri)
        print()
        ...
        try:
            ...
            opened = False
        print(
            "  (Opened your browser)" if opened
            else "  Could not open a browser automatically -- use the URL above."
        )
        print("Waiting for Google sign-in to complete...")
        if not done.wait(timeout=max(30.0, timeout_seconds)):
            raise AuthError(
                "Timed out waiting for the Google sign-in redirect.",
                provider="gemini-oauth",
                code="gemini_login_timeout",
            )
    finally:
        server.shutdown()
        server.server_close()
    if result.get("error"):
        raise AuthError(
            f"Google sign-in was denied: {result['error']}",
            provider="gemini-oauth",
            code="gemini_login_denied",
        )
    return {
...
def gemini_oauth_google_login(
    *,
    use_loopback: Optional[bool] = None,
    timeout_seconds: float = 300.0,
) -> Dict[str, Any]:
    """Built-in 'Login with Google' for the gemini-oauth provider.

    Persists the grant (including client id/secret for refresh) to Hercules'
    auth store and returns the post-login auth status.
    """
    # The token exchange needs the installed-app client secret, which this
    # repo deliberately does not embed. Resolution order: env / stored creds,
    # then an automatic fetch of Google's published constant from the public
    # gemini-cli source — so the normal path needs no manual setup at all.
    client_secret = _gemini_oauth_client_secret()
    if not client_secret:
        print("Fetching Google's published gemini-cli OAuth constants...")
        client_secret = _fetch_published_gemini_client_secret()
    if not client_secret:
        raise AuthError(
            "Could not auto-fetch the Google OAuth client secret (network "
            "blocked?). One-time manual fallback: copy OAUTH_CLIENT_SECRET "
            f"from Google's public gemini-cli source "
            f"({GEMINI_OAUTH_CLIENT_SECRET_SOURCE_URL}) into ~/.hercules/.env "
            "as GEMINI_OAUTH_CLIENT_SECRET=<value>, then re-run. "
            "(Installed-app secrets are published by Google and ship inside "
            "every gemini-cli install; Hercules just doesn't hardcode them.) "
            "Alternatively, log in once with the Gemini CLI (`gemini`).",
            provider="gemini-oauth",
            code="gemini_client_secret_missing",
        )
    ...
    if use_loopback is None:
        use_loopback = (not _is_remote_session()) and _can_open_graphical_browser()

    if use_loopback:
        leg = _gemini_google_loopback_login(
            lambda redirect_uri: _gemini_google_authorize_url(
                redirect_uri=redirect_uri,
                code_challenge=code_challenge,
                state=state_token,
            ),
        ...
        code, returned_state, redirect_uri = leg["code"], leg["state"], leg["redirect_uri"]
        if returned_state != state_token:
            raise AuthError(
                "Google sign-in state mismatch (possible interception); aborting.",
                provider="gemini-oauth",
                code="gemini_login_state_mismatch",
            )
    else:
        redirect_uri = GEMINI_OAUTH_NO_BROWSER_REDIRECT_URI
        url = _gemini_google_authorize_url(
            redirect_uri=redirect_uri,
            code_challenge=code_challenge,
            state=state_token,
        )
        print()
        print("Log in with Google (no-browser mode):")
        print(f"  1. Open this URL on any device: {url}")
        print("  2. Sign in; Google will display an authorization code.")
        try:
    ...
    if not code:
        raise AuthError(
            ...
            provider="gemini-oauth",
            code="gemini_login_no_code",
        )

    payload = _gemini_google_exchange_code(
    ...
    tokens: Dict[str, Any] = {
        "access_token": str(payload.get("access_token") or "").strip(),
        "token_type": str(payload.get("token_type") or "Bearer").strip() or "Bearer",
        # Stored so the existing refresh helper resolves them from the token
        # dict without env configuration (includes an auto-fetched secret).
        "client_id": _gemini_oauth_client_id(),
        "client_secret": client_secret,
    }
    if payload.get("refresh_token"):
        tokens["refresh_token"] = str(payload["refresh_token"]).strip()
    if payload.get("id_token"):
        tokens["id_token"] = str(payload["id_token"]).strip()
    if payload.get("scope"):
        tokens["scope"] = str(payload["scope"]).strip()
    expiry = _gemini_expiry_ms(payload)
    if expiry:
        tokens["expiry_date"] = expiry
    _save_gemini_oauth_tokens(tokens)
