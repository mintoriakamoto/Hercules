"""Helpers for loading Hercules .env files consistently across entrypoints."""
...
# load_hercules_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_hercules_dotenv().
# Used by setup / `hercules model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# HERCULES_HOME paths we've already pulled external secrets for during this
# process.  ``load_hercules_dotenv()`` is called at module-import time from
# several hot modules (cli.py, hercules_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_hercules_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which HERCULES_HOME paths have already had external secrets applied.
...
def format_secret_source_suffix(env_var: str) -> str:
    ...
    except Exception:  # label lookup must never raise
        pass
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `hercules setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``hercules_cli.config._sanitize_env_lines`` which
    already knows all valid Hercules env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from hercules_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hercules_dotenv(
    *,
    hercules_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Hercules environment files with user config taking precedence.

    Behavior:
    - `~/.hercules/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hercules_home or os.getenv("HERCULES_HOME", Path.home() / ".hercules"))
    ...
    #   EnvironmentFile=-/path/to/.hercules/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of HERCULES_HOME / profile). v1
    ...
    except Exception:  # managed scope must never block startup
        return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Hercules reads
    ...
    owns the once-per-HERCULES_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_hercules_dotenv()`` runs at import
    time from several hot modules (cli.py, hercules_cli/main.py,
    ...
    except Exception:  # config errors must not block startup
        return
    if not cfg:
        return
    ...
    except Exception:  # belt-and-braces; apply_all shouldn't raise
        return
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `hercules model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    if not config_path.exists():
        return {}
    try:
        import yaml  # type: ignore  # noqa: F401 — availability probe
    except ImportError:
        return {}
    try:
