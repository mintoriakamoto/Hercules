...
_DOCS_BASE = "https://github.com/mintoriakamoto/Hercules/tree/main/website/docs"
...
# display_hercules_home imported lazily at call sites (stale-module safety during hercules update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Hercules using environment variables or config commands:")
    print_info("  hercules config set model.provider custom")
    print_info("  hercules config set model.base_url http://localhost:8080/v1")
...
def is_noninteractive() -> bool:
    ...
    ``HERCULES_NONINTERACTIVE=1`` (see ``hercules_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("HERCULES_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``HERCULES_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from hercules_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'hercules setup')")


def _print_setup_summary(config: dict, hercules_home):
    ...
    if get_env_value("BROWSERBASE_API_KEY") and get_env_value("BROWSERBASE_PROJECT_ID"):
        ...
        browser_provider = "Local browser"
    ...
    # Video generation — opt-in via `hercules tools` → Video Generation.
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {hercules_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print(
        f"   {color('hercules config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('hercules config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('hercules', Colors.GREEN)}              Start chatting")
    print(f"   {color('hercules gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('hercules doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `hercules tools` and `hercules setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``hercules model``)
    ...
    provider added to ``hercules model`` is automatically available here.
    ...
    try:
        ...
        print_info("You can try again later with: hercules model")
    ...
    # on demand via `hercules auth add`, `hercules setup` vision, and
    # `hercules setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``hercules model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Hercules auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'hercules setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `hercules setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `hercules setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into HERCULES_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("HERCULES_MAX_ITERATIONS")
    ...
    print_info("  Run `hercules setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HERCULES_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from hercules_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_hercules_home(Path(get_hercules_home()))
    except Exception:
        pass
...
def _profile_name_from_hercules_home(hercules_home) -> str | None:
    """Return the active profile name when HERCULES_HOME is a profile dir."""
    if hercules_home.parent.name == "profiles":
        return hercules_home.name
    return None
...
def _setup_bluebubbles():
    ...
    print_info("Connects Hercules to iMessage via BlueBubbles — a free, open-source")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     hercules config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_hercules_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   hercules gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `hercules setup tools` and `hercules tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``hercules_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from hercules_cli.config
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_hercules.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_hercules migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_hercules", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Hercules.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Hercules to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Hercules at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Hercules at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Hercules at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Hercules at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Hercules equivalents",
...
def _offer_openclaw_migration(hercules_home: Path) -> bool:
    ...
    print_info("Hercules can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: hercules claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=hercules_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: hercules claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Hercules configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Hercules config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      hercules setup           — full or quick (auto-detected)
      hercules setup model     — just model/provider
      hercules setup tts       — just text-to-speech
      hercules setup terminal  — just terminal backend
      hercules setup gateway   — just messaging platforms
      hercules setup tools     — just tool configuration
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, hercules_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Hercules configured.")
        ...
        migration_ran = _offer_openclaw_migration(hercules_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Hercules?",
            [
        ...
        if setup_mode == 1:
            _run_blank_slate_setup(config, hercules_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {hercules_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'hercules config edit'")
    ...
    # Tune later with `hercules setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, hercules_home)
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``hercules tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``hercules setup agent`` /
    ``hercules config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hercules_home, is_existing: bool):
    ...
    if path == 0:
        ...
        print_info("  Enable tools:        hercules tools")
        print_info("  Seed skills:         hercules skills opt-in --sync")
        print_info("  Add MCP servers:     hercules mcp add")
        print_info("  Enable plugins:      hercules plugins")
        print_info("  Tune agent settings: hercules setup agent")
        print()
        _print_setup_summary(config, hercules_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, hercules_home)


def _blank_slate_walkthrough(config: dict, hercules_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`hercules update` runs from re-injecting them. Opt back in any")
            print_info("time with `hercules skills opt-in --sync`.")
    except Exception as exc:
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   hercules tools")
    print_info("  Seed skills:         hercules skills opt-in --sync")
    print_info("  Add MCP servers:     hercules mcp add")
    print_info("  Tune agent settings: hercules setup agent")
    print()
...
def _run_quick_setup(config: dict, hercules_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'hercules setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Hercules to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'hercules setup gateway'.")
