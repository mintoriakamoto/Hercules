...
2. **Hercules overlays** — transport type, auth patterns, aggregator flags,
...
# -- Hercules overlay ----------------------------------------------------------
# Hercules-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HerculesOverlay:
    """Hercules-specific provider metadata layered on top of models.dev."""
...
HERCULES_OVERLAYS: Dict[str, HerculesOverlay] = {
    "moa": HerculesOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": HerculesOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-api": HerculesOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": HerculesOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": HerculesOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HERCULES_QWEN_BASE_URL",
    ),
    "kimi-oauth": HerculesOverlay(
        # /coding speaks the Anthropic Messages protocol (unlike legacy
        # platform.moonshot.ai keys, which is why kimi-for-coding stays
        # openai_chat). Mirrors minimax-oauth.
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.kimi.com/coding",
        base_url_env_var="KIMI_OAUTH_BASE_URL",
    ),
    "gemini-oauth": HerculesOverlay(
        # Runs through the OpenAI chat_completions surface, but agent_init swaps
        # in a translating http client (Code Assist). base_url is a placeholder
        # the translating transport rewrites to cloudcode-pa.
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://cloudcode-pa.googleapis.com/v1internal",
    ),
    "lmstudio": HerculesOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": HerculesOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": HerculesOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": HerculesOverlay(
        transport="anthropic_messages",
    ...
    "kimi-for-coding": HerculesOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": HerculesOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": HerculesOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": HerculesOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": HerculesOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": HerculesOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": HerculesOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": HerculesOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": HerculesOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": HerculesOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": HerculesOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": HerculesOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": HerculesOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": HerculesOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": HerculesOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": HerculesOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": HerculesOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": HerculesOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": HerculesOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "hercules", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    overlay = HERCULES_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + hercules extra
    ...
    if overlay is not None:
        # Hercules-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in HERCULES_OVERLAYS
    if provider == "bedrock":
