...
Usage:
    python -m hercules_cli.main web          # Start on http://127.0.0.1:9119
    python -m hercules_cli.main web --port 8080
"""
...
from hercules_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `hercules dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    desktop app spawns a ``hercules dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same HERCULES_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import hercules_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire hercules_cli.gateway import into a background thread so the event
    # loop is not blocked and HERCULES_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (HERCULES_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hercules
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("HERCULES_DESKTOP") == "1":
...
# HERCULES_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
#
# This list is defined in ``hercules_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from hercules_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Hercules-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``hercules-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from hercules_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Hercules (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during hercules update. "
...
class ModelAssignment(BaseModel):
    ...
    # ``hercules model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Hercules provider slug — fine.
    ...
    1. Vendor-name → Hercules-provider mapping: when the provider string is
       not a known Hercules provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    from hercules_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from hercules_cli.model_normalize import normalize_model_for_provider
    from hercules_cli.providers import resolve_custom_provider, resolve_user_provider
...
def _count_status_active_sessions() -> int:
    ...
    while another Hercules process is writing to it.
    """
    from hercules_state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "HERCULES_DASHBOARD_FILES_ROOT"
...
# HERCULES_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to HERCULES_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Hercules credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no HERCULES_HOME resolution, so it blocks these trees wherever
...
def _default_hercules_root_is_opt_data() -> bool:
    raw = os.environ.get("HERCULES_HOME", "").strip()
    if not raw:
        return False
    try:
        from hercules_constants import get_default_hercules_root

        root = get_default_hercules_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``hercules update``.
    ...
    a container — e.g. the hercules-webui image sharing the Hercules source tree),
    the dashboard's ``hercules update`` button is the correct update path and
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Hercules inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``hercules:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.hercules-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``hercules_cli.web_git``;
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_hercules_home() at call time
    ...
    try:
        ...
        # Windows install the first import of hercules_cli.gateway blocks the
        ...
        status = {
            ...
            "can_update_hercules": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Hercules Cloud
        ...
        if not auth_required:
            status.update({
                "hercules_home": str(get_hercules_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the hercules home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_hercules_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
# streamed to a per-action log file under ``~/.hercules/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_hercules_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "hercules-update": "hercules-update.log",
...
def _spawn_hercules_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``hercules <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``hercules_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "hercules_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "HERCULES_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["hercules", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    concurrent ``hercules gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``hercules gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``HERCULES_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/hercules/update")
async def update_hercules():
    """Kick off ``hercules update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Hercules updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("hercules-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "hercules-update",
...
@app.get("/api/hercules/update/check")
async def check_hercules_update(force: bool = False):
    """Report whether a Hercules update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hercules/update`` actually runs ``hercules update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Hercules version string
    ...
    try:
        ...
        if force:
            try:
                (get_hercules_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.hercules/config.yaml`` under ``tts.``.
    """
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Hercules supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        hercules_home / f"{name}.json",
        hercules_home / name / "config.json",
    ):
    ...
    # Holographic stores under plugins.hercules-memory-store.
    plugins_cfg = cfg.get("plugins") if isinstance(cfg, dict) else {}
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("hercules-memory-store")
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_hercules_home()))
            return
...
# in hercules_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.hercules/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``hercules model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from hercules_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``hercules model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        from hercules_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `hercules model` provider — would be
        ...
        # Vertex — a `hercules model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `hercules model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Hercules from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Hercules to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Hercules from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Hercules to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Hercules in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Hercules through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Hercules via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Hercules through an IMAP/SMTP mailbox.",
        "docs_url": "https://github.com/mintoriakamoto/Hercules/tree/main/website/docs/user-guide/messaging",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Hercules to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Hercules inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Hercules to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/messaging/google_chat.md",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/messaging/weixin.md",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Hercules through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Hercules to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/messaging/teams.md",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Hercules to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Hercules as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://github.com/mintoriakamoto/Hercules/tree/main/website/docs/user-guide/messaging",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/messaging/webhooks.md",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in hercules gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in hercules gateway setup",
...
def _whatsapp_session_path() -> Path:
    ...
    return get_hercules_dir("platforms/whatsapp/session", "whatsapp/session")
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Hercules Agent").strip() or "Hercules Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Hercules appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's HERCULES_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hercules/.anthropic_oauth.json`` — Hercules-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_hercules_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _get_hercules_oauth_file,
        )
    except ImportError:
        read_hercules_oauth_credentials = None  # type: ignore
        _get_hercules_oauth_file = None  # type: ignore

    hercules_creds = None
    if read_hercules_oauth_credentials:
        try:
            hercules_creds = read_hercules_oauth_credentials()
        except Exception:
            hercules_creds = None
    if hercules_creds and hercules_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hercules_pkce",
            "source_label": f"Hercules PKCE ({_get_hercules_oauth_file() if _get_hercules_oauth_file else None})",
            "token_preview": _truncate_token(hercules_creds.get("accessToken")),
            "expires_at": hercules_creds.get("expiresAt"),
            "has_refresh_token": bool(hercules_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Hercules even
    when they also have a separate Hercules-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Hercules never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "device_code",
        "cli_command": "hercules auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "hercules auth add qwen-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "hercules auth add xai-oauth",
        "docs_url": "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/guides/xai-grok-oauth.md",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "hercules auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Hercules, so the disconnect
    ...
    user sees exactly what executes, and Hercules then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Hercules — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``hercules model`` universe) — so any OAuth/external provider added as a
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"hercules auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("hercules_pkce", "claude_code", ...)
    ...
    sync with the `hercules model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Hercules-managed PKCE file and auth-store entry.
...
# Guarded so hercules web still starts if anthropic_adapter is unavailable;
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "hercules-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from hercules_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``hercules auth add minimax-oauth``.
    """
    from hercules_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``hercules auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``hercules model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Hercules "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from hercules_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `hercules sessions stats`).
...
async def get_logs(
    ...
    from hercules_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_hercules_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from hercules_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, HERCULES_HOME)."""
    from hercules_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["hercules_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    token = set_hercules_home_override(str(home))
    try:
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Hercules home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (hercules_cli.mcp_config), so
# servers managed here show up under `hercules mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from hercules_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from hercules_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from hercules_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from hercules_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from hercules_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the HERCULES_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``hercules mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        from tools.mcp_oauth import HerculesTokenStorage, force_interactive_oauth
        ...
        # resolve HERCULES_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = HerculesTokenStorage(name)
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `hercules mcp catalog` / `hercules mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds HERCULES_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (hercules_cli.webhook); the webhook
...
async def list_webhooks():
    import hercules_cli.webhook as wh
...
# spawn the real `hercules gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from hercules_cli.auth import read_credential_pool
...
def _dashboard_backup_dir() -> Path:
    return get_hercules_home() / "backups"
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `hercules import`. The spawned action runs with
...
async def list_checkpoints():
    ...
    # Checkpoints live under <hercules_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_hercules_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``hercules`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_hercules_action`` tracks one process/log per name, so a shared
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from hercules_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`hercules -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the HERCULES_HOME
...
def _profile_setup_command(name: str) -> str:
    ...
    return "hercules setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local HERCULES_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local HERCULES_HOME override (same mechanism as
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's HERCULES_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``hercules profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``hercules profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the HERCULES_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``hercules profile describe
    <name> --auto``.
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_hercules_home()`` at call
       time — the context-local override from ``set_hercules_home_override``
    ...
    retargeted to the *current* ``get_hercules_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a HERCULES_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_hercules_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_hercules_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from hercules_cli.skills_config import get_disabled_skills
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``hercules tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``hercules tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `hercules tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``hercules tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``hercules tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``hercules tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``hercules -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same HERCULES_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Hercules,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``hercules computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``hercules --tui`` binary the CLI uses, behind
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from hercules_cli.dashboard_auth.audit import AuditEvent, audit_log
        from hercules_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``hercules --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HERCULES_TUI_RESUME`` env var —
    matching what ``hercules_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``HERCULES_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``HERCULES_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``HERCULES_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``HERCULES_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_hercules_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``hercules -p <name>`` performs. The in-process
    ``HERCULES_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from hercules_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("HERCULES_TUI_DISABLE_MOUSE", "1")
    env.setdefault("HERCULES_TUI_INLINE", "1")
    ...
    env["HERCULES_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["HERCULES_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["HERCULES_TUI_RESUME"] = resume

    if sidecar_url:
        env["HERCULES_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile HERCULES_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["HERCULES_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``HERCULES_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("HERCULES_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="hercules-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Hercules Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Hercules CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="hercules-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_hercules_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from hercules_cli.console_engine import HerculesConsoleEngine
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Hercules Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Hercules inside WSL2 to use the dashboard's /chat "
...
# HERCULES_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`hercules_cli.dashboard_auth.prefix.normalise_prefix`
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/hercules/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /hercules`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__HERCULES_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `hercules serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("HERCULES_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (hercules serve): web UI disabled — use "
            "`hercules dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/hercules``)
        ...
        ``__HERCULES_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__HERCULES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HERCULES_BASE_PATH__="{prefix}";'
                f"window.__HERCULES_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__HERCULES_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__HERCULES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HERCULES_BASE_PATH__="{prefix}";'
                f"window.__HERCULES_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/hercules`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Hercules backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.hercules/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.hercules/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_hercules_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.hercules/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as hercules_cli.plugins):
    1. User plugins:    ~/.hercules/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hercules/plugins/  (only if HERCULES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # ``hercules_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("HERCULES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".hercules" / "plugins", "project"))
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"hercules auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from hercules_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from hercules_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hercules_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hercules_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.hercules/plugins/``) ship with the CWD and are
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``HERCULES_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("HERCULES_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``HERCULES_SERVE_HEADLESS``), so
    ...
    # the hercules-0day MCP-persistence campaign abused unauthenticated public
    ...
    # HERCULES_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Hercules backend listening on {host}:{actual_port}")
            else:
                print(f"  Hercules Web UI → http://{host}:{actual_port}")
