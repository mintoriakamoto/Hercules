...
Config files are stored in ~/.hercules/ for easy access:
- ~/.hercules/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.hercules/.env         - API keys and secrets

This module provides:
- hercules config          - Show current configuration
- hercules config edit     - Open config in editor
- hercules config set      - Set a specific value
- hercules config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``hercules config set`` (which rewrites
    ...
    ``config.yaml`` in place: hercules never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.hercules/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``hercules logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  hercules config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Hercules makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Hercules itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``hercules update``, the TUI build.
...
#   on every plugin install / ``hercules update``.
...
# * ``HERCULES_HOME`` / ``HERCULES_PROFILE`` / ``HERCULES_CONFIG`` /
#   ``HERCULES_ENV`` — Hercules runtime location flags. Writing these into
...
#
# IMPORTANT: ``HERCULES_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (HERCULES_LANGFUSE_PUBLIC_KEY,
# HERCULES_SPOTIFY_CLIENT_ID, ...). The
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Hercules runtime location "
            "(HERCULES_HOME, HERCULES_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # observability/langfuse` or `hercules tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "HERCULES_LANGFUSE_ENV",
    "HERCULES_LANGFUSE_RELEASE",
    "HERCULES_LANGFUSE_SAMPLE_RATE",
    "HERCULES_LANGFUSE_MAX_CHARS",
    "HERCULES_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("HERCULES_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_hercules_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Hercules is running in package-manager-managed mode.

    Two signals: the HERCULES_MANAGED env var (set by the systemd service),
    or a .managed marker file in HERCULES_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``hercules_cli/`` — i.e. the git checkout for source
    installs, ``/opt/hercules`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$HERCULES_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Hercules was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$HERCULES_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. HERCULES_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.hercules``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$HERCULES_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.hercules:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$HERCULES_HOME``, a
    ...
    ``docker`` and ``hercules update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $HERCULES_HOME.
    try:
    ...
    #    host install whose shared $HERCULES_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``hercules update``.
    try:
        method = (
            (get_hercules_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``hercules_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$HERCULES_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/hercules`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Hercules lives in a ``uv tool`` layout.

    ``uv tool install hercules-agent`` places the install at
    ``.../uv/tools/hercules-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``hercules-agent`` happens to be uv-tool-installed on the machine while
    the *active* Hercules is a regular pip/venv install, causing
...
PLATFORM_SUPPORT_DOCS_URL = "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/getting-started/platform-support.md"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``hercules update`` / ``hercules update --check`` so the wording — and the
...
# Long-form text for ``hercules update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# hercules_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Hercules, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``hercules update`` inside Docker.
...
def format_managed_message(action: str = "modify this Hercules installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("HERCULES_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hercules installation is managed by NixOS "
            f"(HERCULES_MANAGED={env_hint}).\n"
            "Edit services.hercules-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Hercules installation is managed by Homebrew "
            f"(HERCULES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade hercules-agent"
        )

    return (
        f"Cannot {action}: this Hercules installation is managed by {managed_system}.\n"
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, hercules_bin
    or None if container mode is not active, we're already inside the
    container, or HERCULES_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("HERCULES_DEV") == "1":
        return None
    ...
    container_mode_file = get_hercules_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "hercules-agent")
    exec_user = info.get("exec_user", "hercules")
    hercules_bin = info.get("hercules_bin", "/data/current-package/bin/hercules")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "hercules_bin": hercules_bin,
    }
...
def _resolve_hercules_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Hercules commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level HERCULES_HOME once, but
    subdirectories created at runtime by ``ensure_hercules_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("HERCULES_UID", "").strip()
    gid_str = os.environ.get("HERCULES_GID", "").strip()
    try:
...
def _chown_to_hercules_uid(path) -> None:
    """Chown ``path`` to ``HERCULES_UID:HERCULES_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_hercules_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_hercules_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the hercules group can
    share state with the gateway service.

    The mode can be overridden via the HERCULES_HOME_MODE environment variable
    (e.g. HERCULES_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HERCULES_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``HERCULES_UID``/``HERCULES_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("HERCULES_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_hercules_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Hercules runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("HERCULES_CONTAINER") or os.environ.get("HERCULES_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set HERCULES_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HERCULES_HOME, upgrading legacy empty templates.
...
def ensure_hercules_home():
    """Ensure ~/.hercules directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``hercules profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old HERCULES_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_hercules_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_hercules_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HERCULES_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Hercules-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Hercules
        ...
        # identity slot (SOUL.md). Empty by default. The HERCULES_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Hercules adds a coding operating brief
    ...
    "terminal": {
        ...
        #             HERCULES_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force HERCULES_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Hercules auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Hercules sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Hercules
        ...
        # bundled Hercules image, which drops to the `hercules` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Hercules sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Hercules should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``hercules chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.hercules/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: hercules sweeps the checkpoint base at startup
    ...
    # See tools/kanban_tools.py and hercules_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``hercules dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``hercules kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``hercules kanban decompose`` and the kanban
        ...
        # ``hercules profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `hercules model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hercules` (and `hercules chat`) launches by default:
        ...
        # REPL and `--tui` (or HERCULES_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `hercules --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hercules -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERCULES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `hercules --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to HERCULES_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/hercules`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `hercules pets`. Disabled until
        ...
        "pet": {
            ...
            # get_hercules_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # (``HERCULES_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``HERCULES_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``HERCULES_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/hercules``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Hercules appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.hercules/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.hercules/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.hercules/cache/delegation/
    ...
    # response. If not, Hercules feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Hercules auto-pauses the goal and
    ...
    "moa": {
        ...
        # <hercules_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # cron daemon.
    #
    # See `hercules curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `hercules curator run --consolidate` overrides
        ...
        # normally restored on every `hercules update`, so pruning them only
        ...
        # skipped), snapshot ~/.hercules/skills/ into
        # ~/.hercules/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `hercules curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (HERCULES_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `hercules doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hercules_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Hercules to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # ticker (default), which needs no external service. Name an installed
        # provider (plugins/cron_providers/<name>/ or $HERCULES_HOME/plugins/<name>/)
        # to relocate the trigger to a self-hosted managed scheduler. An unknown
        # or unavailable provider falls back to the built-in, so cron never
        # loses its trigger.
        "provider": "",
        # Inbound cron-fire webhook auth (cron.fire.*). Lets an EXTERNAL
        # scheduler (a cloud scheduler, a self-hosted timer, cron-job.org, …)
        # trigger a due job by POSTing {your_agent_url}/api/cron/fire with a
        # short-lived signed JWT. All non-secret: the agent holds only the
        # PUBLIC verification key, never a signing key. Leave empty to keep the
        # endpoint closed (it refuses all tokens); the built-in ticker fires
        # jobs on schedule without any of this.
        "fire": {
            # This agent's expected JWT audience (e.g. "agent:{instance_id}").
            "audience": "",
            # JWKS URL (or inline PEM public key) used to verify the inbound
            # fire JWT's signature. Empty → the fire endpoint refuses all
            # tokens (no unsigned decode — this is a security boundary).
            "jwks_url": "",
            # Expected token issuer (optional). When set, a fire JWT whose
            # `iss` claim doesn't match is rejected.
            "issuer": "",
        },
        ...
        # Also overridable via HERCULES_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `hercules -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `hercules` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `hercules kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     hercules-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Hercules tools (terminal, read_file, write_file, patch,
    ...
    # update model picker lists without shipping a hercules-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://mintoriakamoto.github.io/Hercules/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `hercules model` invocation; network failures
    ...
    "gateway": {
        ...
        # `launchctl kickstart ai.hercules.gateway` that defenses 1-2 don't
        ...
        # behavior: files must live under the Hercules cache, under
        ...
        # user. Bridged to HERCULES_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Hercules
        # cache (~/.hercules/cache/{documents,images,audio,video,screenshots})
        ...
        # to HERCULES_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # HERCULES_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
    ...
    "sessions": {
        ...
        # default of ``hercules sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.hercules/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        ...
        # ``hercules update``.  Backups land in ``<HERCULES_HOME>/backups/`` and
        # can be restored with ``hercules import <path>``.  Off by default:
        # zipping a large HERCULES_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``hercules update
        ...
        "backup_keep": 5,
        # What `hercules update` does with uncommitted local changes to the
    ...
    # `hercules tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.hercules/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.hercules/.env (or your shell) and never in config.yaml.
            ...
            # ~/.hercules/bin/ on first use.  When False you must install
            ...
            # `hercules secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.hercules/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Hercules disables it for our users unless they
        # explicitly opt in here. When false (default), Hercules sets
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'hercules model' for endpoint-specific config)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Hercules, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @hercules:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. HERCULES_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #hercules)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: hercules-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'hercules-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Hercules API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Hercules API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Hercules API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # HERCULES_TOOL_PROGRESS and HERCULES_TOOL_PROGRESS_MODE are deprecated —
    ...
    "HERCULES_PREFILL_MESSAGES_FILE": {
    ...
    "HERCULES_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.hercules.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Hercules' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Hercules versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``hercules_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'hercules doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERCULES_HOME", "~/.hercules")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "hercules update / hercules -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("HERCULES_TOOL_PROGRESS")
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$HERCULES_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_hercules_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `hercules plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `hercules curator status` has no
    ...
    #   3. Creates `~/.hercules/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hercules_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_hercules_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `hercules config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `hercules config set`"
                    )
    ...
    # behavior set it to true explicitly via `hercules config set`.
    ...
    try:
        ...
        from hercules_cli.toolset_validation import validate_platform_toolsets
    ...
    # `hercules update`, but only the version bump is persisted.
    ...
    # metadata.hercules.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_hercules_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``hercules config set``
    ...
    backends) — while display paths (``hercules status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _hercules_home / "config.yaml"`` local variable
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.hercules/config.yaml.
    ...
    (which change ``HERCULES_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/hercules/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_hercules_dotenv()
...
#   openai-codex (OAuth — hercules auth) — OpenAI Codex
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.hercules/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Hercules writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.hercules/.env.
    ...
    interactive menu render (`hercules tools`, `hercules setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `hercules tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Hercules env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.hercules/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.hercules/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.hercules/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.hercules/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Hercules — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Hercules vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.hercules/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.hercules/.env`` over ``os.environ``.

    Used for Hercules-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("  hercules config edit     # Edit config file", Colors.DIM))
    print(color("  hercules config set <key> <value>", Colors.DIM))
    print(color("  hercules setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `hercules config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `hercules config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            ...
            print()
            print("Examples:")
            print("  hercules config set model anthropic/claude-sonnet-4")
        ...
        print("  hercules config           Show current configuration")
        print("  hercules config edit      Open config in editor")
        print("  hercules config set <key> <value>   Set a config value")
        print("  hercules config check     Check for missing/outdated config")
        print("  hercules config migrate   Update config with new options")
        print("  hercules config path      Show config file path")
        print("  hercules config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``hercules config`` UI so users
