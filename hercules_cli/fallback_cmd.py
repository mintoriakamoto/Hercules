"""
hercules fallback — manage the fallback provider chain.
...
Subcommands:
  hercules fallback [list]   Show the current fallback chain (default when no subcommand)
  hercules fallback add      Pick provider + model via the same picker as `hercules model`,
                           then append the selection to the chain
  hercules fallback remove   Pick an entry to delete from the chain
  hercules fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.hercules/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `hercules model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  hercules fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `hercules model`, then append the selection to the chain."""
    from hercules_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `hercules model` — select the provider + model you want as a fallback.")
    print()
