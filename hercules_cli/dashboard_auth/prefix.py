...
prefix (e.g. ``mission-control.tilos.com/hercules/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /hercules`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``HERCULES_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``hercules.example.com`` instead of
    ``https://hercules.example.com``) — the single most common cause of
    "I set HERCULES_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "hercules.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/hercules"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# HERCULES_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def _load_dashboard_section() -> dict:
    ...
    try:
        cfg = load_config()
    except Exception as exc:  # broad catch is intentional
        _log.debug(
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``HERCULES_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("HERCULES_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("HERCULES_DASHBOARD_PUBLIC_URL env var", env_raw)
