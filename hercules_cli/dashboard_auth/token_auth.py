...
How it fits the existing auth framework:
  ...
  * :func:`token_auth_middleware` runs OUTERMOST (installed last in
    ...
    :class:`~hercules_cli.dashboard_auth.base.TokenPrincipal` to
...
from hercules_cli.dashboard_auth import list_token_providers
from hercules_cli.dashboard_auth.audit import AuditEvent, audit_log
from hercules_cli.dashboard_auth.base import ProviderError, TokenPrincipal
...
def authenticate_token(
    ...
    for provider in list_token_providers():
        try:
            ...
            continue
        except Exception as e:  # a buggy provider must not 500 the gate
            _log.warning(
