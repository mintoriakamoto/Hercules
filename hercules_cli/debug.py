"""``hercules debug`` debug tools for Hercules Agent.

Currently supports:
    hercules debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.hercules/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[hercules debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.hercules/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``hercules debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``hercules debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "hercules-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``hercules debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.hercules/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``hercules debug share`` also runs an opportunistic sweep as a fallback
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from hercules_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_hercules_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``hercules debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``hercules dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``hercules dump`` is run
        internally.
...
def collect_share_bundle(
    ...
    upload path (:func:`build_debug_share`). Centralising it guarantees the
    uploaded bundle is built from force-redacted snapshots — redaction is the
    safety boundary.
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: hercules debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'hercules debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``hercules debug`` call.
    ...
    # best-effort — any failure is swallowed so ``hercules debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: hercules debug <command>")
        print()
        print("Commands:")
