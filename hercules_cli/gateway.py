"""
Gateway subcommand for hercules CLI.

Handles: hercules gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_hercules_home is imported lazily at call sites to avoid ImportError
# when hercules_constants is cached from a pre-update version during `hercules update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "hercules CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``hercules gateway status`` from falsely counting the ``hercules`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``hercules gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"hercules_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. HERCULES_HOME
        ...
        # HERCULES_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "hercules_home=" in command_lc
            and f"hercules_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``hercules update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Hercules profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Hercules profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from hercules_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "hercules_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m hercules_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose HERCULES_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``hercules update`` users
    # with no running gateway until they re-invoke ``hercules gateway``
    ...
    from hercules_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from hercules_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / HERCULES_HOME) the windowless
        # base interpreter needs to import hercules_cli.  Empty on POSIX, where
...
def _hercules_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``HERCULES_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("HERCULES_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_hercules_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HERCULES_HOME``.

    Under ``sudo``, ``HERCULES_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hercules_home` falls back to ``/root/.hercules`` — the wrong
    profile. The unit file pins ``HERCULES_HOME`` for the actual gateway
    ...
    if current == unit_home:
        return
    os.environ["HERCULES_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}hercules gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}hercules gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: hercules gateway stop")
    else:
        print(
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``hercules gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from hercules_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (hercules_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (HERCULES_HOME-scoped).
...
from hercules_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Hercules Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``hercules gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``HERCULES_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("HERCULES_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary HERCULES_HOME paths
...
def _profile_arg(hercules_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        hercules_home: Optional explicit HERCULES_HOME path. Defaults to the current
            ``get_hercules_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Hercules root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_hercules_root()``
...
def _profile_arg_for_target_user(hercules_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".hercules"
    try:
        Path(hercules_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(hercules_home, default_root=target_root)
    except ValueError:
        return _profile_arg(hercules_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this HERCULES_HOME.

    Default ``~/.hercules`` returns ``hercules-gateway`` (backward compatible).
    Profile ``~/.hercules/profiles/coder`` returns ``hercules-gateway-coder``.
    Any other HERCULES_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Hercules installs that predate the
# hercules-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (hercules-gateway-*.service) and unrelated third-party
# "hercules" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("hercules.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "hercules_cli.main gateway",
    "hercules_cli/main.py gateway",
    "gateway/run.py",
    " hercules gateway ",
    "/hercules gateway ",
)
...
def _find_legacy_hercules_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Hercules gateway units.

    Detects unit files installed by older Hercules versions that used a
    different service name (e.g. ``hercules.service`` before the rename to
    ``hercules-gateway.service``). When both a legacy unit and the current
    ``hercules-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``hercules-gateway-coder.service`` and unrelated third-party
      ``hercules-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``hercules.service`` running an unrelated
...
def has_legacy_hercules_units() -> bool:
    """Return True when any legacy Hercules gateway unit files exist."""
    return bool(_find_legacy_hercules_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Hercules gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_hercules_units()
    if not legacy:
        return
    ...
    print_info("    hercules gateway migrate-legacy")


def remove_legacy_hercules_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Hercules gateway unit files.

    Iterates over whatever ``_find_legacy_hercules_units()`` returns — which is
    ...
    legacy = _find_legacy_hercules_units()
    if not legacy:
        print("No legacy Hercules gateway units found.")
        return 0, []
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: hercules gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    hercules gateway uninstall")
    print_info("    sudo hercules gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: hercules gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Hercules often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.hercules`` → ``ai.hercules.gateway.plist`` (backward compatible).
    Profile ``~/.hercules/profiles/coder`` → ``ai.hercules.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.hercules.gateway-{suffix}" if suffix else "ai.hercules.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hercules/hercules-agent  -> /home/alice/.hercules/hercules-agent
      /opt/hercules                 -> /opt/hercules  (kept as-is)
...
def _hercules_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HERCULES_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hercules_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hercules                    → /home/alice/.hercules
      /root/.hercules/profiles/coder     → /home/alice/.hercules/profiles/coder
      /opt/custom-hercules               → /opt/custom-hercules  (kept as-is)
    """
    current_hercules = get_hercules_home().resolve()
    current_default = (Path.home() / ".hercules").resolve()
    target_default = Path(target_home_dir) / ".hercules"

    # Default ~/.hercules → remap to target user's default
    if current_hercules == current_default:
        return str(target_default)

    # Profile or subdir of ~/.hercules → preserve the relative structure
    try:
        relative = current_hercules.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.hercules) — keep as-is
        return str(current_hercules)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    hercules_node = hercules_home / "node" / "bin"
    if _is_dir(hercules_node):
        candidates.append(str(hercules_node))
    hercules_nm = hercules_home / "node_modules" / ".bin"
    if _is_dir(hercules_nm):
        candidates.append(str(hercules_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m hercules_cli.main``, so module
    ...
    dir, or a clone that ``hercules update`` later relocates/removes — the path
    ...
    ``HERCULES_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if HERCULES_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        hercules_home = _hercules_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(hercules_home, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m hercules_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
"""
    ...
    hercules_home = str(get_hercules_home().resolve())
    profile_arg = _profile_arg(hercules_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned HERCULES_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo hercules gateway … --system``, HERCULES_HOME is often stripped
    # and falls back to ``/root/.hercules``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / HERCULES_HOME comparisons
    ...
    _sync_hercules_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir HERCULES_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved HERCULES_HOME
    ...
    exported a throwaway ``HERCULES_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``hercules gateway restart``
    with ``HERCULES_HOME=/tmp/hercules-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="HERCULES_HOME=..."`` lines and launchd
    ``<key>HERCULES_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'HERCULES_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>HERCULES_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp HERCULES_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: HERCULES_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported HERCULES_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single HERCULES_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only HERCULES_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp HERCULES_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hercules_test`` path into
    # ``Environment="HERCULES_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/hercules_test"' in new_unit
        or "/hercules_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # HERCULES_HOME into the unit (manual E2E homes like /tmp/hercules-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Hercules install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo hercules gateway uninstall --system")
    print_info("         hercules gateway install")
    print_info("         hercules gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("HERCULES_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (hercules.service from pre-rename installs)
    # before installing the new hercules-gateway.service. If both remain, they
    ...
    # Existing system units already pin HERCULES_HOME; adopt it before any
    ...
    # ``sudo hercules gateway install --system --force`` would bake /root/.hercules
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}hercules gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}hercules gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}hercules gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # HERCULES_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `hercules gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `hercules gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}hercules gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}hercules gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}hercules gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}hercules gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.hercules.gateway-{suffix}" if suffix else "ai.hercules.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``hercules gateway start/stop/status`` call).
...
#      background process (the `nohup hercules gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_hercules_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_hercules_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m hercules_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active HERCULES_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "hercules_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup hercules gateway run --replace`
    ...
    log_dir = get_hercules_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup hercules gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>hercules_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>HERCULES_HOME</key>
        <string>{hercules_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.hercules/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See hercules-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_hercules_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Hercules install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  hercules gateway status             # Check status")
    from hercules_constants import display_hercules_home as _dhh
...
def launchd_stop():
    ...
    # `hercules gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # launchd PID and the Hercules PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Hercules install")
    else:
        print("⚠ Service definition is stale relative to the current Hercules install")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: hercules gateway stop")
            else:
    ...
    if deep:
        log_file = get_hercules_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``HERCULES_S6_SUPERVISED_CHILD``.
    """
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``hercules gateway run`` is always a misconfiguration, so we
    ...
    print()
    print("    hercules gateway restart")
    print()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``hercules gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``hercules gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other HERCULES_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("HERCULES_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Hercules gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hercules' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hercules/docker/entrypoint.sh before the Hercules command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$HERCULES_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set HERCULES_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `hercules gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set HERCULES_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `hercules gateway restart` which already
    ...
    # `hercules gateway start/restart` — leaving the gateway vulnerable to
    ...
    # HERCULES_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERCULES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from hercules_constants import get_hercules_home as _ghh
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. hercules) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Hercules delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Hercules connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: hercules pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Hercules will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``hercules setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run hercules under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.hercules/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_hercules_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from hercules_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'hercules pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: hercules pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'hercules gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Hercules will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Hercules will store the returned account_id/token in ~/.hercules/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `hercules gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_hercules_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `hercules pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `hercules pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "HerculesAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``hercules_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from hercules_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.hercules/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.hercules/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: hercules gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo hercules gateway install --system"
                        )
                    print_info("  Or run in foreground:  hercules gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: hercules gateway run")
                print_info(
                    "  For persistence:   tmux new -s hercules 'hercules gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: hercules gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup hercules gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'hercules gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`hercules_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # HERCULES_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``hercules gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``hercules gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``hercules gateway run`` are unaffected.
      2. ``HERCULES_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``hercules gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``HERCULES_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    # `docker logs` and ${HERCULES_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if HERCULES_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or HERCULES_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-hercules service's
    # pattern in docker/s6-rc.d/main-hercules/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: hercules gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: hercules gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hercules 'hercules gateway run'"
                )
                print()
            ...
            print()
            print(
                "  hercules gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hercules 'hercules gateway run'         # persistent via tmux"
            )
            print(
                "  nohup hercules gateway run > ~/.hercules/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: hercules gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: hercules gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: hercules gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        # slot — start them individually via `hercules -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: hercules gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERCULES_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `hercules gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERCULES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `hercules gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: hercules gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  hercules gateway install")
                else:
                    print("To install as a service:")
                    print("  hercules gateway install")
                    print("  sudo hercules gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  hercules gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup hercules gateway run > ~/.hercules/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hercules 'hercules gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  hercules gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  hercules gateway install  # Install as user service")
                    print(
                        "  sudo hercules gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Hercules gateway unit files from
        # pre-rename installs (e.g. hercules.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_hercules_units(interactive=not yes, dry_run=dry_run)
