...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_hercules_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:hercules_pkce",
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``hercules auth add openai-codex`` overwrote
    ...
    if provider == "kimi-oauth":
        ...
        entry = PooledCredential(
            ...
            auth_type=AUTH_TYPE_OAUTH,
            priority=0,
            source=f"{SOURCE_MANUAL}:kimi_cli",
            access_token=creds["api_key"],
            base_url=creds.get("base_url"),
        )
        pool.add_entry(entry)
    ...
    if provider == "gemini-oauth":
        ...
        entry = PooledCredential(
            ...
            auth_type=AUTH_TYPE_OAUTH,
            priority=0,
            source=f"{SOURCE_MANUAL}:gemini_cli",
            access_token=creds["api_key"],
            base_url=creds.get("base_url"),
        )
        pool.add_entry(entry)
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Hercules reads from
    ...
    from hercules_cli.auth import suppress_credential_source
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `hercules auth` is called bare."""
