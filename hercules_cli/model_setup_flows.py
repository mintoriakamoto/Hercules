"""Per-provider model-selection wizard flows for ``hercules setup`` / ``hercules model``.

Extracted from ``hercules_cli/main.py`` as part of the god-file decomposition
campaign (``~/.hercules/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from hercules_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``hercules_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from hercules_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``hercules_cli.main`` at import time -> no import cycle.
"""
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from hercules_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from hercules_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.hercules/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `hercules setup` from scratch. OpenRouter
    ...
    from hercules_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `hercules moa configure <name>` first.")
        return
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `hercules auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``hercules auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from hercules_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from hercules_cli.auth import (
...
def _model_flow_kimi_oauth(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        # Real in-app login: device-code flow (opens kimi.com, enter a code).
        # Falls back to any existing Kimi Code CLI session automatically.
        print("Logging in to Kimi (device-code)...")
        try:
            status = kimi_oauth_device_code_login()
        except AuthError as exc:
            print(f"Kimi login failed: {exc}")
            print(
                "You can also log in with the Kimi Code CLI (`kimi` → /login) "
                "and re-run setup."
            )
            return
        if not status.get("logged_in"):
            print("Kimi login did not complete.")
            if status.get("error"):
                print(f"Error: {status.get('error')}")
            return
        print("Logged in to Kimi. ✓")

    base_url = DEFAULT_KIMI_OAUTH_BASE_URL
    models = None
    try:
        creds = resolve_kimi_oauth_runtime_credentials(refresh_if_expiring=True)
        base_url = creds.get("base_url", base_url) or base_url
        models = fetch_api_models(creds["api_key"], base_url)
    except Exception:
        pass
    if not models:
        # Curated fallback when live discovery is unavailable (the /coding
        # endpoint speaks the Anthropic protocol, so the OpenAI-style /models
        # probe usually fails). Reuse the kimi-coding catalog — it is the same
    ...
    if selected:
        _save_model_choice(selected)
        _update_config_for_provider("kimi-oauth", base_url)
...
def _model_flow_gemini_oauth(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        # Built-in Login with Google (loopback browser flow, or paste-a-code
        # over SSH). Falls back to an existing gemini-cli session automatically.
        print("Logging in to Gemini with Google...")
        try:
            status = gemini_oauth_google_login()
        except AuthError as exc:
            print(f"Google login failed: {exc}")
            print(
                "You can also log in with the Gemini CLI (`gemini` → "
                "'Login with Google') and re-run setup."
            )
            return
        if not status.get("logged_in"):
            print("Google login did not complete.")
            if status.get("error"):
                print(f"Error: {status.get('error')}")
            return
        print("Logged in with Google. ✓")

    # Code Assist model access isn't listed via an OpenAI /models endpoint, so
    # offer the curated stable set.
...
def _model_flow_custom(config):
    ...
    from hercules_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from hercules_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    ...
    from hercules_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Hercules will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Hercules will install it now (the preflight below "
...
def _model_flow_copilot(config, current_model=""):
    ...
    from hercules_cli.config import save_env_value, load_config, save_config
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from hercules_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from hercules_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Hercules turns to `copilot --acp`.")
    print("  Hercules currently starts its own ACP subprocess for each request.")
    print("  Hercules uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set HERCULES_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from hercules_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so hercules knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from hercules_cli.config import load_config, save_config, get_env_value
    from hercules_cli.models import _PROVIDER_MODELS
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``hercules_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from hercules_cli.main import _prompt_provider_choice
    from hercules_cli.auth import ZAI_ENDPOINTS
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Hercules typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Hercules, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'hercules model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
