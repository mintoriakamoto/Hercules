...
from hercules_cli import auth as auth_mod
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from hercules_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from hercules_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Hercules' tool dispatch. Gated behind config key
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        # api.kimi.com/coding speaks the Anthropic Messages protocol — the
        # same endpoint the kimi-coding API-key provider already uses (see
        # KIMI_CODE_BASE_URL and _detect_api_mode_for_url). Routing this as
        # chat_completions sent requests to /coding/chat/completions, which
        # returns HTTP 404 resource_not_found on every call after an
        # otherwise-successful login. OAuth access tokens are JWTs, so the
        # Anthropic adapter sends them as Authorization: Bearer.
        api_mode = "anthropic_messages"
        pconfig = PROVIDER_REGISTRY.get(provider)
        base_url = base_url or (pconfig.inference_base_url if pconfig else "")
    elif provider == "gemini-oauth":
        # Gemini OAuth runs through chat_completions, but agent_init swaps in a
        # translating Code Assist http client (see agent/gemini_code_assist.py).
        api_mode = "chat_completions"
        pconfig = PROVIDER_REGISTRY.get(provider)
        base_url = base_url or (pconfig.inference_base_url if pconfig else "")
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from hercules_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from hercules_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("HERCULES_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``openrouter``, ``xai``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/hercules_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'hercules doctor' for details."
        )
        return None
...
def canonical_custom_identity(
    *,
    ...
    2. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``HERCULES_INFERENCE_PROVIDER`` equivalent). When the agent
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("HERCULES_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'hercules model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.hercules/.env or run 'hercules model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'hercules model')."
        )
    ...
    *,
    ...
    if requested_provider in ("vertex", "google-vertex", "vertex-ai", "gcp-vertex", "vertexai"):
        ...
        if not token or not base_url:
            raise AuthError(
                ...
                "in ~/.hercules/.env, or run 'gcloud auth application-default "
                "login' for ADC. Set the GCP project/region under vertex: in "
                "config.yaml if they aren't embedded in the credentials. "
                "Install the extra with: pip install 'hercules-agent[vertex]'."
            )
        return {
            "provider": "vertex",
            "api_mode": "chat_completions",
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "hercules-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "kimi-oauth":
        try:
            ...
            return {
                ...
                # (same endpoint as kimi-coding; see _detect_api_mode_for_url).
                # chat_completions here sent requests to
                # /coding/chat/completions → HTTP 404 resource_not_found.
                "api_mode": "anthropic_messages",
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "kimi-cli"),
                "expires_at_ms": creds.get("expires_at_ms"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from hercules_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Hercules canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Hercules-compatible importers).
