"""
Profile management for multiple isolated Hercules instances.

Each profile is a fully independent HERCULES_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.hercules/profiles/<name>/`` by default.

The "default" profile is ``~/.hercules`` itself — backward compatible,
zero migration needed.

Usage::

    hercules profile create coder          # fresh profile + bundled skills
    hercules profile create coder --clone  # also copy config, .env, SOUL.md, skills
    hercules profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    hercules -p coder chat                 # or via flag
    hercules profile use coder             # set as sticky default
    hercules profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See hercules_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.hercules``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   hercules-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `hercules backup` archives
...
# Marker file written by `hercules profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `hercules
...
# `hercules skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Hercules infrastructure directories that only the default profile
         (``~/.hercules``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_hercules_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.hercules) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "hercules-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when HERCULES_HOME equals the
...
# into the export. The set below identifies the *known Hercules profile
# artifacts* at the root of HERCULES_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``hercules_constants``.
...
_RESERVED_NAMES = frozenset({
    "hercules", "default", "test", "tmp", "root", "sudo",
})

# Hercules subcommands that cannot be used as profile names/aliases
_HERCULES_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hercules root, NOT to the current HERCULES_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HERCULES_HOME points outside
    ``~/.hercules``, profiles live under ``HERCULES_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_hercules_home() / "profiles"


def _get_default_hercules_home() -> Path:
    """Return the default (pre-profile) HERCULES_HOME path.

    In standard deployments this is ``~/.hercules``.
    In Docker/custom deployments where HERCULES_HOME is outside ``~/.hercules``
    (e.g. ``/opt/data``), returns HERCULES_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_hercules_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Hercules installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HERCULES_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_hercules_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, hercules subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _HERCULES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a hercules subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            hercules_exe = shutil.which("hercules") or "hercules"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(hercules_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Hercules. If we
    ...
    try:
        ...
        from hercules_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `hercules doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``hercules -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``hercules profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``hercules -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``hercules profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``HERCULES_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Hercules config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_hercules_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, hercules_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own HERCULES_HOME.
    ...
    The returned ``hercules_home`` is the path to pass to
    ``set_hercules_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_hercules_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``hercules update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.hercules)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.hercules/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `hercules -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Hercules profile.\n"
    ...
    if not soul_path.exists():
        try:
            from hercules_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `hercules update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`hercules profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Hercules (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `hercules profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `hercules -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches HERCULES_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``hercules profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HERCULES_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_hercules_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Hercules *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Hercules backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``HERCULES_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `hercules -p <canon> profile
    ...
    hercules_markers = ("hercules_cli.main", "hercules-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Hercules process: either an entrypoint marker in argv, or
            # a resolved executable named `hercules`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_hercules = (
                any(marker in joined for marker in hercules_markers)
                or exe_name == "hercules"
                or exe_name.startswith("hercules")
            )
            if not is_hercules:
                continue
            ...
            # ...or by HERCULES_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("HERCULES_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.hercules).\n"
            "To remove everything, use: hercules uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_hercules_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``HERCULES_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from hercules_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set HERCULES_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("HERCULES_HOME")
    try:
        os.environ["HERCULES_HOME"] = str(profile_dir)
        from hercules_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["HERCULES_HOME"] = old_home
        elif "HERCULES_HOME" in os.environ:
            del os.environ["HERCULES_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.hercules/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: hercules profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from HERCULES_HOME.

    Returns ``"default"`` if HERCULES_HOME is not set or points to ``~/.hercules``.
    Returns the profile name if HERCULES_HOME points into ``~/.hercules/profiles/<name>``.
    Returns ``"custom"`` if HERCULES_HOME is set to an unrecognized path.
    """
    from hercules_constants import get_hercules_home
    hercules_home = get_hercules_home()
    resolved = hercules_home.resolve()

    default_resolved = _get_default_hercules_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      HERCULES_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hercules file the
      user may have lying alongside HERCULES_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.hercules itself — its parent is ~/ and its
        # directory name is ".hercules", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: hercules profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.hercules itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.hercules). "
            "Specify a different name: hercules profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="hercules_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"hercules_{old_name}"
    legacy_old_host = f"hercules.{old_name}"
    new_host = f"hercules_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_hercules_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("hercules_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HERCULES_HOME path string.

    Called early in the CLI entry point, before any hercules modules
    are imported, to set the HERCULES_HOME environment variable.
    """
