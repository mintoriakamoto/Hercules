#!/usr/bin/env python3
"""
Hercules CLI - Main entry point.

Usage:
    hercules                     # Interactive chat (default)
    hercules chat                # Interactive chat
    hercules gateway             # Run gateway in foreground
    hercules gateway start       # Start gateway as service
    hercules gateway stop        # Stop gateway service
    hercules gateway status      # Show gateway status
    hercules gateway install     # Install gateway service
    hercules gateway uninstall   # Uninstall gateway service
    hercules setup               # Interactive setup wizard
    hercules logout              # Clear stored authentication
    hercules status              # Show status of all components
    hercules cron                # Manage cron jobs
    hercules cron list           # List cron jobs
    hercules cron status         # Check if cron scheduler is running
    hercules doctor              # Check configuration and dependencies
    hercules honcho setup                    # Configure Honcho AI memory integration
    hercules honcho status                   # Show Honcho config and connection status
    hercules honcho sessions                 # List directory → session name mappings
    hercules honcho map <name>               # Map current directory to a session name
    hercules honcho peer                     # Show peer names and dialectic settings
    hercules honcho peer --user NAME         # Set user peer name
    hercules honcho peer --ai NAME           # Set AI peer name
    hercules honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    hercules honcho mode                     # Show current memory mode
    hercules honcho mode [hybrid|honcho|local]  # Set memory mode
    hercules honcho tokens                   # Show token budget settings
    hercules honcho tokens --context N       # Set session.context() token cap
    hercules honcho tokens --dialectic N     # Set dialectic result char cap
    hercules honcho identity                 # Show AI peer identity representation
    hercules honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    hercules honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hercules + Honcho
    hercules version             Show version
    hercules update              Update to latest version
    hercules uninstall           Uninstall Hercules Agent
    hercules acp                 Run as an ACP server for editor integration
    hercules sessions browse     Interactive session picker with search

    hercules claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: hercules_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``hercules_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``hercules update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``hercules_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, hercules crashes on import and the user can't run
# ``hercules update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'hercules' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``hercules tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``hercules.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("hercules")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"hercules", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"hercules")
        # Windows: the .exe name is already ``hercules.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* hercules_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("HERCULES_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".hercules", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``HERCULES_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``hercules … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("HERCULES_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``HERCULES_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("HERCULES_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`hercules --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from hercules_cli import __release_date__, __version__
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``hercules --version`` before config/logging imports on Termux."""
    if os.environ.get("HERCULES_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from hercules_cli.subcommands.model import build_model_parser
from hercules_cli.subcommands.setup import build_setup_parser
from hercules_cli.subcommands.postinstall import build_postinstall_parser
from hercules_cli.subcommands.whatsapp import build_whatsapp_parser
...
from hercules_cli.subcommands.doctor import build_doctor_parser
from hercules_cli.subcommands.security import build_security_parser
from hercules_cli.subcommands.dump import build_dump_parser
...
from hercules_cli.subcommands.uninstall import build_uninstall_parser
from hercules_cli.subcommands.dashboard import build_dashboard_parser
from hercules_cli.subcommands.gui import build_gui_parser
from hercules_cli.subcommands.logs import build_logs_parser
from hercules_cli.subcommands.prompt_size import build_prompt_size_parser
from hercules_cli.subcommands.memory import build_memory_parser
from hercules_cli.subcommands.acp import build_acp_parser
from hercules_cli.subcommands.tools import build_tools_parser
from hercules_cli.subcommands.insights import build_insights_parser
from hercules_cli.subcommands.skills import build_skills_parser
from hercules_cli.subcommands.pairing import build_pairing_parser
from hercules_cli.subcommands.plugins import build_plugins_parser
from hercules_cli.subcommands.mcp import build_mcp_parser
from hercules_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (hercules tools, hercules setup, hercules model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'hercules {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any hercules module import.
#
# Many modules cache HERCULES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERCULES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.hercules/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HERCULES_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `hercules mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Hercules' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo hercules -p <name>` against the invoking user's home.
        ...
        candidate = home / ".hercules" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # Mirrors hercules_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If HERCULES_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.hercules/profiles/coder or
    # /opt/data/profiles/coder).  If HERCULES_HOME points to the hercules root
    # instead (e.g. systemd hardcodes HERCULES_HOME=/root/.hercules), we must
    # still read active_profile — the user may have switched profiles via
    # `hercules profile use` and the gateway should honour that choice.
    # See issue #22502.
    hercules_home_env = os.environ.get("HERCULES_HOME", "")
    if profile_name is None and hercules_home_env:
        if Path(hercules_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the hercules root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as HERCULES_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``hercules gateway run`` to mean
    # "the root HERCULES_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("HERCULES_S6_SUPERVISED_CHILD"):
        try:
            from hercules_constants import get_default_hercules_root

            active_path = get_default_hercules_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set HERCULES_HOME
    if profile_name is not None:
        try:
            from hercules_cli.profiles import resolve_profile_env

            hercules_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            hercules_home = _resolve_sudo_user_profile_env(profile_name)
            ...
            # A bug in profiles.py must NEVER prevent hercules from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["HERCULES_HOME"] = hercules_home
...
# Load .env from ~/.hercules/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from hercules_cli.config import get_hercules_home
from hercules_cli.env_loader import load_hercules_dotenv

load_hercules_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERCULES_REDACT_SECRETS env
# var BEFORE hercules_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "HERCULES_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["HERCULES_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `hercules` subcommands
...
try:
    from hercules_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from hercules_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if hercules_constants not importable yet
...
from hercules_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_hercules_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("HERCULES_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("HERCULES_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from hercules_cli.config import get_env_path, get_hercules_home, load_config
    from hercules_cli.auth import get_auth_status

    # Determine whether Hercules itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_hercules_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Only count these if Hercules has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Hercules to use their tokens.
    if _has_hercules_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, hercules_bin
        cli_args: the original CLI arguments (everything after 'hercules')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    hercules_bin = container_info["hercules_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo hercules {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo hercules {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  hercules --tui --resume {target}")
    if title:
        print(f'  hercules --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @hercules/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@hercules" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/hercules-ink/package.json",
    "packages/hercules-ink/index.js",
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``HERCULES_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("HERCULES_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``HERCULES_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("HERCULES_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    hercules_home = os.environ.get("HERCULES_HOME") or str(Path.home() / ".hercules")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HERCULES_HOME": hercules_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _find_bundled_tui(hercules_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if hercules_cli_dir is None:
        hercules_cli_dir = Path(__file__).parent
    bundled = hercules_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``hercules update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Hercules checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `hercules update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Hercules checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `hercules --tui`\n"
        "If the checkout is still inconsistent, run `hercules update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (HERCULES_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("HERCULES_NODE")
    ...
    ext_dir = os.environ.get("HERCULES_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with HERCULES_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset HERCULES_TUI_DIR (e.g. `unset HERCULES_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @hercules/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @hercules/ink resolves through
        # packages/hercules-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "hercules-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from hercules_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env["HERCULES_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["HERCULES_PYTHON_SRC_ROOT"] = os.environ.get(
        "HERCULES_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("HERCULES_PYTHON", sys.executable)
    env.setdefault("HERCULES_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["HERCULES_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["HERCULES_MODEL"] = model
        env["HERCULES_INFERENCE_MODEL"] = model
    if provider:
        env["HERCULES_TUI_PROVIDER"] = provider
        env["HERCULES_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["HERCULES_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["HERCULES_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["HERCULES_TUI_SKILLS"] = value
    if query:
        env["HERCULES_TUI_QUERY"] = query
    if image:
        env["HERCULES_TUI_IMAGE"] = image
    if checkpoints:
        env["HERCULES_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["HERCULES_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["HERCULES_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["HERCULES_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["HERCULES_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["HERCULES_ACCEPT_HOOKS"] = "1"
    ...
    # HERCULES_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `hercules --tui` try to
    ...
    # still set HERCULES_TUI_RESUME themselves.
    env.pop("HERCULES_TUI_RESUME", None)
    if resume_session_id:
        env["HERCULES_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `hercules update` so
    ...
    if code == 42:
        from hercules_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``HERCULES_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``hercules kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``hercules kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("HERCULES_KANBAN_BOARD"):
        return
    try:
        from hercules_cli.kanban_db import get_current_board

        os.environ["HERCULES_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.hercules/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Hercules — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Hercules still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``hercules … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("HERCULES_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'hercules sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from hercules_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from hercules_cli.config import load_config as _load_config_for_xai_check
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Hercules isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  hercules setup")
        print()

        from hercules_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'hercules setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from hercules_cli.banner import prefetch_update_check
    ...
    # user's ~/.hercules/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["HERCULES_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["HERCULES_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["HERCULES_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from hercules_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from hercules_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Hercules?")
        print()
    ...
    # has no creds.json.  Every subsequent `hercules gateway` then paid a 30s
    ...
    session_dir = get_hercules_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: hercules gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `hercules gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  hercules gateway")
        ...
        print()
        print("  Or install as a service: hercules gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'hercules whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``hercules whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``hercules_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from hercules_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from hercules_cli.config import stamp_install_method
    from hercules_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``hercules model``) and the setup wizard
    ...
    from hercules_cli.providers import resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'hercules model' for "
                "available providers, or run 'hercules doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in hercules_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.hercules/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.hercules/.env if the active provider is not 'custom'.
    ...
    """
    from hercules_cli.config import get_env_value, save_env_value, load_config
...
#
# Hercules uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `hercules model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `hercules model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`hercules_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``hercules model`` flow, then route aux tasks to them here.
    """
    from hercules_cli.config import load_config
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from hercules_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Hercules URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# the catalog eagerly costs ~55ms on every `hercules` invocation — including
# fast paths like `hercules --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.hercules/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from hercules_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `hercules setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Hercules will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Hercules CLI with a provider."""
    from hercules_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from hercules_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from hercules_cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from hercules_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from hercules_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``hercules slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: hercules slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `hercules slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from hercules_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from hercules_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from hercules_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from hercules_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from hercules_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `hercules security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from hercules_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from hercules_cli.dump import run_dump
...
def cmd_config(args):
    ...
    from hercules_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Hercules home directory to a zip file."""
    if getattr(args, "quick", False):
        from hercules_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Hercules backup from a zip file."""
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from hercules_cli.banner import format_banner_version_label
    ...
    try:
        from hercules_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Hercules Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from hercules_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from hercules_cli.uninstall import run_gui_uninstall
    ...
    from hercules_cli.uninstall import run_uninstall
...
# Critical files that Hercules must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "hercules_cli/main.py",
    "hercules_cli/config.py",
    "hercules_cli/__init__.py",
    "hercules_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "hercules_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``hercules`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="hercules-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``hercules update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``hercules_cli/web_dist/`` (per vite.config.ts
    outDir: "../hercules_cli/web_dist"), NOT to ``web/dist/``, so Python
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hercules`` launcher present but ``hercules_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the hercules venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — hercules venv python3, no nix-shell overhead
    ...
    # a hercules venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``hercules update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``hercules web``).
    ...
    # build path usable on a stock `py -m hercules_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_hercules_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (hercules web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``hercules update`` can unconditionally call ``hercules desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``hercules desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $HERCULES_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``hercules update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Hercules.app/Contents/MacOS/Hercules"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Hercules.exe",
            release_dir / "win-ia32-unpacked" / "Hercules.exe",
            release_dir / "win-arm64-unpacked" / "Hercules.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hercules",
            release_dir / "linux-unpacked" / "Hercules",
            release_dir / "linux-arm64-unpacked" / "hercules",
            release_dir / "linux-arm64-unpacked" / "Hercules",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Hercules'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_hercules_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Hercules.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Hercules.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Hercules.exe: Access is
    ...
    an unrelated "Hercules" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Hercules is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Hercules.app/Contents/MacOS/Hercules  ->  app bundle = .../Hercules.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``hercules desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the HERCULES_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    # with_hercules_node_path() copies os.environ when called with no arg.
    env = with_hercules_node_path()
    if getattr(args, "fake_boot", False):
        env["HERCULES_DESKTOP_BOOT_FAKE"] = "1"
    ...
    if getattr(args, "cwd", None):
        ...
        env["HERCULES_DESKTOP_CWD"] = os.getcwd()
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  hercules gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Hercules is
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``hercules dashboard`` processes other than ourselves.

    ``hercules dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``hercules update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Hercules Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``hercules serve`` as
    ...
    environment variable ``HERCULES_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "hercules dashboard",
        "hercules_cli.main dashboard",
        "hercules_cli/main.py dashboard",
        # The headless backend (`hercules serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "hercules serve",
        "hercules_cli.main serve",
        "hercules_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "hercules.*dashboard"`) keeps us consistent
            # with `hercules_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `hercules update`.
    ...
    print("  Preview now:  hercules curator run --dry-run")
    print("  Pause it:     hercules curator pause")
    print(
        "  Docs:         https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/features/curator.md"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``hercules update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``hercules update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: hercules curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``hercules dashboard`` processes.

    Called at the end of ``hercules update`` (default ``reason``) and also
    from ``hercules dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Hercules Desktop Electron app spawns this dashboard as a
    # backend child, it sets HERCULES_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("HERCULES_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    hercules dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.hercules-update-staging"
    backup = f"{dst}.hercules-update-old"
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `hercules update "
            f"--branch {branch}`, or update against main with `hercules update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/mintoriakamoto/Hercules/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="hercules-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"hercules-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to hercules-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"hercules-agent-{branch}")
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: hercules skills list-modified  "
    ...
    try:
        from hercules_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "hercules-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Hercules behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes hercules completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Hercules couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Hercules couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Hercules behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Hercules couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Hercules couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `hercules update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/mintoriakamoto/Hercules.git",
    "git@github.com:mintoriakamoto/Hercules.git",
    "https://github.com/mintoriakamoto/Hercules",
    "git@github.com:mintoriakamoto/Hercules",
}
OFFICIAL_REPO_URL = "https://github.com/mintoriakamoto/Hercules.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_hercules_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_hercules_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from mintoriakamoto/Hercules.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/mintoriakamoto/Hercules.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/mintoriakamoto/Hercules.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``hercules update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from hercules_constants import get_default_hercules_root

    default_home = get_default_hercules_root()
...
# Install-scoped breadcrumb dropped right before ``hercules update`` mutates the
...
# survives and the next ``hercules`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $HERCULES_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``hercules update`` left half-done.
    ...
    protocol stream (``hercules acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if hercules.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _hercules_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Hercules is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Hercules from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `hercules update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from hercules_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``hercules update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hercules_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_hercules_instances(
    ...
    The Hercules Desktop Electron app spawns ``hercules.EXE`` as a backend child,
    so during ``hercules update`` the user-invoked process and the desktop's
    ...
    shims (``hercules.exe`` / ``hercules-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``hercules update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``hercules.exe``) is a separate process from the Python
    ...
    for shim in _hercules_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated hercules.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``hercules update``
    ...
    #      hercules.exe sitting *under* a non-Hercules parent (e.g. a Hercules
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "hercules.exe"
    lines = ["✗ Another hercules.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Hercules Desktop, exit any open `hercules` REPLs, and")
    lines.append("  stop the gateway (`hercules gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `hercules update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_hercules_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``hercules.exe``.
    ...
    when ``hercules update`` runs, ``hercules.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``hercules.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next hercules invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Hercules Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Hercules Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hercules_exe_shims(scripts_dir):
        ...
        print(
            "    Close Hercules Desktop, exit other `hercules` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `hercules update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_hercules_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``hercules.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``hercules.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``hercules`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_hercules_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``hercules.exe.old.*`` left by prior updates.

    Called early on every hercules invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``hercules update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `hercules update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hercules.exe`` / ``hercules-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_hercules_exe`` for the rationale.
    """
    ...
    # hours later inside ``hercules update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``hercules.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``hercules.exe`` shim is locked during ``hercules update``, or when uv/distlib
    skips a launcher write. The symptom is ``hercules-agent.exe`` and
    ``hercules-acp.exe`` present but ``hercules.exe`` missing, so ``hercules`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `hercules update --force` after "
            "closing other hercules processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m hercules_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``hercules update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``hercules.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``hercules`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `hercules update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `hercules update --force` after closing other hercules processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from hercules_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$HERCULES_HOME/bin/uv``, but on Termux the official
    ...
    """
    from hercules_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `hercules update`, so we install root-only first
    ...
    nixos_env = with_hercules_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `hercules update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``hercules update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.hercules/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``hercules update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``hercules update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.hercules/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``hercules update --gateway``) the update is already
    ...
    try:
        ...
        # hercules_cli.config.get_hercules_home to simulate setup failure.
        from hercules_cli.config import get_hercules_home as _get_hercules_home

        logs_dir = _get_hercules_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.hercules/logs/update.log`` only, never the terminal.

    During ``hercules update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``hercules update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``hercules update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``hercules update`` without requiring a reinstall.  The
    ...
    ``hercules`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/hercules.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v hercules'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/hercules, code at /usr/local/lib/hercules-agent).
    fhs_link = Path("/usr/local/bin/hercules")
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of HERCULES_HOME before running the update.
    ...
    HERCULES_HOME directories.  The ``--backup`` flag on ``hercules update``
    ...
    # ``hercules_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # HERCULES_HOME can add minutes to every update. Users who want the
    ...
    # Render path using display_hercules_home so the user sees ~/.hercules/...
    try:
        from hercules_constants import get_hercules_home, display_hercules_home

        home = get_hercules_home()
        try:
            display_path = f"{display_hercules_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  hercules import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``hercules update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``hercules update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run hercules from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.hercules-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".hercules-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The hercules.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m hercules_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``hercules
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m hercules_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "hercules_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Hercules processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Hercules Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    hercules.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Hercules...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m hercules_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: hercules gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``hercules gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from hercules_cli import gateway_windows
        from hercules_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``hercules update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `hercules update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another hercules.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_hercules_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `hercules serve` backend) will keep .pyd
    ...
    # the hercules.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        # though `git pull` can't touch $HERCULES_HOME, this is cheap
        ...
        # orphan merge-conflict markers in hercules_cli/config.py bricked
        # every user who ran ``hercules update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``hercules
        ...
        # the old bytecode (e.g. get_hercules_home added to hercules_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``hercules`` launch finishes the
        ...
        # build.  ``hercules desktop --build-only`` uses the content-hash stamp
        ...
        # current — so copy it straight over ~/.hercules/cache/model_catalog.json
        ...
        # modules in this process.  Reload hercules_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hercules_home() added since the last release.
        try:
            import importlib
            import hercules_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit HERCULES_HOME so
        # it is not affected by sync_skills()'s module-level HERCULES_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's HERCULES_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from hercules_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: hercules config migrate")
            else:
                print()
                print("Skipped. Run 'hercules config migrate' later to configure.")
        else:
        ...
        # check `hercules curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``hercules update`` gives users a
        ...
        # When running as ``hercules update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_hercules_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... hercules-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... hercules-gateway*)
            ...
            # Discover all hercules-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. hercules-gateway.service or hercules-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `hercules update` restart it automatically, allow\n"
                            ...
                            # path in `hercules gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: hercules gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: hercules gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: hercules -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Hercules gateway unit files are still installed.
        # When both hercules.service (from a pre-rename install) and the
        # current hercules-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `hercules update` surfaces the issue until the user migrates.
        try:
            from hercules_cli.gateway import (
                has_legacy_hercules_units,
                _find_legacy_hercules_units,
                supports_systemd_services,
            )
            ...
            if supports_systemd_services() and has_legacy_hercules_units():
                ...
                print()
                print("  These pre-rename units (hercules.service) fight the current")
                print("  hercules-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    hercules gateway migrate-legacy")
                print()
        ...
        print("  hercules model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``hercules -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `hercules profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_hercules_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → hercules -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.hercules)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  hercules profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     hercules -p {name} chat")
                else:
        ...
        from hercules_cli import profiles as _profiles_mod
        ...
        from hercules_cli import profile_describer as _pd
        ...
        from hercules_cli.profiles import profile_exists, validate_alias_name
        ...
        from hercules_cli.profiles import rename_profile
        ...
        from hercules_cli.profiles import export_profile
        ...
        from hercules_cli.profiles import import_profile
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="hercules_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  hercules -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      hercules -p {plan.manifest.name} chat")
        ...
        from hercules_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `hercules profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"hercules -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hercules_requires"):
            print(f"Requires:     Hercules {data['hercules_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hercules_requires:
        print(f"  Requires: Hercules {mf.hercules_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``hercules dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``hercules dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No hercules dashboard processes running.")
        return 0

    print(f"{len(pids)} hercules dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``hercules dashboard register`` for OAuth.
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No hercules dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `hercules update`.
    ...
    # launches the dashboard (`worker dashboard` → HERCULES_HOME points into
    ...
    if (
        ...
        # HERCULES_HOME.  We must resolve the root explicitly instead of just
        # dropping HERCULES_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV HERCULES_HOME=/opt/data`), so an unset
        # HERCULES_HOME falls back to $HOME/.hercules = /opt/data/.hercules — an
        ...
        # update-button guard also misfires).  get_default_hercules_root()
        # returns the root for both layouts: ~/.hercules for a standard install
        ...
        try:
            ...
            # behaviour (drop HERCULES_HOME) rather than block the reroute.
            env.pop("HERCULES_HOME", None)
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "hercules_cli" / "web_dist"
        )
        ...
        # HERCULES_WEB_DIST is set without --skip-build: the build is skipped
        ...
        os.environ["HERCULES_WEB_DIST"] = str(_dist_root)
    ...
    try:
        from hercules_cli.mcp_startup import start_background_mcp_discovery
    ...
    from hercules_cli.web_server import start_server
...
def cmd_gateway_enroll(args):
    ...
    from hercules_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from hercules_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from hercules_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Hercules log files."""
    from hercules_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Hercules command console."""
    from hercules_cli.console_engine import run_console_repl
...
# so that in ``hercules -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``hercules_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``hercules -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``hercules`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("HERCULES_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that HERCULES_YOLO_MODE is set before ANY
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["HERCULES_SAFE_MODE"] = "1"
    os.environ["HERCULES_IGNORE_USER_CONFIG"] = "1"
    os.environ["HERCULES_IGNORE_RULES"] = "1"
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from hercules_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from hercules_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["HERCULES_DEFER_AGENT_STARTUP"] = "1"
            os.environ["HERCULES_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["HERCULES_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `hercules --tui` is the hot path on phones. The full parser setup imports
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from hercules_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from hercules_cli.tools_config import run_post_setup_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from hercules_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from hercules_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from hercules_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from hercules_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from hercules_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from hercules_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for hercules CLI."""
    # Cosmetic: make the process show up as 'hercules' instead of 'python3.11'
    ...
    # Sweep stale ``hercules.exe.old.*`` quarantine files left by previous
    # ``hercules update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_hercules_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``hercules update``
    ...
    # ``hercules skills install update``) merely defers recovery one launch;
    # under-matching (missing ``hercules -p work update``) would race a recovery
    ...
    from hercules_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from hercules_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/features/fallback-providers.md"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `hercules model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.hercules/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://github.com/mintoriakamoto/Hercules/tree/main/website/docs/user-guide/secrets"
        ),
    )
    ...
    from hercules_cli import secrets_cli as _secrets_cli
    from hercules_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from hercules_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # setup command  (parser built in hercules_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in hercules_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in hercules_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `hercules whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in hercules_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    # login command  (parser built in hercules_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in hercules_cli/subcommands/logout.py)
    # =========================================================================
    ...
    # =========================================================================
    # cron command  (parser built in hercules_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in hercules_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from hercules_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in hercules_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in hercules_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in hercules_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in hercules_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in hercules_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in hercules_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.hercules/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo hercules uses to snapshot working directories before "
    ...
    # =========================================================================
    # import command  (parser built in hercules_cli/subcommands/import_cmd.py)
    # =========================================================================
    ...
    # =========================================================================
    # pairing command  (parser built in hercules_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in hercules_cli/subcommands/skills.py)
    # =========================================================================
    ...
    # =========================================================================
    # plugins command  (parser built in hercules_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``hercules --help``, ``hercules version``, ``hercules logs``,
    ...
    try:
        from hercules_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Hercules shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from hercules_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from hercules_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in hercules_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in hercules_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `hercules computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `hercules tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `hercules computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Hercules app. `status` reports the driver's grant\n"
            ...
            if path:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: hercules computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
    ...
    # =========================================================================
    # mcp command  (parser built in hercules_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <hercules home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from hercules_state import workspace_key as _ws_key
            ...
            from hercules_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from hercules_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from hercules_cli.session_export_html import (
            ...
            # Markdown / QMD export
            from hercules_cli.session_export_md import (
            ...
            # `hercules sessions prune`: no time window and no filters at all
            ...
            # Launch hercules --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in hercules_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in hercules_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in hercules_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in hercules_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in hercules_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in hercules_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in hercules_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in hercules_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Hercules-Setup.exe success screen tells users to
    # run `hercules desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in hercules_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in hercules_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in hercules_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``hercules -c Pokemon Agent Dev`` → ``hercules -c 'Pokemon Agent Dev'``
    ...
    from hercules_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'hercules -c model' where 'model' is consumed as the
    ...
    # --yolo: set HERCULES_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (hercules hooks list, cron
