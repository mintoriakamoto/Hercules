...
- ``hercules_cli.providers``        -- canonical provider identity + overlays
- ``hercules_cli.model_normalize``  -- per-provider name formatting
...
_HERCULES_MODEL_WARNING = (
    "Nous Research Hercules 3 & 4 models are NOT agentic and are not designed "
    "for use with Hercules Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hercules 3 / Hercules 4 chat families.
# The previous substring check (`"hercules" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hercules-brain:qwen3-14b-ctx16k`` that just
# happen to carry "hercules" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hercules-3-Llama-3.1-70B, hercules-4-405b, openrouter/hercules3:70b
# Negative examples it must NOT match:
#   hercules-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HERCULES_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hercules[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_hercules_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Hercules 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_HERCULES_NON_AGENTIC_RE.search(model_name))


def _check_hercules_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hercules 3/4 chat model."""
    if is_nous_hercules_non_agentic(model_name):
        return _HERCULES_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``hercules config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from hercules_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'hercules model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'hercules doctor' — config issues detected:"
        ...
        from hercules_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from hercules_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from hercules_cli.config import load_config
            from hercules_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from hercules_cli.providers import resolve_user_provider as _ruser
    ...
    # hercules_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from hercules_cli.models import normalize_opencode_base_url
    ...
    hercules_warn = _check_hercules_model_warning(new_model)
    if hercules_warn:
        warnings.append(hercules_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from hercules_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from hercules_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from hercules_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from hercules_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    if "lmstudio" not in curated and (
        ...
        from hercules_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Hercules-mapped providers ---
    from hercules_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hercules_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hercules_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(hercules_id)
        if (
            _alias_target
            and _alias_target != hercules_id
        ...
        pconfig = PROVIDER_REGISTRY.get(hercules_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (HERCULES_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Hercules may not route yet.
        ...
        from hercules_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(hercules_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(hercules_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        hercules_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `hercules model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(hercules_id)
        if not model_ids:
            model_ids = curated.get(hercules_id, [])
            if hercules_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(hercules_id, model_ids)
    ...
    # Build reverse mapping: models.dev ID → Hercules provider ID.
    # HERCULES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hercules IDs ("copilot").
    _mdev_to_hercules = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HERCULES_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Hercules slug — e.g. "github-copilot" → "copilot"
        hercules_slug = _mdev_to_hercules.get(pid, pid)
        if hercules_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(hercules_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, hercules_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or hercules_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(hercules_slug):
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", hercules_slug, exc)
        ...
        # configured.
        if not has_creds and hercules_slug == "anthropic":
            try:
                ...
                if (hercules_creds and hercules_creds.get("accessToken")) or \
        ...
        if hercules_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(hercules_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(hercules_slug)
            ...
            if not model_ids:
                model_ids = curated.get(hercules_slug, []) or curated.get(pid, [])
                if hercules_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(hercules_slug, model_ids)
        total = len(model_ids)
        if hercules_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": hercules_slug,
            "name": get_label(hercules_slug),
            "is_current": hercules_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hercules",
        ...
        seen_slugs.add(hercules_slug.lower())
        _record_builtin_endpoint(hercules_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or HERCULES_OVERLAYS (keeps /model in sync
    # with `hercules model`).
    try:
        from hercules_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Hercules's canonical write key (matches
            ...
            # Hercules writes ``models:`` as a dict keyed by model id, but older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Hercules's own writer uses; a hyphen variant is accepted
            ...
            # active model. Hercules's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, hercules_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from hercules_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`hercules_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from hercules_cli.models import fetch_openrouter_models
