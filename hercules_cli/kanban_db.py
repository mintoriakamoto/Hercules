...
``<root>`` is the **shared Hercules root** (the parent of any active
...
``hercules -p <profile>`` joins the same board as the dispatcher that
...
the profile's Hercules home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``HERCULES_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``HERCULES_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``hercules kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.hercules``. In Docker / custom
deployments where ``HERCULES_HOME`` points outside ``~/.hercules`` (e.g.
``/opt/hercules``), ``<root>`` is ``HERCULES_HOME``. Legacy env-var
overrides still work:

* ``HERCULES_KANBAN_DB`` — pin the database file path directly.
* ``HERCULES_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``HERCULES_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``HERCULES_KANBAN_DB``,
``HERCULES_KANBAN_WORKSPACES_ROOT``, and ``HERCULES_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/hercules-kanban-v1-spec.pdf`` for the full
...
from hercules_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active HERCULES_HOME so dispatcher- and
...
# ``HERCULES_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``HERCULES_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("HERCULES_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``HERCULES_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("HERCULES_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``HERCULES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "hercules_kanban_current_board_override",
    default=None,
)
...
def kanban_home() -> Path:
    """Return the shared Hercules root that anchors the kanban board.

    Resolution order:

    1. ``HERCULES_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hercules_root()``, which already returns ``<root>``
       when ``HERCULES_HOME`` is ``<root>/profiles/<name>``, and returns
       ``HERCULES_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``HERCULES_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("HERCULES_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``hercules kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``HERCULES_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``hercules kanban boards
    ...
    env = os.environ.get("HERCULES_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``hercules kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``HERCULES_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("HERCULES_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``HERCULES_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("HERCULES_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``HERCULES_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("HERCULES_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``hercules kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``HERCULES_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (hercules_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``HERCULES_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("HERCULES_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``hercules gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Hercules treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``HERCULES_KANBAN_DB`` env → ``HERCULES_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `hercules kanban list` mid-integrity-probe) block
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``hercules kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from hercules_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``hercules --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from hercules_cli import projects_db as _pdb
    ...
    # `hercules --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``hercules kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `hercules kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``hercules kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``hercules kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``hercules kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``hercules kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``HERCULES_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Hercules' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Hercules-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``hercules kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Hercules checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Hercules treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Hercules creates/reuses that linked worktree. With
      no ``workspace_path``, Hercules anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Hercules uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``HERCULES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Hercules
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``hercules kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``hercules kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Hercules profile.
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Hercules profile.
        # `_default_spawn` invokes ``hercules -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Hercules
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `hercules kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from hercules_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_hercules_argv() -> list[str]:
    """Return the interpreter-bound Hercules CLI invocation."""
    # ``hercules_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``hercules`` package — there is no
    # ``hercules`` package to import.
    return [sys.executable, "-m", "hercules_cli.main"]


def _absolute_hercules_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Hercules shim."""
...
def _hercules_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Hercules executable path.
    ...
    return [_absolute_hercules_path(path)]


def _resolve_hercules_argv() -> list[str]:
    """Resolve the ``hercules`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$HERCULES_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("hercules")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\hercules.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m hercules_cli.main`` — fallback for setups where
       Hercules is launched from a venv and the ``hercules`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hercules_bin`` for the same reason. Kept
    local (not imported from gateway) because ``hercules_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("HERCULES_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _hercules_path_argv(env_bin)
...
def _resolve_worker_cli_toolsets(hercules_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``HERCULES_KANBAN_TASK`` is set.
    """
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``hercules -p <profile> chat -q ...`` subprocess.
    ...
    ``HERCULES_KANBAN_DB`` / ``HERCULES_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject HERCULES_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `hercules -p <name>` the
    # _apply_profile_override() runs *before* hercules_constants is imported.
    # If HERCULES_HOME is absent from the child's env, get_hercules_home() falls
    # back to Path.home() / ".hercules" (the DEFAULT profile root), ignoring the
    ...
    try:
        ...
        # _apply_profile_override() via HERCULES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # HERCULES_HOME never had profiles created.
        pass
    if task.tenant:
        env["HERCULES_TENANT"] = task.tenant
    env["HERCULES_KANBAN_TASK"] = task.id
    env["HERCULES_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["HERCULES_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["HERCULES_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["HERCULES_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["HERCULES_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["HERCULES_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`hercules -p <name>`
    # rewrites HERCULES_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hercules_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["HERCULES_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["HERCULES_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["HERCULES_KANBAN_BOARD"] = resolved_board
    # HERCULES_PROFILE is the author the kanban_comment tool defaults to.
    # `hercules -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["HERCULES_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited HERCULES_TUI=1
    ...
    # older hercules builds on PATH that predate the flag's precedence.
    env.pop("HERCULES_TUI", None)
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("HERCULES_HOME"))
    if worker_toolsets:
    ...
    # `hercules kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``hercules kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERCULES_PROFILE values like "hercules-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Hercules root exists

    Reads profile paths directly so this module has no import dependency on
    ``hercules_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from hercules_constants import get_default_hercules_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``hercules kanban assignees`` for the terminal.
