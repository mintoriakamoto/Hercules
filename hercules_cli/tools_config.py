...
`hercules tools` and `hercules setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.hercules/config.yaml under
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("blackboard",      "📋 Blackboard",                "shared board for agent-to-agent communication"),
...
# They're still in _HERCULES_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `hercules tools` → Video Generation, which walks
...
# `hercules tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from hercules_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `hercules tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``hercules tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from hercules_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``hercules tools`` checklist actually offers
    for ``platform``.
...
from hercules_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "image_gen": {
        ...
        # All provider rows are injected at runtime from the plugin
        # packages; no hardcoded UX setup-flow rows remain.
        "providers": [],
    },
    "video_gen": {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Hercules routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set HERCULES_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "HERCULES_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "HERCULES_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "HERCULES_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "HERCULES_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "HERCULES_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `hercules model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("HERCULES_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Hercules telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``hercules update`` and
      by ``hercules computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `hercules update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `hercules computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hercules_home()}/hercules-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in hercules-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.hercules/config.yaml")
        ...
        # Run the full `hercules auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.hercules/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from hercules_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: hercules auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `hercules auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: hercules auth spotify")
        except Exception as exc:
        ...
        try:
            from hercules_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: hercules plugins enable observability/langfuse")
        _print_info("    Restart Hercules for tracing to take effect.")
        _print_info("    Verify: hercules plugins list")
        ...
        try:
            ...
            _print_info("    Run later: hercules auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `hercules auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: hercules auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: hercules auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``hercules tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``hercules tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: hercules tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``hercules-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``hercules-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"hercules-{platform}"
    ...
    # "hercules-cli" (which include all _HERCULES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[hercules-cli, spotify]`` after enabling Spotify via ``hercules
        ...
        # (e.g. "hercules-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``hercules tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `hercules tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"hercules-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("hercules-")}
    ...
    # shipped as a bundled plugin but user must opt in via `hercules tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `hercules tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by hercules tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `hercules` instead of
    # `hercules-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `hercules update`/`hercules doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `hercules tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (hercules-cli, hercules-telegram, etc.)
    ...
    # Opening `hercules tools` is the user's opt-in to reconfigure tools, so treat
...
# ("Firecrawl Self-Hosted") remains in TOOL_CATEGORIES because it
# describes an alternative *setup flow* for the firecrawl backend
# rather than a distinct provider.
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `hercules tools` gets a silent no-op
...
def _write_provider_config(provider: dict, config: dict) -> None:
    ...
    and clears the ``use_gateway`` flags, but does NOT prompt for env vars,
    run post-setup hooks, or run interactive model pickers. Both the CLI
    ...
    # Find which category this provider belongs to and clear use_gateway
...
def _configure_provider(
    ...
    *,
    ...
    if not env_vars:
        ...
        video_plugin = provider.get("video_gen_plugin_name")
        if video_plugin:
            _select_plugin_video_gen_provider(video_plugin, config, use_gateway=False)
            return
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``hercules model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `hercules model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `hercules model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://github.com/mintoriakamoto/Hercules/blob/main/website/docs/user-guide/features/tools.md", Colors.DIM))
    print()
    ...
    print()
    from hercules_constants import display_hercules_home
    print(color(f"  Tool configuration saved to {display_hercules_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'hercules' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, hercules mcp
