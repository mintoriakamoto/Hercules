...
- Keep data: Remove code but keep ~/.hercules/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Hercules PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing hercules-agent or hercules PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Hercules Agent" comment and following line
                if '# Hercules Agent' in line or '# hercules-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('hercules' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hercules
                if 'hercules' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the hercules wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "hercules",
        Path("/usr/local/bin/hercules"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains hercules_cli reference)
                content = wrapper.read_text()
                if 'hercules_cli' in content or 'hercules-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hercules_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``hercules`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Hercules home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (hercules_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``hercules gateway run`` processes
    ...
    try:
        from hercules_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from hercules_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``HERCULES_HOME`` and ``HERCULES_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\hercules\git\cmd``, ``%LOCALAPPDATA%\hercules\git\bin``,
#      ``%LOCALAPPDATA%\hercules\git\usr\bin``, ``%LOCALAPPDATA%\hercules\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\hercules\git\`` and Node to
#      ``%LOCALAPPDATA%\hercules\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``hercules dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\hercules\gateway-service\`` and sometimes
...
def _hercules_path_markers(hercules_home: Path) -> list[str]:
    """Path-entry substrings that identify Hercules-owned User-PATH entries."""
    root = str(hercules_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare hercules-agent install dir.
    markers = [root + "\\hercules-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if HERCULES_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "hercules".  We don't want to catch
    # unrelated entries like "chercules-foo" or "ephermeral", so we look for
    # backslash-hercules as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hercules_home: Path) -> list[str]:
    """Strip Hercules-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hercules_path_markers(hercules_home)
            kept: list[str] = []
...
def remove_hercules_env_vars_windows() -> list[str]:
    """Delete HERCULES_HOME and HERCULES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("HERCULES_HOME", "HERCULES_GIT_BASH_PATH"):
                try:
...
def _is_default_hercules_home(hercules_home: Path) -> bool:
    """Return True when ``hercules_home`` points at the default (non-profile) root."""
    try:
        from hercules_constants import get_default_hercules_root
        return hercules_home.resolve() == get_default_hercules_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its HERCULES_HOME directory.

    We shell out to ``hercules -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current HERCULES_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m hercules_cli.main` so we don't depend on a `hercules`
    #    wrapper that may be half-removed mid-uninstall.
    hercules_invocation = [_sys.executable, "-m", "hercules_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                hercules_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's HERCULES_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``hercules uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$HERCULES_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from hercules_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(hercules_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Hercules Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Hercules Chat GUI installation was found.")
        print(f"  Checked: {hercules_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Hercules agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(hercules_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Hercules agent at {hercules_home / 'hercules-agent'}")
        print(f"  • Your config, sessions, and secrets under {hercules_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Hercules agent is still installed. Run 'hercules' to use the CLI,")
    print("or 'hercules uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.hercules/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.hercules/ for future reinstall
    """
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            ...
            hercules_home=hercules_home,
            full_uninstall=bool(getattr(args, "full", False)),
        )
        return

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie HERCULES_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_hercules_home(hercules_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.hercules data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {hercules_home / 'config.yaml'}")
    print(f"  Secrets: {hercules_home / '.env'}")
    print(f"  Data:    {hercules_home / 'cron/'}, {hercules_home / 'sessions/'}, {hercules_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their HERCULES_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Hercules data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Hercules code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hercules_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Hercules PATH entries from shell configs / Windows User PATH")
    print("  • Hercules wrapper scripts and Hercules-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Hercules config/data: {hercules_home}")
        if _is_default_hercules_home(hercules_home):
        ...
        print(f"  • Keep Hercules config/data: {hercules_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``hercules`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$HERCULES_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in hercules_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\hercules\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(hercules_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Hercules-owned PATH entries in User environment")

        log_info("Removing HERCULES_HOME / HERCULES_GIT_BASH_PATH User env vars...")
        removed_env = remove_hercules_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Hercules-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing hercules command...")
    ...
    #     (only when they still point into this Hercules home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Hercules-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(hercules_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Hercules-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(hercules_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE HERCULES_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from hercules_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(hercules_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.hercules/, just remove the hercules-agent subdir
            if hercules_home in project_root.parents or project_root.parent == hercules_home:
    ...
    #     under HERCULES_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(hercules_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(hercules_home)
    ...
    # 5. Optionally remove ~/.hercules/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile HERCULES_HOME dirs live under
        ...
        try:
            if hercules_home.exists():
                shutil.rmtree(hercules_home)
                log_success(f"Removed {hercules_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {hercules_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {hercules_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  git clone https://github.com/mintoriakamoto/Hercules.git", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m hercules_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import hercules_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``hercules_constants`` + ``hercules_cli.colors``
    (and lazily ``hercules_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m hercules_cli.uninstall")
    parser.add_argument(
        "--mode",
