...
Add, remove, or reorder entries here — both `hercules setup` and
`hercules` provider-selection will pick up the change automatically.
"""
...
from hercules_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HERCULES_USER_AGENT = f"hercules-cli/{_HERCULES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `hercules model`
    flow without maintaining a separate static list.
    """
    from hercules_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $HERCULES_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Hercules-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $HERCULES_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Hercules refreshes the cache from
...
# this list:  hercules model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `hercules model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `hercules model` TUI
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("kimi-oauth",     "Kimi Code (OAuth)",        "Kimi Code via subscription login (built-in device-code login; also reuses a Kimi Code CLI session)"),
    ...
    ProviderEntry("gemini-oauth",   "Gemini (Login with Google)", "Gemini via Google account OAuth (reuses local Gemini CLI login; Code Assist)"),
]
...
#
# Some vendors expose several Hercules provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `hercules model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``hercules model``, the
...
# sensible default. For any metered aggregator whose list is ordered
# best-/most-capable-first, entry [0] may be the priciest flagship; pin the
# silent default to a low-cost curated model here so a missing model can never
# escalate to the flagship on the non-interactive fallback path.
#
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``hercules auth add openai-codex`` without
    ``hercules model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``hercules model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    hercules-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from hercules_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — hercules-agent
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, etc.).
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``hercules model``, ``/model``, etc.).
    """
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        novita:deepseek/deepseek-v3             →  ("novita", "deepseek/deepseek-v3")
...
def _resolve_static_model_alias(
    ...
    try:
        from hercules_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model openai` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Hercules' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``hercules_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Hercules should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``hercules auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.hercules/.env``.
    ...
    """
    try:
        from hercules_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Hercules release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Hercules release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from hercules_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``hercules model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $HERCULES_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_hercules_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$HERCULES_HOME/auth.json`` and external
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    """
    try:
        from hercules_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=is_agent_turn)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "HerculesAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _HERCULES_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Hercules' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _HERCULES_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from hercules_constants import get_hercules_home
    return get_hercules_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `hercules moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Hercules will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Hercules cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Hercules actually ships.  (#46850)
