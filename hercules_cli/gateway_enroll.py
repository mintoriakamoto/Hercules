"""``hercules gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.hercules/.env``. The per-gateway secret authenticates the WS upgrade;
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `hercules gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.hercules/.env (0600 dir) and never logs.
    to_write = {
    ...
    from hercules_cli.config import get_env_path
