...
sidebar_position: 3
title: "Desktop App"
description: "The native Hercules desktop app — a polished experience for chatting with Hercules, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Hercules desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Hercules Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hercules` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Hercules has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`hercules`) and **[TUI](./tui.md)** (`hercules --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`hercules dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Hercules Desktop](../getting-started/installation.md).

If you already have Hercules installed, simply run
...
- **The same conversation history** as every other Hercules surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Hercules instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Hercules Desktop to a remote backend](./features/web-dashboard.md#connecting-hercules-desktop-to-a-remote-backend).
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `hercules desktop --cwd <path>` (or the `HERCULES_DESKTOP_CWD` environment variable).

### Voice

Talk to Hercules and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `hercules model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Hercules management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#hercules-cron).
- **Profiles** — switch between [Hercules profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Hercules agent, your config, and your chats stay. (Same as `hercules uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `hercules uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `hercules uninstall --full`.)
...
You can do the same from the terminal — `hercules uninstall --gui` for the GUI alone, or `hercules uninstall` / `hercules uninstall --full` for the agent too.

:::note
Running `hercules uninstall --gui` from a **source checkout** (a `hercules desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `hercules desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `hercules desktop`

To launch via the CLI, simply run `hercules desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `hercules update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `HERCULES_DESKTOP_CWD`)           |
| `--hercules-root PATH` | Override the Hercules source root the app uses (sets `HERCULES_DESKTOP_HERCULES_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `hercules` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Hercules Agent runtime into `HERCULES_HOME` (`~/.hercules`, or `%LOCALAPPDATA%\hercules` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `HERCULES_DESKTOP_HERCULES_ROOT`, then a completed managed install, then a probed `hercules` on `PATH` (unless `--ignore-existing` / `HERCULES_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `HERCULES_DESKTOP_HERCULES` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `hercules serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `hercules --tui`. The desktop app is **self-contained**: it runs its own `hercules serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Hercules backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `hercules serve` process
"Remote backend" means a **`hercules serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `hercules serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
- **OAuth-backed provider — preferred for anything reachable beyond your own machine.** For a VPS, a public host, or any remote backend, protect the dashboard with an OAuth-backed provider such as **self-hosted OIDC** (bring your own identity provider). Register the dashboard with `hercules dashboard register` to provision its OAuth client, then sign in from the app.
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.hercules/.env` (the secrets file, mode 0600):
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/hercules` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `HERCULES_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `HERCULES_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.hercules/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `HERCULES_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Hercules Desktop to a remote backend](./features/web-dashboard.md#connecting-hercules-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Hercules Desktop](../reference/environment-variables.md#web-dashboard--hercules-desktop).

## Troubleshooting

Boot logs land in `HERCULES_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.hercules/hercules-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
- [TUI](./tui.md) — the modern terminal UI used by `hercules --tui` and the dashboard chat tab
