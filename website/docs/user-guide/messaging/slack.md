...
public HTTP endpoints, so your Hercules instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Hercules uses the modern
...
The fastest path is to paste a manifest Hercules generates for you. It
...
### Option A: From a Hercules-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   hercules slack manifest --write
   ```
   This writes `~/.hercules/slack-manifest.json` and prints paste-in
   instructions.
...
it will only work in DMs. Without `files:read`, Hercules can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `hercules-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Hercules configuration issue.
:::
...
Hercules uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
hercules gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
hercules gateway              # Foreground
hercules gateway install      # Install as a user service
sudo hercules gateway install --system   # Linux only: boot-time system service
```
...
Every Hercules command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Hercules command with its description.

Under the hood: Hercules ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/mintoriakamoto/Hercules/blob/main/hercules_cli/commands.py)
...
When Hercules adds new commands (e.g. after `hercules update`), regenerate
...
3. Paste the new contents of `~/.hercules/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/hercules <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/hercules btw run the tests` — Hercules routes it the same way as `/btw
run the tests`. Free-form questions also work: `/hercules what's the
...
Slack never delivers them to Hercules.

As a workaround, Hercules recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Hercules treats it identically to the slash form and replies in the same
thread.
...
Hercules falls back to a text prompt, the prompt instructs you to reply
...
```bash
hercules slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Hercules behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Hercules Agent what time is it?`). In channels, Hercules replies in a thread attached to that message. |
| **Threads** | If you @mention Hercules inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.hercules/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Hercules in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Hercules will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Hercules will deliver scheduled messages,
...
Hercules can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Hercules also loads tokens from an **OAuth token file** at:

```
~/.hercules/slack_tokens.json
```
...
- When a message arrives, Hercules uses the correct workspace-specific client to respond.
...
Hercules supports voice on Slack:
...
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Hercules now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
- Tokens should be stored in `~/.hercules/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Hercules config directory
