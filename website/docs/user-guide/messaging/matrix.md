...
Hercules works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Hercules behaves once it's connected.

## How Hercules Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Hercules responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Hercules requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Hercules supports Matrix threads (MSC3440). If you reply in a thread, Hercules keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Hercules auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | Hercules accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Hercules normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, Hercules isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
Hercules sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Hercules now includes the current Matrix room name, room ID, topic, message ID,
...
2. Choose a username like `hercules` — the full user ID will be `@hercules:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `hercules-bot`).

### Option C: Use Your Own Account

You can also run Hercules as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Hercules needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Hercules the bot's user ID and password. Hercules will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@hercules:your-server.org
...
Add the following to your `~/.hercules/.env` file:
...
# MATRIX_USER_ID=@hercules:matrix.example.org
...
Bridge and appservice deployments need extra loop protection. Hercules always
...
Optional behavior settings in `~/.hercules/config.yaml`:
...
:::tip
You can run `hercules gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Hercules supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with hercules extras
cd ~/.hercules/hercules-agent && uv pip install -e ".[matrix]"
```
...
When E2EE is enabled, Hercules:

- Stores encryption keys in `~/.hercules/platforms/matrix/store/` (legacy installs: `~/.hercules/matrix/store/`)
...
In Matrix conversations, Hercules exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Hercules knows who requested the action.

### Media Limits

Hercules uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Hercules rejects arbitrary
...
Hercules includes an opt-in Synapse harness for local validation:

```bash
docker compose -f tests/e2e/matrix_synapse_gateway/docker-compose.yml up -d
HERCULES_MATRIX_SYNAPSE_INTEGRATION=1 \
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Hercules imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Hercules bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.hercules/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Hercules detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@hercules:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@hercules:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@hercules:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@hercules:your-server';
   "
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40hercules%3Ayour-server/devices/DEVICE_ID'
   ```
...
2. Delete the local crypto store and restart Hercules:
   ```bash
   rm -f ~/.hercules/platforms/matrix/store/crypto.db*
   # restart hercules
   ```
...
Hercules supports the same gateway commands in Matrix that it supports on other
...
Hercules only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/mintoriakamoto/Hercules/issues/12614).
...
If you previously used Hercules with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@hercules:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Hercules Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.hercules/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.hercules/platforms/matrix/store/crypto.db
   rm -f ~/.hercules/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.hercules/.env`:
...
a working E2EE setup with a previous version of Hercules and are upgrading.
...
services:
  hercules-matrix:
    ...
    environment:
      ...
      MATRIX_DEVICE_ID: "HERCULES_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.hercules/platforms/matrix/store
```
...
RUN cd ~/.hercules/hercules-agent && uv pip install -e ".[matrix]"

CMD ["hercules", "gateway"]
```
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Hercules API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Hercules-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: Hercules uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Hercules's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Hercules logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Hercules gateway isn't running, or it failed to connect.

**Fix**: Check that `hercules gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.hercules/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: Hercules can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
