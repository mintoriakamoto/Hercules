...
:::tip
Hercules also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `hercules --tui` — see the [TUI](tui.md) guide.
:::
...
hercules chat --model "anthropic/claude-sonnet-4"

# With a specific provider
hercules chat --provider openrouter  # Force OpenRouter

# With specific toolsets
hercules chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
hercules -s hercules-agent-dev,github-auth
hercules chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
hercules --continue             # Resume the most recent CLI session (-c)
hercules --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
hercules chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
hercules -w                         # Interactive mode in worktree
hercules -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Hercules CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Hercules CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERCULES_YOLO_MODE` is on (either `hercules --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`hercules -c` or `hercules --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Hercules to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Hercules supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Hercules replies |
...
```bash
hercules -s hercules-agent-dev,github-auth
hercules chat -s github-pr-workflow -s github-auth
```

Hercules loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.hercules/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.hercules/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Hercules..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Hercules recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Hercules — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
Resume this session with:
  hercules --resume 20260225_143052_a1b2c3
...
```bash
hercules --continue                          # Resume the most recent CLI session
hercules -c                                  # Short form
hercules -c "my project"                     # Resume a named session (latest in lineage)
hercules --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
hercules --resume "refactoring auth"         # Resume by title
hercules -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `hercules sessions rename <id> <title>` from the command line. Use `hercules sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Hercules's SQLite state database under `~/.hercules/state.db`. The database keeps:
...
Hercules immediately confirms the task and gives you back the prompt:
