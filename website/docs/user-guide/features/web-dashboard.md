...
```bash
# Custom port
hercules dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
hercules dashboard --host 0.0.0.0

# Start without opening browser
hercules dashboard --no-open
```
...
child with the selected profile's `HERCULES_HOME`, so the conversation runs
...
processes (manage them via `hercules -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `hercules-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.hercules/hercules-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.hercules/hercules-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `hercules dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `hercules dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `hercules --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.hercules/hercules-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.hercules/.env
ExecStart=/path/to/venv/bin/python -m hercules_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.hercules/.env` containing:

```bash
HERCULES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
HERCULES_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `hercules config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Hercules instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `hercules profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.hercules/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `hercules skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `hercules mcp` reads from.
...
`hercules mcp catalog` / `hercules mcp install` use.
...
Connect Hercules to any messaging platform from the browser — full parity with
`hercules setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.hercules/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
![System admin page — host stats and Tool Gateway status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.hercules/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`HERCULES_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/hercules/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Hercules Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth-backed provider](#oauth-backed-provider)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Hercules Desktop connection](#connecting-hercules-desktop-to-a-remote-backend). Every login is verified against an external OAuth identity provider, so this is a provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). Suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `hercules dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `hercules dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `hercules dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `hercules dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Hercules doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `hercules dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### OAuth-backed provider

An OAuth-backed dashboard provider auto-registers a `DashboardAuthProvider` when a client ID is configured.

Because every login is verified against an external OAuth identity provider, **this is a provider suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use this provider you need an OAuth client ID (shape `agent:{id}`). Get one with **`hercules dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing login (run `hercules setup` first if you're not logged in), registers a self-hosted OAuth client with the provider, and writes `HERCULES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hercules/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

```bash
hercules dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HERCULES_DASHBOARD_OAUTH_CLIENT_ID to ~/.hercules/.env
```
...
|---------|-----------|--------|----------------|
| `HERCULES_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `hercules dashboard register` |

Per the Hercules Agent convention (`~/.hercules/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • oauth: HERCULES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). Your OAuth
    provider provisions this env var (shape 'agent:{instance_id}') when
    it deploys a Hercules Agent instance — set it to your provisioned
...
From a logged-in Hercules install to an OAuth-gated dashboard in three steps.

**1. Log in and register the dashboard.** `hercules dashboard register` uses your existing login to provision an OAuth client and writes `HERCULES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hercules/.env` for you:

```bash
hercules setup            # if you're not already logged in
hercules dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HERCULES_DASHBOARD_OAUTH_CLIENT_ID to ~/.hercules/.env
```
...
```bash
hercules dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click the sign-in button → authenticate at your OAuth provider → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://hercules.example.com/auth/callback` and set `HERCULES_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `HERCULES_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `HERCULES_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `HERCULES_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.hercules/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.hercules/.env <<EOF
HERCULES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
HERCULES_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.hercules/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [OAuth-backed provider](#oauth-backed-provider) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
Like the OAuth-backed provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback / `--insecure` dashboards.
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `HERCULES_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `HERCULES_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `HERCULES_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-hercules.json` — it defines a `hercules` realm, a **public PKCE client** (`hercules-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "hercules-dashboard",
      "name": "Hercules Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-hercules.json:/opt/keycloak/data/import/realm-hercules.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/hercules/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/hercules`). The admin console is at
...
```bash
export HERCULES_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/hercules"
export HERCULES_DASHBOARD_OIDC_CLIENT_ID="hercules-dashboard"
export HERCULES_DASHBOARD_PUBLIC_URL="http://localhost:9119"
hercules dashboard --host 0.0.0.0 --port 9119 --no-open
```

`HERCULES_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → hercules-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `HERCULES_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/hercules"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `HERCULES_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `HERCULES_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements an authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with your provider" button → `/auth/login?provider=oauth`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to the provider's `/oauth/authorize` endpoint.
4. User authenticates with the provider, lands at `/auth/callback?code=…&state=…`.
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the provider's JWKS (`/.well-known/jwks.json`), and sets the `hercules_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `hercules_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `hercules_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `hercules_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$HERCULES_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.hercules/plugins/dashboard-auth-myidp/__init__.py
from hercules_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `HERCULES_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
HERCULES_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  hercules dashboard --host 0.0.0.0
...
Hercules Desktop can drive a Hercules backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Hercules Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is an **OAuth-backed provider** (register it with [`hercules dashboard register`](#registering-a-dashboard)). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [OAuth-backed provider](#oauth-backed-provider).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.hercules/.env (secrets file, 0600).
cat >> ~/.hercules/.env <<'EOF'
HERCULES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
hercules dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.hercules/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.hercules/.env`, so the credentials are in the environment at boot.

:::warning
The dashboard reads and writes your `.env` (API keys, secrets) and can run agent commands. The **username/password** setup shown here is for a trusted network — never expose a password-protected dashboard directly to the open internet. Put it behind a VPN. [Tailscale](https://tailscale.com/) is the clean option: bind to the machine's tailscale IP (`--host <tailscale-ip>`) and use `http://<tailscale-ip>:9119` as the Remote URL. Only devices on your tailnet can reach it. To reach a backend over the public internet, use an **OAuth-backed provider** instead.
:::
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/hercules` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `HERCULES_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `HERCULES_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `HERCULES_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `HERCULES_DASHBOARD_BASIC_AUTH_USERNAME` / `HERCULES_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `HERCULES_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `hercules_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `hercules update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `hercules dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Hercules Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Hercules Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
