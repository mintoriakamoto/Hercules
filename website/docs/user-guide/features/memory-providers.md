...
```bash
hercules memory setup      # interactive picker + configuration
hercules memory status     # check what's active
hercules memory off        # disable external provider
```

You can also select the active memory provider via `hercules plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.hercules/config.yaml`:
...
When a memory provider is active, Hercules automatically:
...
**Setup Wizard:**
```bash
hercules memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `hercules honcho`
If you previously used `hercules honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Hercules profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Hercules profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Hercules profile. Host key `hercules` → default; `hercules.<profile>` for others. |
...
```bash
hercules profile create coder --clone
```

`--clone` creates a `hercules.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
hercules honcho sync
```

Scans every Hercules profile, creates host blocks for any profile without one, inherits settings from the default `hercules` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `hercules memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/mintoriakamoto/Hercules/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hercules).
...
hercules memory setup    # select "openviking"
# Or manually:
hercules config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.hercules/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.hercules/.env
```
...
`OPENVIKING_AGENT` is Hercules' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
hercules memory setup    # select "mem0" → "Platform"
# Or manually:
...
**Setup (OSS):**
```bash
hercules memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
hercules memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```
...
**Config:** `$HERCULES_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.hercules/.env`.
...
| `user_id` | `hercules-user` | User identifier |
...
**Switching modes:** Re-run `hercules memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
hercules memory setup    # select "hindsight"
# Or manually:
hercules config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.hercules/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p hercules ui start`

**Config:** `$HERCULES_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `hercules` | Memory bank identifier |
...
See [plugin README](https://github.com/mintoriakamoto/Hercules/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
hercules memory setup    # select "holographic"
# Or manually:
hercules config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.hercules-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$HERCULES_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
hercules memory setup    # select "retaindb"
# Or manually:
hercules config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.hercules/.env
```
...
# Then configure Hercules
hercules memory setup    # select "byterover"
# Or manually:
hercules config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$HERCULES_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=hercules) |
...
**Setup:**
```bash
hercules memory setup    # select "supermemory"
# Or manually:
hercules config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.hercules/.env
```

**Config:** `$HERCULES_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `hercules` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hercules-{identity}` → `hercules-coder`) to isolate memories per Hercules profile
...
| **Requires** | `pip install hercules-memori` + `hercules-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install hercules-memori
hercules-memori install
hercules config set memory.provider memori
...
| **Memori** | Cloud | Free/Paid | 5 | `hercules-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$HERCULES_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$HERCULES_HOME/` so each profile has its own credentials
