...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `hercules tools`.
...
Hercules Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `hercules tools` and persists in `config.yaml`.
...
:::tip Image generation setup
Image generation uses FAL. Set `FAL_KEY` in `~/.hercules/.env`, then enable the tool via `hercules tools`.
:::
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Hercules materializes them to the local cache so delivery still works after expiry
