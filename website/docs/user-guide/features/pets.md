...
Hercules can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<HERCULES_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Hercules renders the sprite at full fidelity when your
...
hercules pets list
hercules pets list cat

# Install a pet and make it active in one step
hercules pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
hercules pets show

# Check your setup
hercules pets doctor
```

## `hercules pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `hercules pets list [query] [--limit N]` |
| List installed pets | `hercules pets list --installed` |
| Install a pet | `hercules pets install <slug> [--select] [--force]` |
| Set the active pet | `hercules pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `hercules pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `hercules pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `hercules pets off` |
| Remove an installed pet | `hercules pets remove <slug>` |
| Diagnose setup | `hercules pets doctor` |

`hercules pets show` flags:
...
Beyond installing pre-made pets from the gallery, Hercules can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `hercules pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `hercules tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `HERCULES_PET_IMAGE_PROVIDER` env var (e.g. `HERCULES_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Hercules is
...
Run `hercules pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Hercules uses its own
  profile-scoped `<HERCULES_HOME>/pets/` instead — install through `hercules pets`.
