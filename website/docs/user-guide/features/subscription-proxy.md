...
description: "Use your OAuth provider subscription as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Hercules-managed provider subscription as their
...
| Use case | "Use Hercules as a chat backend" | "Use my provider sub from another app" |
...
```bash
hercules auth add xai-oauth --type oauth
```

This opens your browser for the provider's OAuth flow. Hercules stores
the refresh token in `~/.hercules/auth.json` — the same place all Hercules
provider logins live.

### 2. Start the proxy

```bash
hercules proxy start
```

```
Starting Hercules proxy for xAI
...
Model:      Hercules-4-70B    # or Hercules-4.3-36B, Hercules-4-405B
```

The proxy ignores the `Authorization` header from your app and attaches
your real provider credential to the upstream request. Refreshes happen
automatically when the bearer approaches expiry.

## Available providers

```bash
hercules proxy providers
```

Currently shipped: `xai` (xAI / Grok). More OAuth providers can be added
by implementing the `UpstreamAdapter` interface in
`hercules_cli/proxy/adapters/`.

## Check status

```bash
hercules proxy status
```

```
Hercules proxy upstream adapters

  [xai     ] xAI — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `hercules auth add xai-oauth --type oauth`.
If you see `credentials need attention`, your refresh token was revoked
(rare — happens if you signed out from the provider) — just re-run the
login.

## Allowed paths

The proxy only forwards paths the upstream actually serves. For the
upstream provider:
...
OpenViking's VLM calls now flow through your provider subscription. The
embedding model side still needs its own provider — the upstream may
serve `/v1/embeddings` but the model selection depends on what your tier
supports.
...
INFERENCE_TEXT_MODEL=Hercules-4-70B
```
...
```bash
hercules proxy start --host 0.0.0.0 --port 8645
```

⚠ **Be aware:** anyone on your network can now use your provider
...
Your provider tier's RPM/TPM limits apply across the whole proxy. The
proxy doesn't fan out or pool — it's a single bearer with your full
subscription quota. Monitor usage in your provider's dashboard.
...
`hercules_cli/proxy/adapters/<provider>.py` and registering it in
