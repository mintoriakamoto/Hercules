...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Hercules CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Hercules CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
Or set the default skin in `~/.hercules/config.yaml`:
...
Create YAML files under `~/.hercules/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.
...
## Hercules Mod — Visual Skin Editor

[Hercules Mod](https://github.com/cocktailpeanut/hercules-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.
...
- Opens any skin into a visual editor with all Hercules skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.hercules/skins/`
- Activates a skin by updating `~/.hercules/config.yaml`
...
6. Click **Save** to write the skin YAML to `~/.hercules/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Hercules Mod respects the `HERCULES_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `hercules_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.hercules/skins/` take precedence over built-in skins with the same name.
