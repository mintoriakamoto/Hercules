...
- Building a custom/local tool without modifying Hercules core? Start with [Build a Hercules Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Hercules itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Hercules instance (throwaway HERCULES_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m hercules_cli.main
scripts/dev-sandbox.sh --persistent python -m hercules_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.hercules/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.hercules` — use `get_hercules_home()` from `hercules_constants` for code paths and `display_hercules_home()` for user-facing messages. See [AGENTS.md](https://github.com/mintoriakamoto/Hercules/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
- **Discussions**: [github.com/mintoriakamoto/Hercules/discussions](https://github.com/mintoriakamoto/Hercules/discussions)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/mintoriakamoto/Hercules/blob/main/LICENSE).
