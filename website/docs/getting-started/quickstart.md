...
This guide gets you from zero to a working Hercules setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Hercules Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.
...
- Setting up Hercules for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Hercules working on my machine | `hercules setup` | Run a real chat and verify it responds |
| I already know my provider | `hercules model` | Save the config, then start chatting |
| I want a bot or always-on setup | `hercules gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `hercules model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `hercules model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Hercules cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Hercules Agent
### Command-line install (recommended)
Install the CLI first; the desktop app can then be launched from it with
`hercules desktop` (no separate installer download is published for this
repository — desktop packages are built from source or via the
`desktop-release.yml` workflow's artifacts).
...
The single most important setup step. Use `hercules model` to walk through the choice interactively:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `hercules update`. Re-enable anything later with `hercules tools`, seed skills with `hercules skills opt-in --sync`, or tune settings with `hercules setup agent`.
:::
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hercules_cli/models.py` may change between releases) | `hercules model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `hercules model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `hercules model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `hercules model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `hercules model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `hercules model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Hercules separates secrets from normal config:

- **Secrets and tokens** → `~/.hercules/.env`
- **Non-secret settings** → `~/.hercules/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
hercules config set model anthropic/claude-opus-4.6
hercules config set terminal.backend docker
hercules config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
hercules            # classic CLI
hercules --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Hercules ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `hercules` vs `hercules --tui`.
:::
...
- Hercules replies without error
...
```bash
hercules --continue    # Resume the most recent session
hercules -c            # Short form
```
...
```bash
hercules gateway setup    # Interactive platform configuration
```
...
- `hercules tools` — tune tool access per platform
- `hercules skills` — browse and install reusable workflows
...
```bash
hercules config set terminal.backend docker    # Docker isolation
hercules config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Hercules install directory (the curl installer placed it at
# ~/.hercules/hercules-agent on Linux/macOS or %LOCALAPPDATA%\hercules\hercules-agent on Windows):
cd ~/.hercules/hercules-agent
...
Skills are on-demand instruction documents that teach Hercules how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Hercules ships with a catalog of bundled skills already installed in `~/.hercules/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
hercules skills browse                      # list everything available
hercules skills search kubernetes           # find skills by keyword
hercules skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `hercules skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Hercules ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.hercules/hercules-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Hercules opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `hercules model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `hercules gateway setup` and check `hercules gateway status` |
| `hercules --continue` can't find old session | Switched profiles or session never saved | Check `hercules sessions list` and confirm you're in the right profile |
...
| `hercules doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `hercules model`
3. `hercules setup`
4. `hercules sessions list`
5. `hercules --continue`
6. `hercules gateway status`
...
| `hercules model` | Choose your LLM provider and model |
| `hercules tools` | Configure which tools are enabled per platform |
| `hercules setup` | Full setup wizard (configures everything at once) |
| `hercules doctor` | Diagnose issues |
| `hercules update` | Update to latest version |
| `hercules gateway` | Start the messaging gateway |
| `hercules --continue` | Resume last session |
