...
# Hercules on Android with Termux
...
Hercules Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Hercules from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `hercules` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `hercules` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/hercules" "$PREFIX/bin/hercules"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `hercules` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.hercules/.env`.
...
### `hercules doctor` says ripgrep or Node is missing
