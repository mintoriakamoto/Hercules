...
To easily install the command-line and desktop applications, [download the Hercules Desktop installer](https://github.com/mintoriakamoto/Hercules/) from our website and run it.

### Without Hercules Desktop:

For a command-line only install without Hercules Desktop, run:
...
After installing, add an `OPENROUTER_API_KEY` to `~/.hercules/.env` and run `hercules model` to pick a model — one API key covers 300+ models. See [OpenRouter](/integrations/providers#openrouter).
:::
...
| 🔌 **[MCP Integration](/user-guide/features/mcp)**                      | Connect to MCP servers, filter their tools, and extend Hercules safely  |
| 🧭 **[Use MCP with Hercules](/guides/use-mcp-with-hercules)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with Hercules](/guides/use-voice-mode-with-hercules)** | Hands-on setup and usage patterns for Hercules voice workflows          |
| 🎭 **[Personality & SOUL.md](/user-guide/features/personality)**        | Define Hercules' default voice with a global SOUL.md                    |
...
| 💡 **[Tips & Best Practices](/guides/tips)**                            | Quick wins to get the most out of Hercules                              |
...
- **Built by model trainers** — Created by [Nous Research](https://nousresearch.com), the lab behind Hercules, Nomos, and Psyche. Works with [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, and TTS built in — see the [Tool Gateway](/user-guide/features/tool-gateway)
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [Nous Research](https://nousresearch.com) — the lab behind Hercules, Nomos, and Psyche models
