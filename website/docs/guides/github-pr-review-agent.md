...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Hercules instantly when PRs are opened or updated.
:::
...
- **Hercules Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  hercules gateway install   # Install as a service
  # or
  hercules gateway           # Run in foreground
  ```
...
Use `deliver: "local"` to save reviews to `~/.hercules/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Hercules can access GitHub. Start a chat:
...
```
Run: gh pr list --repo mintoriakamoto/Hercules --state open --limit 3
```
...
Still in the chat, ask Hercules to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo mintoriakamoto/Hercules
```
...
A skill gives Hercules consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.hercules/skills/code-review
```

Create `~/.hercules/skills/code-review/SKILL.md`:
...
Verify it loaded — start `hercules` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Hercules your team's standards:
...
2. Teach Hercules your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
hercules gateway status    # Is the gateway running?
hercules cron list         # Is the job enabled?
```
