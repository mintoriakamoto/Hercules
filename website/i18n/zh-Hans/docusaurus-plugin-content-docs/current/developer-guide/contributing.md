...
sidebar_position: 4
title: "贡献指南"
description: "如何为 Hercules Agent 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 Hercules Agent 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
- 构建自定义/本地工具而不修改 Hercules 核心？从 [构建 Hercules 插件](../developer-guide/plugins/index.md) 开始
- 为 Hercules 本身构建新的内置核心工具？从 [添加工具](./adding-tools.md) 开始
...
```bash
# 标准安装器已经把 `hercules` 放到了 PATH 上。
hercules doctor
hercules chat -q "Hello"
```

如果你使用了手动克隆备用路径，可以在 checkout 中运行 `./hercules`，或显式把这个 clone 的 venv 链接到 PATH：
...
- **Profile 安全路径**：不得硬编码 `~/.hercules` — 代码路径使用 `hercules_constants` 中的 `get_hercules_home()`，面向用户的消息使用 `display_hercules_home()`。完整规则参见 [AGENTS.md](https://github.com/mintoriakamoto/Hercules/blob/main/AGENTS.md#profiles-multi-instance-support)。

## 跨平台兼容性

Hercules 官方支持 **Linux、macOS、WSL2 以及原生 Windows（通过 PowerShell 安装）**。原生 Windows 使用 [Git for Windows](https://git-scm.com/download/win) 提供的 Git Bash 执行 shell 命令。部分功能依赖 POSIX 内核原语，已做条件限制：dashboard 内嵌的 PTY 终端面板（`/chat` 标签页）仅支持 WSL2。如果您主要在 Windows 上开发，推送前请运行 Windows 陷阱（footgun）lint（`scripts/check-windows-footguns.py`）。
...
2. **手动测试**：运行 `hercules` 并验证您修改的代码路径
...
- 使用 [GitHub Issues](https://github.com/mintoriakamoto/Hercules/issues)
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/mintoriakamoto/Hercules/blob/main/LICENSE) 授权。