...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/mintoriakamoto/Hercules/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hercules_meta = (fm.get("metadata") or {}).get("hercules") or {}
    tags = hercules_meta.get("tags") or []
    related = hercules_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `hercules skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Hercules ships with a large built-in skill library copied into `~/.hercules/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Hercules also syncs bundled skills on `hercules update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hercules/skills/` tree, it is still shipped with Hercules; restore it with `hercules skills reset <name> --restore`.",
        "",
